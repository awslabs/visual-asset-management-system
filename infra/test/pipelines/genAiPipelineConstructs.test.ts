...
describe.each([
    [
        // Cosmos 3 and Cosmos Predict were absent from this table while three sibling GPU pipelines
        // were covered, which is why both shipped without the Batch terminate grant and without a
        // Catch on their first state: every assertion below existed and simply did not run for them.
        "Cosmos3",
        (h: Harness) =>
    ...
    test("the first state catches to a handler that reaches PipelineEndTask", () => {
        // PipelineEndTask is the only state that resolves the parent workflow's task token. Without a
        // Catch on the first state, a pre-Batch failure ends the execution before it runs and the
        // parent's waitForCallback task stays RUNNING for its whole taskTimeout — eight hours here —
        // for a job that failed in under a second.
        const definitions = Object.values(stateMachineDefinitions(template));
        expect(definitions.length).toBeGreaterThan(0);
        for (const definition of definitions) {
            ...
            // Followed through the handler rather than merely asserting a Catch exists: a Catch to a
            // terminal Fail state would satisfy "has error handling" while leaving the token unreleased.
            expect(first.Catch).toBeDefined();
            const handler = asl.States[first.Catch[0].Next];
            expect(handler).toBeDefined();
            expect(handler.Next).toMatch(/^PipelineEndTask/);
        }
    });
