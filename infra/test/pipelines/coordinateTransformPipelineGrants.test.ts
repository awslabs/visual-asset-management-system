 ...
 *
 *   1. The container JOB role must carry the shared-key KMS grant, and the EXECUTION role must not.
 *      Every object the Batch job reads and writes lives in a CMK-encrypted bucket whenever
 *      useKmsCmkEncryption is on, so without the grant on the job role each job dies on its first S3
 *      call with an AccessDenied logged inside the container. The execution role is a different
 *      principal with a different job — see the block that asserts the split.
...
describe("CoordTransformContainerJobRole", () => {
    test("can use the shared KMS key for both the input and the output bucket policy", () => {
        const statements = inlineStatements("CoordTransformContainerJobRole");
...
/**
 * The EXECUTION role is deliberately minimal, and this block replaces an assertion that required the
 * opposite.
 *
 * The two roles previously received the same three inline policies, and this file asserted the KMS grant
 * on BOTH. That was wrong about which role does the work: a container's own AWS calls are signed with the
 * TASK role, while the execution role is what the ECS agent assumes to pull the image and write the task's
 * log stream. Verified before changing it that the execution role has no other reason to hold the key —
 * this container definition injects no `secrets` and the Fargate construct configures no CMK-encrypted log
 * group, so the bucket policies were its only KMS path and nothing used it.
 *
 * The assertions are stated as the SPLIT rather than as a removal, so a future change that hands the
 * execution role the bucket policies again fails here rather than passing quietly.
 */
describe("CoordTransformContainerExecutionRole", () => {
    /** The execution role's resource properties. */
    const executionRole = () => {
        const roles = Object.entries(template.findResources("AWS::IAM::Role")).filter(
            ([logicalId]) => logicalId.includes("CoordTransformContainerExecutionRole")
        );
        expect(roles).toHaveLength(1);
        return (roles[0][1] as any).Properties;
    };

    test("carries no inline policies at all", () => {
        // Undefined rather than an empty array: CDK omits the property when there are none.
        expect(executionRole().Policies).toBeUndefined();
    });

    test("still carries the managed policies the ECS agent needs", () => {
        // The positive control. Without it, a change that deleted the role's permissions entirely would
        // satisfy the assertion above while making every task fail to pull its image.
        const managed = JSON.stringify(executionRole().ManagedPolicyArns ?? []);
