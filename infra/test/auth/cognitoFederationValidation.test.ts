...
jest.mock("../../config/oidc-config", () => ({
    oidcSettings: {
        ...
        scopes: ["openid", "email"],
        // A real mapping, not an empty object: getConfig() requires at least one mapped attribute,
        // because a Cognito OIDC provider with none produces a federated user carrying no claims and
        // the authorizer identifies a user by email. The shipped oidc-config.ts maps email.
...
describe("SAML provider settings are validated before deploy", () => {
    /**
     * `saml-config.ts` is NOT mocked in this file, so these tests run against the values the repository
     * actually ships — which are placeholders, including a misspelled `myidentiyprovidername`. Enabling
     * SAML against them used to deploy an identity provider that could not complete a login, with the
     * failure surfacing at the identity provider rather than at synth.
     *
     * Note what this exposed about the mutual-exclusion test above: it asserts
     * `not.toThrow(BOTH_MESSAGE)`, which passes even when a DIFFERENT error is thrown. That is why adding
     * this guard did not turn it red, and why the placeholder rejection needs its own assertion here.
     */
    afterEach(() => {
        (fs.readFileSync as unknown as jest.Mock).mockReset();
    });

    test("rejects the shipped placeholder provider name and domain prefix", () => {
        const run = loadConfig(commercialTemplate, "us-east-1", (c) => {
            c.app.authProvider.useCognito.useSaml = true;
        });
        expect(run).toThrow(/saml-config\.ts still holds placeholder values/);
    });

    test("names every offending field, so one edit does not hide the others", () => {
        const run = loadConfig(commercialTemplate, "us-east-1", (c) => {
            c.app.authProvider.useCognito.useSaml = true;
        });
        // All three shipped values are placeholders; reporting only the first would send the operator
        // back for a second and third deploy.
        expect(run).toThrow(/name/);
        expect(run).toThrow(/cognitoDomainPrefix/);
        expect(run).toThrow(/metadataContent/);
    });

    test("the guard does not fire when SAML is disabled", () => {
        // The control. The shipped placeholders must not block a plain-Cognito deployment, which is what
        // every default config is — a guard that fired unconditionally would break every deploy.
        const run = loadConfig(commercialTemplate, "us-east-1", (c) => {
    ...
    test("the guard does not fire when Cognito itself is disabled", () => {
        // useSaml is resolved to false when Cognito is off, so the placeholders are irrelevant there.
        const run = loadConfig(commercialTemplate, "us-east-1", (c) => {
            c.app.authProvider.useCognito.enabled = false;
            c.app.authProvider.useCognito.useSaml = true;
            c.app.authProvider.useExternalOAuthIdp.enabled = true;
        });
        expect(run).not.toThrow(/saml-config\.ts still holds placeholder values/);
    });
});
