...
import { LAMBDA_MEMORY_SIZE } from "../../config/config";
...
/**
 * How many web-bundle deployments each shape emits.
 *
 * TWO, not one. Both constructs upload the built bundle twice: the main deployment excludes `*.mjs`, and a
 * second one uploads only `*.mjs` with an explicit `text/javascript` Content-Type. `BucketDeployment`
 * applies `contentType` to every object in a deployment, so an extension needing its own type can only be
 * handled by a deployment of its own — left to the uploader's own guess, `.mjs` becomes `text/plain` and a
 * browser refuses it as a module script, which is what broke the PDF viewer's worker.
 *
 * Both expand the SAME asset, so both need the raised /tmp budget this file is about, and the assertions
 * below check every one rather than a representative.
 */
const WEB_DEPLOYMENTS_PER_SHAPE = 2;

/**
 * Every web-bundle deployment in a synth, with the count asserted.
 *
 * Throws rather than returning an empty list when the count is wrong: a shape that emitted no
 * BucketDeployment at all must fail the /tmp assertions instead of satisfying them by absence, and a shape
 * that stopped emitting the ES-module pass must fail rather than quietly reverting `.mjs` to `text/plain`.
 */
function webBundleDeployments(s: SynthResult): Deployment[] {
    const found = webDeployments(s);
    if (found.length !== WEB_DEPLOYMENTS_PER_SHAPE) {
        const total = s.countOfType(BUCKET_DEPLOYMENT);
        throw new Error(
            `expected exactly ${WEB_DEPLOYMENTS_PER_SHAPE} web-bundle BucketDeployments in the ` +
                `StaticWeb stack (the bundle, and the ES-module pass that types *.mjs), found ` +
                `${found.length} (${total} bucket deployments across the whole assembly). An ` +
                `assertion on their /tmp budget would prove nothing.`
        );
    }
    return found;
}

/** The first web-bundle deployment, for assertions that hold for any of them. */
function webDeployment(s: SynthResult): Deployment {
    return webBundleDeployments(s)[0];
}
...
describe("harness controls: both web deployment shapes emit a bucket deployment", () => {
    test("the CloudFront shape emits both web bundle deployments, from the CloudFront construct", () => {
        const s = S.cloudFront();
        const all = webBundleDeployments(s);
        const d = all.find((x) => x.cr.properties.DistributionId !== undefined) ?? all[0];
    ...
    test("the ALB hybrid emits both web bundle deployments, from the ALB construct", () => {
        const s = S.alb();
        const d = webBundleDeployments(s)[0];
        expect(destinationBucketId(d)).toMatch(/WebAppBucket/);
    ...
    test("the resolved uploader is a real function and keeps the project memory limit", () => {
        ...
        // trading memory away.
        //
        // Asserted against Config.LAMBDA_MEMORY_SIZE rather than a literal. This test previously hardcoded
        // 1024, which the constructs then raised to the project-wide figure without it being updated — a
        // stale literal here fails for a reason that has nothing to do with the /tmp budget the file is
        // about, and sends the reader to the wrong place.
        for (const d of [
            ...webBundleDeployments(S.cloudFront()),
            ...webBundleDeployments(S.alb()),
        ]) {
            expect(d.handler.properties.Handler).toBe("index.handler");
            expect(d.handler.properties.MemorySize).toBe(LAMBDA_MEMORY_SIZE);
        }
    });
});
...
describe("the web bundle uploader gets more /tmp than the Lambda default", () => {
    it("CloudFront path: the BucketDeployment Lambda carries an ephemeral storage size above 512 MiB", () => {
        for (const d of webBundleDeployments(S.cloudFront())) {
            const tmpMiB = tmpBudget(d);
