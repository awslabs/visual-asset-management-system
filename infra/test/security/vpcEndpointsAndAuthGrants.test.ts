...
describe("Batch/ECS/Fargate pipeline VPC condition blocks", () => {
    ...
    /**
     * Which blocks a pipeline belongs in depends on the subnets its compute runs in, not on whether it
     * uses Batch. A pipeline placed in PRIVATE subnets needs all three; one placed in ISOLATED subnets
     * needs only the endpoint block, and listing it in the other two creates a NAT gateway per
     * Availability Zone plus an ECS control-plane endpoint that its tasks never use.
     *
     * The split is read from `pipelineBuilder-nestedStack.ts`, which passes either
     * `pipelineNetwork.privateSubnets.pipeline` or `pipelineNetwork.isolatedSubnets.pipeline`.
     */
    const privateSubnetBatchFlags = [
    ...
    const isolatedSubnetBatchFlags = [
    ...
    test.each(privateSubnetBatchFlags)(
        "%s runs in private subnets, so it appears in all three condition blocks",
        (flag) => {
    ...
    test.each(isolatedSubnetBatchFlags)(
        "%s runs in isolated subnets, so it appears in the endpoint block ONLY",
        (flag) => {
            // The endpoint block is what its Fargate tasks actually need — Batch, ECR API, ECR Docker.
            // Present is the load-bearing half: without it the tasks cannot pull their image, which is
            // a silent break far worse than the wasted NAT gateway the other two assertions prevent.
            expect(blocks.batchBlock).toContain(flag);
            expect(blocks.subnetBlock).not.toContain(flag);
            expect(blocks.ecsBlock).not.toContain(flag);
        }
    );
