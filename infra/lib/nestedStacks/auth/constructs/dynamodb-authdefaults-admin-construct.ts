        ...
        for (const constraint of adminInitialConstraints) {
            ...
            for (const item of denormalizedItems) {
                const awsSdkCall: AwsSdkCall = {
                    ...
                    physicalResourceId: PhysicalResourceId.of(
                        `${props.storageResources.dynamo.constraintsStorageTable.tableName}_initialization${roleNameIDCleanAdmin}_${i}`
                    ),
                };

                new AwsCustomResource(
                    this,
                    `constraintsStorageTable_${roleNameIDCleanAdmin}CustomResource_${i}`,
                    {
                        onCreate: awsSdkCall,
                        onUpdate: awsSdkCall,
                        role: props.customResourceRole,
