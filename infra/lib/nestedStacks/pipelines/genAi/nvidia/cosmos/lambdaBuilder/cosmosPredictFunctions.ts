...
export function buildVamsExecuteCosmosText2WorldPipelineFunction(
    ...
    const name = "vamsExecuteCosmosText2WorldPipeline";
    const constructId = modelKey ? `${name}-${modelKey}` : name;
    const fun = new lambda.Function(scope, constructId, {
        code: lambda.Code.fromAsset(
            path.join(
                __dirname,
                `../../../../../../../../backendPipelines/genAi/nvidia/cosmos/predict/lambda`
            )
        ),
...
export function buildVamsExecuteCosmosVideo2WorldPipelineFunction(
    scope: Construct,
    lambdaCommonBaseLayer: LayerVersion,
    openPipelineLambdaFunction: lambda.IFunction,
    config: Config.Config,
    ...
    const name = "vamsExecuteCosmosVideo2WorldPipeline";
    const constructId = modelKey ? `${name}-${modelKey}` : name;
    const fun = new lambda.Function(scope, constructId, {
        code: lambda.Code.fromAsset(
            path.join(
