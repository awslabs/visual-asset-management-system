...
export class ModelOpsConstruct extends NestedStack {
    ...
    constructor(parent: Construct, name: string, props: ModelOpsConstructProps) {
        ...
        // former per-output-format registrations (usdz / glb / gltf, with a duplicate) collapse to ONE
        // pipeline + three templates (model-ops-to-{usdz,glb,gltf}) selected per execution.
        if (props.config.app.pipelines.useModelOps.autoRegisterWithVAMS === true) {
            new VamsSchemaRegistration(this, "ModelOpsRegistration", {
                ...
                idOverrides: {
                    pipelineId: "vntana-model-ops",
                    workflowId: "vntana-model-ops",
                },
            });
        }
