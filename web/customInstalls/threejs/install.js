...
const auditFix = async () => {
    console.log("ThreeJS: Running npm audit fix (safe fixes only)...");
    try {
        await execSync("npm audit fix", { cwd: npmPackageDir, stdio: "inherit" });
        console.log("ThreeJS: npm audit fix complete");
    } catch (err) {
        console.warn(
            "ThreeJS: npm audit fix reported unresolved/unfixable vulnerabilities (continuing)."
        );
    }
};
