...
const auditFix = async () => {
    console.log("BabylonJS: Running npm audit fix (safe fixes only)...");
    try {
        await execSync("npm audit fix", { cwd: npmPackageDir, stdio: "inherit" });
        console.log("BabylonJS: npm audit fix complete");
    } catch (err) {
        console.warn(
            "BabylonJS: npm audit fix reported unresolved/unfixable vulnerabilities (continuing)."
        );
    }
};
