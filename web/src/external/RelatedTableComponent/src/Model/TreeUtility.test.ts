...
describe('TreeUtility', () => {
  ...
  beforeEach(() => {
    const newNodes = TreeUtility.buildTreeNodes(items, new Map() as TreeMap<any>, 'id', 'parentId');
  ...
  it('custom filtering function by name', () => {
    ...
    const nodesFiltered = nodes.filter((node) =>
      TreeUtility.filteringFunction(node, 'Great Grand Child', undefined, customFilteringFunction)
    );
    expect(nodesFiltered.length).toEqual(6);
  });
