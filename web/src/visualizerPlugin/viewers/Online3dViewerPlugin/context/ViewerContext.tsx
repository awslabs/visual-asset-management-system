                ...
                try {
                    ...
                    if (selection.type === "Material" && selection.materialIndex !== undefined) {
                        ...
                        if (viewer && viewer.SetMeshesHighlight) {
                            ...
                            viewer.Render();
                            console.log(
                                "Material selection highlighting applied:",
                                selection.materialIndex
                            );
                        }
                        ...
                        if (viewer && viewer.SetMeshesHighlight) {
                            ...
                            viewer.Render();
                            console.log(
                                "Mesh selection highlighting applied:",
    ...
    useEffect(() => {
        if (state.viewerState.viewer && state.viewerState.model && state.selection.type) {
            try {
                ...
                if (
                    state.selection.type === "Material" &&
                    state.selection.materialIndex !== undefined
                ) {
                    // Update mesh highlighting for material selection
                    ...
                    if (viewer && (viewer as any).SetMeshesHighlight) {
                        const highlightColor = { r: 142, g: 201, b: 240 };
                        console.log(
                            "Attempting to highlight mesh with ID:",
                            state.selection.meshInstanceId
                        );
                        ...
                        (viewer as any).SetMeshesHighlight(highlightColor, (meshUserData: any) => {
                            ...
                            if (typeof targetId === "string" && targetId.startsWith("mesh_")) {
                                ...
                                if (
                                    meshUserData.originalMeshInstance?.id?.meshIndex === meshIndex
                                ) {
                                    matchFound = true;
                            ...
                            if (
                                ...
                                (targetId as any).IsEqual
                            ) {
                                if (
                                    meshUserData.originalMeshInstance?.id &&
                                    (targetId as any).IsEqual(meshUserData.originalMeshInstance.id)
                                ) {
                                    matchFound = true;
                        ...
                        (viewer as any).Render();
                        console.log(
                            "Mesh selection highlighting applied. Match found:",
                            matchFound
                        );
                    }
                }
