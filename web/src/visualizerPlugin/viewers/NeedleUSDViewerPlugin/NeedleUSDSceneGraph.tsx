...
const NeedleUSDSceneGraph: React.FC<NeedleUSDSceneGraphProps> = ({
    ...
    // Force re-render when selection changes to update UI highlighting
    useEffect(() => {
        ...
        (node: SceneNode, event: React.MouseEvent) => {
            ...
            if (event.ctrlKey) {
                ...
                if (isSelected) {
                    ...
                    onSelectObjects([...externalSelectedObjects, node.object]);
                }
            } else {
                // Regular click: Replace selection
                if (isSelected && externalSelectedObjects.length === 1) {
                    // Deselect if it's the only selected object
