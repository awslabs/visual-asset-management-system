...
export function onUploadRetry(uploadExecutionProps: UploadExecutionProps) {
    ...
    const failedItems = uploadExecutionProps.assetDetail.Asset?.filter(item => item.status === "Failed") || [];
        ...
        try {
            ...
            for (let i = 0; i < filesToProcess.length; i++) {
                ...
                for (let j = 0; j < fileResponse.numParts; j++) {
                    ...
                    if (!partUrl) {
                        throw new Error(`Missing presigned URL for part ${partNumber} of file ${file.name}`);
                    }
                    ...
                    const blob = await file.handle.getFile().then((f: File) => f.slice(start, end));
                    ...
                    AssetUploadService.uploadPart(partUrl, blob)
                        .then(etag => {
                            ...
                            const progress = Math.round((completedParts / totalParts) * file.size);
            ...
            await AssetUploadService.completeUpload(
                initResponse.uploadId,
                completionRequest
            );
