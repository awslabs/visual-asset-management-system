...
async function downloadAllFilesRecursively(tree: FileTree, directoryHandle: any, dispatch: any) {
    for (let subtree of tree.subTree) {
        if (subtree.subTree.length > 0) {
            const subFolderDirectoryHandle = await directoryHandle.getDirectoryHandle(subtree.name, {create: true});
            ...
            dispatch({type: "UPDATE_STATUS", payload: {relativePath: subtree.relativePath, status: "Completed"}})
        }
    }
}
...
function assetDownloadReducer(state: FileUploadTableItem[], action: {type: string, payload: any}): FileUploadTableItem[] {
    switch (action.type) {
        case "UPDATE_INDICES":
...
export default function AssetDownloadsPage() {
    ...
    const [fileUploadTableItemsState, dispatch] = useReducer(assetDownloadReducer, fileUploadTableItems);
    return (
        <div>
            <h1> Downloading {fileTree.relativePath} </h1>
            <FileUploadTable allItems={fileUploadTableItemsState}
                             resume={resume}
                             onRetry={() => {downloadFolder(fileTree, dispatch).then(() => {setResume(false)})}}
                             mode={"Download"}/>
        </div>
    )
}