    ...
    return (
        <>
            {selectedItems.map(item => {
                ...
                return (
                    <ExpandableSection defaultExpanded key={item.assetId} headerText={item.assetName}>
                        {allComments.map(comment => {
                            nestedCommentBool = (comment.commentOwnerID === previousOwnerId) && (moment(comment.dateCreated).diff(previousCommentTime, 'minutes') < 10);
