 ...
 * @returns {Promise<boolean|{message}|any>}
 */
export const createAssetVersion = async ({ databaseId, assetId, useLatestFiles = true, files = [], comment }, api = API) => {
    try {
        if (!databaseId || !assetId) {
            return [false, "Database ID and Asset ID are required"];
        }
 ...
 * @returns {Promise<boolean|{message}|any>}
 */
export const revertAssetVersion = async ({ databaseId, assetId, assetVersionId, comment = "" }, api = API) => {
    try {
        if (!databaseId || !assetId || !assetVersionId) {
            return [false, "Database ID, Asset ID, and Asset Version ID are required"];
        }
 ...
 * @returns {Promise<boolean|{message}|any>}
 */
export const fetchAssetS3Files = async ({ databaseId, assetId, includeArchived = false }, api = API) => {
    try {
        if (!databaseId || !assetId) {
            return [false, "Database ID and Asset ID are required"];
        }
