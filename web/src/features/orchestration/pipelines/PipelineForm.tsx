...
const PipelineForm: React.FC<PipelineFormProps> = ({
    ...
    // A system pipeline (shipped by a vamsSchema bundle) accepts only `enabled` on update; every other
    // field is locked and the save sends nothing else.
    const isSystemPipeline = mode === "edit" && !!initial?.isSystem;
        ...
        if (isSystemPipeline) {
            try {
                ...
                toast.success("Pipeline saved", {
                    description: initial?.pipelineName || undefined,
                });
                setIsOpen(false);
                onDone();
            } catch (err) {
        ...
        <form id="pipeline-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {isSystemPipeline && (
                <div className="p-3 bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 rounded">
                    <strong>System pipeline:</strong> shipped with the deployment and read-only
                    ...
                    >
                        <div>
                            ...
                            >
                                ...
                                {(showDeadlineCloud ||
                                    initial?.executionConfig?.executionType ===
                                        "DeadlineCloud") && (
                            ...
                            <>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        {...register(
                                            "executionConfig.deadlineCloud.storageProfileId"
                                        )}
                                        disabled={isDeadlineCloudDisabled}
                                        className="orch-outline w-full px-3 py-2 border border-border-input rounded bg-surface-input text-text-primary disabled:opacity-50"
                                    />
                        ...
                        <div>
                            ...
                            <select
                                ...
                                disabled={
                                    executionType === "DeadlineCloud" || isDeadlineCloudDisabled
                                }
                                className="orch-outline w-full px-3 py-2 border border-border-input rounded bg-surface-input text-text-primary disabled:opacity-50"
                            >
                        ...
                        <div>
                            ...
                            {undeclaredRules.length > 0 && (
                                <p className="text-xs text-text-secondary mb-2">
                                    ...
                                    undeclared rule defers to the workflow&apos;s own rule; changing
                                    it below declares it here.
                                </p>
                            )}
                            <AssetSpanControl
                                ...
                                onChange={(s) =>
                                    setValue(
                                        "systemConfig.assetScope",
                                        s as Record<string, boolean>
                                    )
                                }
                            />
                        </div>
