    ...
    const deleteLink = async () => {
        try {
            ...
            if (res.response && res.response.status === 403) {
                ...
                showMessage({
                    type: "success",
                    message: `Successfully deleted ${modalTitle.toLowerCase()} link to ${
                        modalAsset.assetName
                    }`,
                    dismissible: true,
                    autoDismiss: true,
                });
        ...
        API.post("api", "asset-links", {
            ...
            .catch((err) => {
                if (err.response?.status === 400) {
                    ...
                    let msg = `Unable to add ${linktype} link. Error: Request failed with status code ${
