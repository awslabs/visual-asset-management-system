...
const getAllFiles = (multiFileSelect: MultiFileSelectState): FileInfo[] => {
    ...
    for (const directory in multiFileSelect.directories) {
        if (multiFileSelect.directories[directory]) {
            fileInfo = [
                ...fileInfo,
                ...multiFileSelect.directories[directory]
            ]
        }
    }
...
export function MultiFileSelect({
    ...
    useEffect(() => { onChange(getAllFiles(state)) },[state])
    return (
        <Container>
            <FormField>
                <Toggle
                    onChange={() => setIsMultiFile(!isMultiFile)}
                    checked={isMultiFile}
                >
            ...
            <SpaceBetween size={"xs"} direction={"vertical"}>
                <FolderUpload
                    ...
                    description={`Total Files to upload: ${state.files.length + Object.keys(state.directories).reduce((acc, directory) => {
                ...
                ></FolderUpload>
                <SpaceBetween size={"xs"} direction={"vertical"}>
                    {state.directories && Object.keys(state.directories)
                        ...
                        .map(directory => {
                            return (
                                <Button variant={"primary"}
                                    ...
                                    Directory {directory} - {state.directories[directory].length} Files
                                </Button>
                            )
                        })
                    }
                ...
                <SpaceBetween size={"xs"} direction={"vertical"}>
                    {state.files && state.files.map(file => {
                            return (
                                <Button variant={"primary"} key={file} onClick={() => dispatch({
                                    type: 'REMOVE_FILE',
                                    payload: {
