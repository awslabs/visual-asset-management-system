...
class SearchCacheManager {
    ...
    getAsset(key: string): AssetDetails | null {
        ...
        if (age > this.ASSET_TTL_MS) {
            console.log(`[Cache CHECK] Asset key: ${key} - EXPIRED (age: ${age}ms, TTL: ${this.ASSET_TTL_MS}ms)`);
    ...
    setAsset(key: string, data: AssetDetails): void {
        ...
        console.log(`[Cache SET] Asset details for key: ${key}, cache size: ${this.assetCache.size}`);
    }
    ...
    setPreview(key: string, data: PreviewImage, sizeBytes: number): void {
        ...
        while (this.previewCacheSize + sizeBytes > this.MAX_PREVIEW_SIZE_BYTES && this.previewCache.size > 0) {
