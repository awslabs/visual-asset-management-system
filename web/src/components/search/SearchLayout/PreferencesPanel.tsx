...
const PreferencesPanel: React.FC<PreferencesPanelProps> = ({
    ...
    const assetSpecificColumns = [
        'str_assetname', 'str_assetid', 'str_assettype', 'str_description',
        'str_bucketid', 'str_bucketname', 'str_bucketprefix',
        'str_asset_version_id', 'date_asset_version_createdate', 'str_asset_version_comment',
        'bool_isdistributable',
        'bool_has_asset_children', 'bool_has_asset_parents', 'bool_has_assets_related'
    ];
    
    const fileSpecificColumns = [
        'str_key', 'str_assetname', 'str_bucketid', 'str_bucketname', 'str_bucketprefix',
        'str_fileext', 'num_filesize',
        'date_lastmodified', 'str_etag', 'str_s3_version_id'
    ];
    
    const commonColumns = [
        'str_databaseid', 'list_tags', 'bool_archived',
        'date_created', 'str_createdby', 'metadata'
    ];
    ...
    const currentColumns = recordType === 'asset'
    ...
    const handleResetPreferences = () => {
        ...
        const defaultAssetColumns = ['str_assetname', 'str_databaseid', 'str_assettype', 'str_description', 'str_asset_version_id', 'list_tags', 'metadata'];
        const defaultFileColumns = ['str_key', 'str_assetname', 'str_databaseid', 'str_fileext', 'num_filesize', 'date_lastmodified', 'list_tags', 'metadata'];
        ...
        >
            <SpaceBetween direction="vertical" size="m">
                ...
                >
                    <Select
                        selectedOption={
                            pageSizeOptions.find((opt) => opt.value === String(preferences.pageSize)) ||
                            pageSizeOptions[2]
                        }
                        onChange={({ detail }) =>
                            onPreferencesChange({ pageSize: parseInt(detail.selectedOption.value || '100') })
                        }
