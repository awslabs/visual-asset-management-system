...
const ModernSearchContainer: React.FC<SearchContainerProps> = ({
    ...
    const { preferences, updatePreferences, savePreferences, hasUnsavedChanges, isLoaded: preferencesLoaded } = usePreferences();
    ...
    useEffect(() => {
        if (!searchState.initialResult && !useNoOpenSearch && preferencesLoaded && searchState.pagination.size === preferences.pageSize) {
    ...
    const handleSearch = async () => {
        try {
            ...
            const result = await searchAPI.executeSearch(searchQuery, databaseId, metadataSearchMode, metadataOperator);
    ...
    useEffect(() => {
        ...
        const currentColumns = recordType === 'asset' ? preferences.assetTableColumns : preferences.fileTableColumns;
        ...
        if (hasArchivedFilter && !hasArchivedColumn && currentColumns) {
            ...
            if (recordType === 'asset') {
                updatePreferences({ assetTableColumns: currentColumns.filter(col => col !== 'bool_archived') });
            } else {
                updatePreferences({ fileTableColumns: currentColumns.filter(col => col !== 'bool_archived') });
            }
        }
    ...
    const handlePagination = async (from: number, size?: number) => {
        try {
            console.log('[Pagination] handlePagination called with:', { from, size, pageSize: preferences.pageSize });
            searchState.setPagination({ from, size: size || preferences.pageSize });
            console.log('[Pagination] After setPagination, searchState.pagination:', searchState.pagination);
            await handleSearch();
        } catch (error: any) {
                ...
                return (
                    <SearchPageListView
                        state={{
                            ...
                            tablePreferences: {
                                pageSize: preferences.pageSize,
                                visibleContent: recordType === 'asset'
                            ...
                            switch (action.type) {
                                ...
                                case 'query-paginate':
                                    if (action.pagination) {
                                        console.log('[Pagination] query-paginate action received:', action.pagination);
                                        ...
                                        (async () => {
                                            try {
                                                searchState.setLoading(true);
                                                // Pass pagination directly to buildSearchQuery to avoid stale closure
                                                const searchQuery = searchState.buildSearchQuery(action.pagination);
                                                console.log('[Pagination] Executing search with pagination:', searchQuery.pagination);
                                                const result = await searchAPI.executeSearch(searchQuery, databaseId, metadataSearchMode, metadataOperator);
                                                searchState.setResult(result);
                                            } catch (error: any) {
    ...
    return (
        <div style={containerStyle}>
            ...
            <SearchTopBar
                ...
                title={embedded?.title || (databaseId ? `${Synonyms.Assets} for ${databaseId}` : 'Assets and Files - Search')}
            ...
            <ResizableSplitter
                leftPanel={
                    ...
                    onAddMetadataFilter={() => searchState.addMetadataFilter({
                        key: '',
                        operator: '=',
                        value: '',
                        type: 'string',
