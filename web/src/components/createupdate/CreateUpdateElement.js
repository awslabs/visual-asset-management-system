...
import { ACTIONS } from "../../services/APIService";
      ...
      if (validateForm({ isSubmit: true })) {
        const formattedFormValues = Object.keys(formValues).reduce(
          (acc, cur) => {
            ...
            } else if (formValues[cur] && formValues[cur].value) {
        ...
        API.put("api", pluralName, config)
          .then((response) => {
            ...
            if (
              ...
              setSubmitUpdateError(
                `Unable to ${actionStrings[actionType].lowerCase} ${singularName}. Error: ${response.message}`
              );
            } else {
              handleClose();
              setReload(true);
          ...
          .catch((error) => {
            setReadySubmit(false);
            setSubmitUpdateError(
              `Unable to ${actionStrings[actionType].lowerCase} ${singularName}. Error: ${error?.message}`
            );
            console.log(error);
          })
          .finally(() => {
