...
export default function ViewAsset() {
        ...
        const getData = async () => {
            if (databaseId && assetId) {
                try {
                    ...
                    if (item !== false) {
                        console.log("Asset data fetched successfully:", item);
                        setAsset(item);
    ...
    // Separate useEffect for fetching asset files after asset data is loaded
    useEffect(() => {
        const fetchFiles = async () => {
            if (asset && asset.assetId) {
                ...
                try {
                    const files = await fetchAssetS3Files({ databaseId: databaseId!, assetId: assetId!, includeArchived: false });
                                                ...
                                                <div style={{ marginBottom: "16px" }}>
                                                    ...
                                                    {asset?.currentVersion?.Version && (
                                                        <Button
                                                            iconName="external"
                                                            variant="link"
                                                            onClick={() => navigate(`/databases/${databaseId}/assets/${assetId}/versions`)}
                                                        >
                                                            Manage Versions
                                                        </Button>
                                                    )}
