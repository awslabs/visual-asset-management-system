...
import {
  ...
  modelFileFormats,
  cadFileFormats,
  archiveFileFormats, presentationFileFormats,
...
export default function ViewAsset() {
  ...
  const generateUrl = async () => {
    const newDownloadUrl = await Storage.get(asset?.assetLocation?.Key);
    setDownloadUrl(newDownloadUrl);
  };

  useEffect(() => {
    const getData = async () => {
      const item = await fetchAsset(databaseId, assetId);
      if (item !== false) {
        setAsset(item);
