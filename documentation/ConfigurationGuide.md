...
-   `app.pipelines.usePreview3dThumbnail.enabled` | default: false | #Feature to create a 3D preview thumbnail generation pipeline for mesh files (PLY, STL, OBJ, GLB, GLTF, FBX, DRC), point clouds (LAS, LAZ, E57, PTX, PCD, FLS, FWS), CAD files (STP, STEP), and USD files (USD, USDA, USDC, USDZ). Generates rotating animated GIF previews and static PNG thumbnails for asset file preview within the VAMS web UI. This will enable the global VPC option and all pipeline components will be put behind the VPC. Supports input files up to 100GB with 200 GiB ephemeral container storage.
-   `app.pipelines.usePreview3dThumbnail.autoRegisterWithVAMS` | default: false | #Feature to automatically register the 3D preview thumbnail pipeline and associated workflow in the global VAMS database during CDK deployment. When enabled, the pipeline will be available immediately after deployment without manual registration through the UI.
-   `app.pipelines.usePreview3dThumbnail.autoRegisterAutoTriggerOnFileUpload` | default: false | #Feature to automatically trigger the pipeline when files are uploaded to VAMS. When enabled along with autoRegisterWithVAMS, the pipeline will automatically execute on file uploads matching the pipeline's supported 3D file types.
