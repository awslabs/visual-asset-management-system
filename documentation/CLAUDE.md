...
-   **Source pages**: `documentation/docusaurus-site/docs/` (136 Markdown and MDX pages)
...
├── pipelines/                  # Pipeline overview + system pipelines (category page + 2 pipelines) + 12 other pipeline docs + custom pipeline guide + v2.5→v2.6 porting guide
...
| New/changed pipeline                             | `pipelines/` new page + `pipelines/overview.md` table + `sidebars.ts`; `overview/features.md` table **and its spelled-out built-in-pipeline count** (the sentence "VAMS includes _twenty-three_ built-in processing pipelines…" must equal the table's row count); a system pipeline (an `isSystem` bundle) is additionally listed in `pipelines/system-pipelines.md`; and, when the pipeline adds/changes a third-party model, base image, or licensed dependency, the license entries in **both** `additional/notices.md` (per-pipeline license paragraph + closing attribution list) and the repo-root `NOTICE.md` (per-pipeline dependency table + attribution note). Record the exact license and any required attribution string. |
...
| New feature switch                               | The feature-flag tables in `overview/features.md` and `architecture/details.md` (both must list every `VAMS_APP_FEATURES` member), plus the `concepts/` or `user-guide/` pages the switch gates                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
