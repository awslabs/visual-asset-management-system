...
| `allowWorkflowTriggerChaining`                | Whether another workflow's output may fire this workflow's triggers. A file whose provenance records this workflow as its writer never re-triggers it, whatever this is set to — narrower than it reads; see the warning below. Defaults to `false`.    |
