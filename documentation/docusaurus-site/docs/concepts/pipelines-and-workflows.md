...
VAMS includes several built-in pipelines that are auto-registered as `GLOBAL` during deployment. These include 3D model conversion, point cloud processing, Gaussian splatting, GenAI metadata generation, and 3D preview thumbnail generation. Built-in pipelines are configured through the CDK deployment configuration. For details, see the [Pipelines](../pipelines/overview.md) section.
:::

:::warning[Built-in pipelines, workflows, and templates are schema-owned by the CDK]
A built-in pipeline, its workflow, its shipped templates, and its file-upload trigger are defined by a `vamsSchema` bundle that lives with the pipeline's source and is registered by the deployment. The deployment is the owner of those records: a change made to one through the web interface, the API, or the CLI is replaced by the bundle's definition when the deployment next registers it, and the pipeline and workflow are written back as enabled and unarchived. **Disabling or archiving a built-in in the web interface is therefore temporary — a deployment re-enables it.**

Control a built-in through the deployment configuration instead: `autoRegisterWithVAMS` decides whether it is registered at all (turning it off archives the pipeline and workflow), and `autoRegisterAutoTriggerOnFileUpload` decides whether its file-upload trigger fires. Records you create yourself are never touched by a deployment — a workflow of your own that references a built-in pipeline, and a template or an additional trigger added to a built-in under an identifier of its own — so those are the durable places to put a customization. For the field-level detail of what a registration rewrites and what it preserves, see [Update the solution](../deployment/update-the-solution.md#in-place-update). A bundle that declares `isSystem: true` goes one step further: the API itself refuses the changes a registration would undo — see [System pipelines](#system-pipelines).
:::

### System pipelines

A pipeline or workflow whose `vamsSchema` bundle declares `isSystem: true` is a **system** record: shipped by the deployment, owned by it, and read-only through the API, the web interface, the CLI, and the MCP server except for a small set of switches. System records carry a category of the form `SYSTEM - <Area>` — `SYSTEM - Preview` for the 3D preview thumbnail pipeline. Only the deployment's importer sets `isSystem`; the pipeline and workflow request models ignore the key, so a caller who sends `isSystem: true` creates an ordinary record. The value is returned on every pipeline and workflow response, where the web interface renders it as a **System** badge.

| Operation on a system record                         | Allowed                                                                                               | Response when refused                                                                                         |
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Pipeline update                                      | Only `enabled`                                                                                        | `400` `System pipelines are read-only; only "enabled" may be changed.`                                        |
| Pipeline delete (archive)                            | No                                                                                                    | `400` `System pipelines cannot be archived or restored through the API; the deployment owns them.`            |
| Pipeline restore with `PUT {"archived": false}`      | No                                                                                                    | `400` `System pipelines are read-only; only "enabled" may be changed.` (`archived` is outside the allow-list) |
| Pipeline re-create (`POST`) of an archived system id | No                                                                                                    | `400` `System pipelines cannot be archived or restored through the API; the deployment owns them.`            |
| Template create or delete on a system pipeline       | No                                                                                                    | `400` `Templates of system pipelines cannot be added or deleted.`                                             |
| Template update                                      | `configBody`, `tagSchema`, and `webFormJson`; every other supplied field must equal the stored value  | `400` naming the locked field                                                                                 |
| Template tag schema (`PUT …/tagSchema`)              | Yes                                                                                                   | —                                                                                                             |
| Workflow update                                      | Only `enabled`                                                                                        | `400` `System workflows are read-only; only "enabled" may be changed.`                                        |
| Workflow delete (archive)                            | No                                                                                                    | `400` `System workflows cannot be archived or restored through the API; the deployment owns them.`            |
| Workflow restore with `PUT {"archived": false}`      | No                                                                                                    | `400` `System workflows are read-only; only "enabled" may be changed.` (`archived` is outside the allow-list) |
| Trigger update on a system workflow                  | Only `enabled`; `inputFileFilters` and `defaultTemplateIds`, when sent, must equal the stored trigger | `400` naming the locked field                                                                                 |
| Trigger create (a new trigger key) or delete         | No                                                                                                    | `400` `Triggers of system workflows cannot be added or deleted.`                                              |
| Execute, get, list                                   | Yes                                                                                                   | —                                                                                                             |

The switches are pause switches. A deployment re-registers every bundle whose content changed and re-asserts the shipped values as it does for every built-in: the pipeline and workflow come back `enabled`, and the trigger comes back with the deployment's `autoRegisterAutoTriggerOnFileUpload` value. `isSystem` is not a permission constraint field; scope access to system records by `databaseId`, `pipelineId`/`workflowId`, or `category` as for any other pipeline or workflow.
...
| `concurrencyRestriction`                      | How concurrent executions are limited: `none`, `perAsset`, `perInputFile`, or `perInputFileVersion`.                                                                                                                                                                                                                                                                            |
| `outputTarget`                                | Where the workflow writes its output: `locationType` (`asset` or `none`) and `allowOverride`.                                                                                                                                                                                                                                                                                   |
| `allowWorkflowTriggerChaining`                | Whether another workflow's output may fire this workflow's triggers. A file whose provenance records this workflow as its writer never re-triggers it, whatever this is set to — narrower than it reads; see the warning below. A version written by a file or asset unarchive never fires any trigger: it restores content the triggers already fired on. Defaults to `false`. |
...
`concurrencyRestriction` bounds how many executions of the workflow run at once. The three restrictions are locks at different granularities, taken before the state machine starts: `perAsset` locks each selected input asset, `perInputFile` each selected input file, and `perInputFileVersion` each selected file version. A second launch that overlaps a running one at the workflow's granularity -- a duplicate upload notification, two operators submitting the same file, or two files of one asset under `perAsset` -- is rejected with `400` until the first execution finishes, fails, or is aborted; the lock row carries an expiry the table's time-to-live enforces should a release never run. Because the decision is a conditional write rather than an inspection of past executions, an asset's history never affects whether a launch is admitted. Whole-asset and folder selections lock with an empty version under `perInputFileVersion`, as do files on unversioned buckets.

:::warning[Chained workflows can trigger one another indefinitely]
Nothing bounds the length of a chain. Two workflows that each enable `allowWorkflowTriggerChaining` and each accept a file the other writes trigger one another indefinitely -- A fires on B's output, B fires on A's, and each round runs their pipelines again and writes their output again. VAMS applies no depth limit and performs no cycle detection: the self-output rule compares a file only against the workflow that produced that file, not against the chain of runs that led to it.

The self-output rule also depends on the file's recorded provenance. A file whose provenance does not name the workflow that produced it is treated as another workflow's output, because it cannot be shown to be self-output, so a workflow with chaining enabled can fire on a file it wrote itself.

Keeping a chain finite is therefore an administrative responsibility. Before enabling chaining, an administrator reviews the input-file filters and the output file types of every workflow involved, so that no group of them accepts one another's output in a cycle.
:::
...
Each execution is identified by a workflow execution id and tracked in Amazon DynamoDB, independent of any single asset (an execution may span input files across multiple assets). The main execution record holds the workflow identity, status (`NEW`, `RUNNING`, `SUCCEEDED`, `FAILED`, `TIMED_OUT`, `ABORTED`), start and stop dates, trigger type, and the initiating user. Related records capture the input files (with the exact Amazon S3 version read), the gathered metadata, per-pipeline configuration snapshots, and the produced outputs.

An output index keyed by `databaseId:assetId` records which execution wrote to each output asset, so a caller with access to an output asset can see the runs that produced it. Execution listings are permission-filtered: an execution is visible when the caller can view its workflow, every asset the run read — each input file's asset plus each asset named as a metadata source — and the asset the run wrote to whenever it wrote to one. Every asset is required rather than any one of them because the listing and the details both return the metadata of all of them, and the output asset counts on the same footing: the same views return its identity, the inventory of files written into it, and the metadata and results produced against it. A results-only run neither reads nor writes an asset, leaving the workflow as its whole gate. An asset that has been permanently deleted is authorized on the database it lived in, so deleting an asset leaves the history of the runs against it reachable by whoever can read that database rather than by nobody. Archiving an asset is not a deletion: its record is retained, so it stays authorized on its own attributes.
