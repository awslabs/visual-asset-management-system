...
`concurrencyRestriction` bounds how many executions of the workflow run at once. `perAsset` and `perInputFile` check the workflow's running executions at launch and reject an overlap on the same asset or input file. `perInputFileVersion` locks each selected file version before the state machine starts, so a second launch on the same version -- a duplicate upload notification, or two operators submitting the same file -- is rejected with `400` until the first execution finishes, fails, or is aborted; the lock row carries an expiry the table's time-to-live enforces should a release never run. Whole-asset and folder selections lock with an empty version, as do files on unversioned buckets.
