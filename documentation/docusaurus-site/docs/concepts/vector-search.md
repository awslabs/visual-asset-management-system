...
| The file is unarchived                 | All of the file's items are marked live again and keep their latest marks, so the file is searchable as soon as the flip lands. The restore is an Amazon S3 copy of the newest content version under a new version id; it is not treated as a new version, is not re-embedded, and does not re-run the pipeline — the items still name the version they were embedded from. |
...
| The asset is unarchived with its files | Each file's own unarchive marks that file's items live, as above.                                                                                                                                                                                                                                                                                                           |
...
-   **Web** — the **Primary Search** tab of the Assets and Files page offers a **Keyword / Natural language** switch in its query band when both Amazon OpenSearch and vector search are enabled, and natural-language mode alone, with no switch, when only vector search is. A line under the search box states what the active mode matches; natural-language mode searches nothing until a description is entered, and the table says so instead of reporting no matches. See [Search and Discovery](../user-guide/search-and-discovery.md).
...
The reindexer is not incremental: `enqueue` launches the full workflow for every file it enumerates, whether or not the file already has a vector, so each of those files is analyzed by the model again (its `genai_*` metadata is rewritten and its whole-file vector replaced) and the run costs as many analyses as there are files in scope — `databaseId` and `limit` are the only ways to narrow it. A run launched this way is configured like an upload-triggered run (the file-upload trigger's template and its default tag values), so a video whose windows were produced by a manual execution with `VIDEO_SEGMENT_SECONDS` set loses those window vectors until it is re-executed with the tag.

Turning `app.vectorSearch.enabled` off and back on needs this catch-up: the vectors written before the switch survive it and are searchable again as soon as the feature is enabled, but a file whose pipeline run completed while the feature was off recorded its embedding as skipped, has no vector, and is not found by natural-language search until the reindexer (or a re-execution of the workflow on the file) embeds it. Nothing does that on its own.
