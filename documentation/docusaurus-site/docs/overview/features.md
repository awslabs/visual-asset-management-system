...
| SYSTEM - GenAI Metadata Generation      | `useSystemGenAiMetadata`                          | Amazon Bedrock analysis of every viewer-supported file: `sys_*` attributes, typed `ext_*` and `genai_*` metadata, and embeddings for vector search | Enabled  |
