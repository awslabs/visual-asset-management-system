...
-   **The two API key read tools return key inventory, not key material.** `get_api_key` and `get_user_api_key` report that a key exists and who owns it; no key value or hash is returned, and the self-service tool reports another user's key as not found. They are deliberately absent from the auto-approve sample in `tools/VamsMCP/README.md`. There is no API key listing tool.
