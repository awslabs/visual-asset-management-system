...
| [CAD/Mesh Metadata Extraction](cad-mesh-extraction.md) | `useConversionCadMeshMetadataExtraction`               | Extract metadata from CAD and mesh files                                                                                                                               | STEP, STP, DXF, STL, OBJ, PLY, GLTF, GLB, XYZ                                                           | Lambda                             | No           |
...
| [GenAI Metadata Labeling](genai-labeling.md)           | `useGenAiMetadata3dLabeling`                           | AI-powered metadata labeling for 3D files                                                                                                                              | GLB, FBX, OBJ, STL, PLY, USD, DAE, ABC                                                                  | AWS Batch (Fargate)                | Yes          |
| [Video SOP/BOM Extraction](video-sop-bom.md)           | `useGenAiVideoSopBom`                                  | Transcribe 1-4 narrated teardown videos and derive a step-by-step SOP, an LCA bill of materials, a lab summary, and verification key frames                            | MP4, MOV, M4V, WEBM, MKV                                                                                | AWS Batch (Fargate)                | Yes          |
...
| Video SOP/BOM Extraction                | Yes          | AWS Batch (Fargate)                | Amazon Bedrock Runtime, Amazon Transcribe³         |
...
¹ For GenAI Metadata Labeling, the Amazon Bedrock Runtime and Amazon Rekognition endpoints are created only when the pipeline is enabled **and** all Lambda functions run in the VPC (`useGlobalVpc.useForAllLambdas`), because its Amazon Bedrock and Amazon Rekognition calls are made from a Lambda function. The Amazon Bedrock Runtime endpoint is also created, independently of `useForAllLambdas`, whenever Video SOP/BOM Extraction is enabled (³); the two conditions share one endpoint.
...
³ Amazon Bedrock Runtime and Amazon Transcribe endpoints are created whenever Video SOP/BOM Extraction is enabled: its container calls both services from the isolated subnets, which have no other route to them. No Amazon Rekognition endpoint is created for this pipeline.

:::info[Endpoint placement and ECS consolidation]
Pipeline interface endpoints are placed in the isolated subnets, except the Amazon ECS endpoint, which is placed in private subnets for the container pipelines that need egress (Gaussian Splatting, NVIDIA Cosmos, NVIDIA Cosmos 3, NVIDIA Gr00t, RapidPipeline, ModelOps) and in isolated subnets for Isaac Lab Training. The AWS Fargate pipelines in isolated subnets (Coordinate Transform, Potree Point Cloud Viewer, 3D Preview Thumbnail, GenAI Metadata Labeling, Video SOP/BOM Extraction) do not use the Amazon ECS endpoint, which serves the ECS agent on EC2-launch-type container instances. Only one Amazon ECS interface endpoint can exist per VPC when private DNS is enabled, so VAMS consolidates ECS endpoint subnets across pipeline types — private subnets take priority over isolated subnets when both are needed. Amazon ECS Agent and Amazon ECS Telemetry are distinct services from Amazon ECS and do not conflict with that single ECS endpoint.
:::
