...
The analysis prompt carries untrusted content — the text extracted from an uploaded file, user-entered metadata, and asset descriptions. Amazon Bedrock Guardrails screens that content before the model reads it, and the deployment creates the guardrail by default (`app.pipelines.useSystemGenAiMetadata.bedrockGuardrail.create.enabled`, `true` in the commercial template). The pipeline wraps the untrusted blocks of every Converse call — the analysis call and each video-window call — in guardrail content tags so the filters evaluate them. The same content reaches the embeddings model, which accepts no guardrail on its own invocation, so with vector search enabled the embedding step (`GenerateEmbeddingTask`, and the window child for its window text) first sends the composed text — and every content chunk, several per call — through the guardrail's `ApplyGuardrail` operation and embeds the text the guardrail returns: an entity the PII filter masks is a type token (`{NAME}`, `{EMAIL}`, …) in the vector's input and in the `sourceText` the vector table stores, never the value. The three functions receive `bedrock:ApplyGuardrail` on that guardrail only.
...
**How an intervention surfaces.** Amazon Bedrock returns `stopReason: guardrail_intervened` whenever the guardrail acted — when a filter **blocked** the prompt or the response, and also when the PII filter only **masked** text — so the pipeline reads the guardrail trace to tell the two apart. A `BLOCKED` action in any filter of the trace — a prompt-attack detection, or PII under `piiFilter: block` — is a caught failure: the pipeline writes `execution.status.json` with `BedrockGuardrailIntervened` and a cause that lists the filters that fired (policy, type, action, and confidence — never the matched text), and the execution is recorded **FAILED** while the file's attributes and promoted metadata still land (see [Failure behaviour](#failure-behaviour)). A video window the guardrail blocks fails that window only. A trace whose actions are all `ANONYMIZED` is not an intervention: the response is the complete answer with mask tokens, the analysis completes, and `guardrailMasked` is recorded (on `analysis-summary.json` for the file, on `segments/<segmentKey>.json` for a window). A `guardrail_intervened` response that carries no trace at all is treated as an intervention. The embedding step's `ApplyGuardrail` pass follows the same reading: a masked text is embedded and stored masked, and the embedding summary (`embedding/summary.json` under the execution's auxiliary prefix) records `guardrailMasked` with the masked types; a blocked whole-file text is the same `BedrockGuardrailIntervened` failure with no vector published, and a blocked content chunk is skipped while the other chunks are still embedded and the run is recorded **FAILED** naming that chunk. Every successful analysis, window, and embedding logs one `Guardrail assessment` line — the filters that acted, by side (prompt or response), policy, type, and action — so input-side masking is auditable from Amazon CloudWatch.

:::tip[Security baseline]
The created guardrail satisfies the common security control that every Amazon Bedrock model invocation fed with user-supplied content runs behind a prompt-attack filter, with PII masked in both the input and the output — the analysis and window Converse calls through their guardrail configuration, the embeddings calls through the `ApplyGuardrail` pass on the text they receive. Set `create.enabled` to `false` without a `guardrailIdentifier` only in a deployment whose security review accepts unscreened prompts and unmasked stored `sourceText`; configuration validation prints a warning for that combination. The AWS GovCloud (US) and AWS European Sovereign Cloud templates ship with `create.enabled` `false` because Amazon Bedrock Guardrails availability in those partitions is unverified; enable it there once verified in the account.
:::
...
| `VIDEO_SEGMENT_SECONDS`          | integer | `0`     | `0` turns per-window video analysis off; `2` or more divides the video into equal windows of at most that many seconds (the duration split into the smallest whole number of windows that fit) and publishes one embedding per window (see [Video windows](#video-windows)). About $0.005 per window with Claude Haiku 4.5, about $2 per hour of video at 10 seconds, at most 360 windows per file; roughly four to eight videos analyze concurrently within a typical Bedrock quota (see [Video windows](#video-windows)) |
...
Each execution incurs one Amazon Bedrock Converse call (text plus up to eight images) on the analysis model and, with vector search enabled, one embeddings call. Lambda time is dominated by rendering (up to 10 GB memory for the 3D images). With `VIDEO_SEGMENT_SECONDS` above 0, add one Converse call with three images per window (about $0.005 with Claude Haiku 4.5, at most 360 windows per file); content chunks add one embeddings call per chunk, which is negligible. With a guardrail configured, the embedding step adds `ApplyGuardrail` calls on the text it embeds: one for the whole-file text and one per ten content chunks (a call carries at most ten texts and 20,000 characters), each billed per policy on the guardrail's text units of 1,000 characters — about $0.85 per 1,000 text units for the created guardrail's prompt-attack and PII policies, so a 10-page document of some 30 chunks costs about 6 cents to screen and a 1,000-chunk document about $1.50; a video window adds one call of a single text unit. See [Costs](../overview/costs.md).
