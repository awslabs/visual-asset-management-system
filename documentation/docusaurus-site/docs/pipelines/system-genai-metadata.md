...
The analysis prompt carries untrusted content — the text extracted from an uploaded file, user-entered metadata, and asset descriptions. Amazon Bedrock Guardrails screens that content for prompt attacks before the model reads it. VAMS does not create a guardrail, because guardrails are account-scoped resources an operator owns; create one in the deployment Region with the **Prompt attacks** content filter enabled at input strength `LOW` or higher, publish a version, and reference it in `app.pipelines.useSystemGenAiMetadata.bedrockGuardrail` (`guardrailIdentifier` and `guardrailVersion`, both or neither). When configured, the pipeline wraps the untrusted blocks of every Converse call — the analysis call and each video-window call — in guardrail content tags so the prompt-attack filter evaluates them, and the pipeline roles receive `bedrock:ApplyGuardrail` on that guardrail only. A prompt the guardrail blocks is a caught failure: the execution is recorded **FAILED** with `BedrockGuardrailIntervened` while the file's attributes and promoted metadata still land (see [Failure behaviour](#failure-behaviour)).

:::tip[Security baseline]
Attaching a guardrail with the prompt-attack filter satisfies the common security control that every Amazon Bedrock model invocation fed with user-supplied content runs behind prompt attack filters. Leave the two keys empty only in a deployment whose security review accepts unscreened prompts.
:::
...
| `bedrockGuardrail`                    | `guardrailIdentifier` and `guardrailVersion` of an operator-created Amazon Bedrock guardrail, both set or both empty; see [Amazon Bedrock guardrail](#amazon-bedrock-guardrail).                                                                                                 |
