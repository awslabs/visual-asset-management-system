...
| `cadAgentStatus` is `partial`                                                                                                 | The STEP file is valid but `cadAgentUnresolved` lists requested elements the agent could not complete, verify or had to assume (often a reference it could not find online); a run completed on assumed figures the instruction implied reads the same as one with a geometric shortfall, so read that list. Re-run with a more specific instruction where it names a figure the agent lacked. |
