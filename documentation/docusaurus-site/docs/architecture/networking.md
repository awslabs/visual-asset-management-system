...
| Amazon Bedrock Runtime        | (`useGlobalVpc.useForAllLambdas` and either `pipelines.useSystemGenAiMetadata.enabled` or `vectorSearch.enabled`) or (`vectorSearch.enabled` and the search Lambda functions are in the VPC: provisioned OpenSearch, a private Serverless collection, or `useForAllLambdas`) | `bedrock-runtime` — analysis and embedding model invocations from in-VPC Lambda functions. Standard endpoint only; no FIPS variant is created.                                                      |

:::warning[BYO-VPC deployments must supply the Bedrock endpoint]
When a VPC is imported with `useGlobalVpc.addVpcEndpoints: false`, VAMS creates none of the endpoints above. A deployment whose in-VPC Lambda functions call Amazon Bedrock — the SYSTEM GenAI metadata pipeline under `useForAllLambdas`, or natural-language search with in-VPC search functions — needs an operator-managed `bedrock-runtime` interface endpoint, and its route to Amazon DynamoDB (the gateway endpoint or an equivalent) must reach the DynamoDB vector search hostnames that `SearchVectors` uses.
:::
...
VPC-requiring pipelines (AWS Batch Fargate and GPU pipelines) create their own interface endpoints — a shared set of **AWS Batch**, **Amazon ECR API**, and **Amazon ECR Docker** whenever any AWS Batch pipeline is enabled, plus additional per-pipeline endpoints (Amazon ECS, Amazon ECS Agent, Amazon ECS Telemetry, Amazon EFS) depending on which pipelines are enabled. The Amazon Bedrock Runtime endpoint is listed with the conditional endpoints above because it is shared by a pipeline and by search.
