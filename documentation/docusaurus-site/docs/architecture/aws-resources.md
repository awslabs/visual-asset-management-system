...
| WorkflowExecutionLocksStorageTable              | `lockKey`             | --                                     | --                                                                                                                                                                                                                                                                 | Per-input-file-version execution locks |

`WorkflowExecutionLocksStorageTable` holds one row per lock a running execution acquired under the `perInputFileVersion` concurrency restriction — `lockKey`, `workflowExecutionId`, `acquiredAt`, and `expiresAt`. It is the only VAMS table with a TTL attribute: `expiresAt` (epoch seconds) lets DynamoDB remove a lock whose release path never ran, so a failed execution cannot hold its file version indefinitely.
