...
| `complianceFunctions.ts`     | complianceSchemaService, complianceSchemaBindingService, complianceEvaluateService, complianceQuarantineService, complianceCascadeService, complianceCascadeExecutor (invoked asynchronously by the cascade service), complianceAuditService, complianceTrigger (SNS-invoked), complianceWorkflowCallback (EventBridge-invoked) | Compliance schemas, evaluation, quarantine, cascades, audit |
...
| **ComplianceWorkflowCompletionDLQ**   | Dead-letter queue for `workflow.execution.completed` events the completion rule could not deliver after its three retries and for invocations the compliance workflow callback Lambda failed after Lambda's two asynchronous retries (the function's on-failure destination); holds them for an operator to redrive |
...
| **Compliance Workflow Completion Rule** | Routes `workflow.execution.completed` events from the deployment's sources to the `complianceWorkflowCallback` Lambda, which records the verdict of a `pending_pipeline` evaluation; retries a failed delivery three times within one hour, then dead-letters the event to ComplianceWorkflowCompletionDLQ |
...
| `/aws/vendedlogs/Pipelines/Metadata3dLabelingBlenderRenderer<hash>` | Blender renderer (AWS Fargate)            | `useGenAiMetadata3dLabeling`       | DESTROY        | Yes         |
