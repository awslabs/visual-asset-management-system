...
VAMS deploys 55 Amazon DynamoDB tables for persistent data storage — 48 read by Lambda handlers and 7 migration source tables. All tables use on-demand (PAY_PER_REQUEST) billing, point-in-time recovery, and optional AWS KMS customer-managed key encryption.
...
| WorkflowExecutionLocksStorageTable              | `lockKey`             | --                                     | --                                                                                                                                                                                                                                                                 | Concurrency locks per asset, input file, or file version held by running workflow executions (TTL `expiresAt`) |

`WorkflowExecutionLocksStorageTable` holds one row per lock a running execution acquired under its workflow's concurrency restriction (`perAsset`, `perInputFile`, or `perInputFileVersion`) — `lockKey`, `workflowExecutionId`, `lockScope`, `acquiredAt`, and `expiresAt`. It is the only VAMS table with a TTL attribute: `expiresAt` (epoch seconds) lets DynamoDB remove a lock whose release path never ran, so a failed execution cannot hold its file version indefinitely.
...
| Table                        | Partition Key (PK)   | Sort Key (SK)    | Vector index                                                                                                    | Purpose                                                    |
| ---------------------------- | -------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| VectorEmbeddingsStorageTable | `databaseId:assetId` | `fileVersionKey` | `vec-<embedding model slug>-<dimensions>` on `embedding` (COSINE), present only when `app.vectorSearch.enabled` | Per-file-version embeddings behind natural-language search |

The table exists in every deployment, so its resource-name parameter and backend constant never depend on the feature switch. A file version's whole-file embedding is one item; a segment of that version (a video time window or a text chunk) is a further item under the same partition whose sort key appends `#<segmentKey>`, so every lifecycle rule that addresses a file by its sort-key prefix covers its segments too. The vector index, and the attribute definitions of its seven inline-filter attributes (`databaseId`, `isLatest`, `isArchived`, `fileClass`, `fileExt`, `embeddingModelId`, `segmentKind`, all strings), are added only when `app.vectorSearch.enabled` is true and removed again when it is turned off — the items stay. The filter set is fixed when the index is created, which is why `segmentKind` (`none` on a whole-file item) is declared from the start. The index uses an `INCLUDE` projection of the identity, filter, segment, and display attributes a search hit needs, so `SearchVectors` answers without a base-table read; the vector itself, a list of numbers on the item, is never returned. Because the index name is derived from the embedding model id and dimensions, changing the model creates a new index rather than altering the existing one. No streams, TTL, or global secondary indexes.
...
| **FileIndexerSnsTopic**               | Routes DynamoDB Stream events and bucket-sync S3 records to the file indexer and, when vector search is enabled, the vector indexer |
| **AssetIndexerSnsTopic**              | Routes DynamoDB Stream events to the asset indexer and, when vector search is enabled, the vector indexer                           |
...
| **VectorIndexerQueue**                | Buffers `vector.embedding.ready` events, S3 file lifecycle records, asset stream records, and the indexer's own continuation messages for the vector indexer, consumed by at most 10 concurrent invocations (conditional on `app.vectorSearch.enabled`) |
| **VectorIndexerDLQ**                  | Dead-letter queue for vector indexing records that fail three receives (14-day retention)                                                                                                                                                               |
| **VectorEmbeddingReadyRuleDLQ**       | Dead-letter queue for `vector.embedding.ready` events the Amazon EventBridge rule could not deliver to the vector indexer queue                                                                                                                         |
| **SystemWorkflowLaunchQueue**         | Buffers reindex launch messages, one file per message, consumed at `app.vectorSearch.indexingConcurrency` concurrent launches                                                                                                                           |
| **SystemWorkflowLaunchDLQ**           | Dead-letter queue for launch messages that fail three receives (14-day retention)                                                                                                                                                                       |
...
The vector indexer and the system-workflow launcher report partial batch failures on the same terms and dead-letter after three deliveries with 14-day retention. An asset- or file-wide rule the indexer cannot finish within one invocation (an archive of an asset with hundreds of thousands of vectors, for example) re-enqueues the rest of its work to the vector indexer queue as a `vector.indexer.continue` message and resumes it in a later invocation, so the queue also carries the indexer's own messages. The `vector.embedding.ready` rule delivers to the vector indexer queue with three retries and its own dead-letter queue, so an event Amazon EventBridge could not enqueue is retained rather than discarded. Nothing replays a dead-lettered embedding on its own: the file version it names has no vector until the message is redriven or the vector reindexer is run.
...
| **Vector Embedding Ready Rule**  | Routes `vector.embedding.ready` events from the deployment's sources to the vector indexer queue, with a dead-letter queue for undeliverable events (conditional on `app.vectorSearch.enabled`) |
...
| ----------------------------------------------------------- | ------------------------------------------- | -------------------------------------------------- | -------------- | ----------- |
| `/aws/vendedlogs/Pipelines/CoordTransform<hash>`            | Coordinate transform (AWS Fargate)          | `useConversionCoordinateTransform`                 | DESTROY        | Yes         |
| `/aws/vendedlogs/Pipelines/SystemGenAiMetadataRender<hash>` | 3D render for oversized files (AWS Fargate) | `useSystemGenAiMetadata` with `useFargateRenderer` | DESTROY        | Yes         |
...
| ------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/<name>-<baseStackName>/resourceNames/dynamoTables/*`        | 48     | DynamoDB table names resolved by Lambda functions at cold start                                                                                                                                                                      |
...
| `/<name>-<baseStackName>/resourceNames/lambdaFunctions/*`     | 2      | OpenSearch and vector reindexer function names, read by the data-migration tooling only; each present only when its feature is enabled                                                                                               |
