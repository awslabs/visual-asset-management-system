...
-   **External asset buckets now require an explicit default, and a `createNewBucket: false` configuration that was valid in v2.5 fails `cdk synth`.** One bucket across the deployment holds all pipeline template bodies and execution run I/O, and `app.assetBuckets.externalAssetBuckets[].isDefault` is what names it. When `app.assetBuckets.createNewBucket` is `false`, exactly one external entry must set `isDefault: true`; at most one may set it in any configuration. v2.5 only required that `externalAssetBuckets` be non-empty when no bucket was created, so an external-bucket-only configuration carries no `isDefault` key and now throws at configuration validation. The error names the field, so recovery is a one-line edit. See the [external asset bucket object](configuration-reference.md#external-asset-bucket-object).
...
:::info[What this release changes for an existing deployment]
v2.7 retires two built-in metadata pipelines in favour of one system pipeline, adds an optional
DynamoDB-backed natural-language search over files, and formalises system-owned pipelines and workflows
(`isSystem`). An upgrading `config.json` receives the new configuration sections **disabled**, so nothing
turns on by itself; enabling the system pipeline and vector search is the configuration edit in step 2
below. The features themselves are described on the vector search concept page (`concepts/vector-search`),
the system pipelines page (`pipelines/system-pipelines`), and the system GenAI metadata pipeline page
(`pipelines/system-genai-metadata`).
:::

**Breaking changes:**

-   **The GenAI 3D Metadata Labeling and CAD/Mesh Metadata Extraction pipelines are retired, and their configuration keys are rejected.** `app.pipelines.useGenAiMetadata3dLabeling` and `app.pipelines.useConversionCadMeshMetadataExtraction` are removed from the configuration schema; a `config.json` that still carries either block fails `cdk synth` with an error naming the replacement key (`app.pipelines.useSystemGenAiMetadata`) and this section. The deploy removes the two nested stacks and archives their `GLOBAL` pipeline and workflow rows (`genai-metadata-3d-labeling-obj-glb-fbx-ply-stl-usd`, `metadata-extraction-cad-mesh`). Their historical executions, the `autoGeneratedKeywords` asset metadata, and the `AB_*` / CAD attribute rows they wrote are left in place as deprecated data; the new pipeline writes its own `sys_*` attribute and `genai_*` metadata vocabulary. A workflow of your own that references a retired pipeline id fails at execute because the pipeline is archived — the `systemPipelineRetirement` migration step lists them; re-pointing a step to `GLOBAL:system-genai-metadata` is a manual edit.
-   **Archived workflows left orphaned `fileUpload` trigger rows, and the retired built-ins' triggers are the largest such set.** Before v2.7 nothing deleted a trigger when its workflow was archived, so the upload dispatcher kept matching the row and every matching upload paid one `executeWorkflow` invocation that ended `400 Workflow is archived and cannot be executed.` — no execution row and no alarm. v2.7 skips archived and disabled workflows in the dispatcher and deletes a workflow's trigger rows when it is archived; the rows orphaned before the upgrade are removed by the `orphanedTriggers` migration step.
-   **Built-in categories are renamed to `SYSTEM - Preview` (3D Preview Thumbnail, formerly `Preview`) and `SYSTEM - GenAI` (the new pipeline; the retired GenAI pipeline used `GenAI`).** `category` is a constraint field for the `pipeline` and `workflow` object types, so a role constraint whose criterion matches `Preview` or `GenAI` no longer matches the shipped built-ins — an **allow** rule scoped that way silently stops granting access to them. Re-author those criteria against the new values before upgrading; no migration step rewrites constraints. See [Object types and constraint fields](../concepts/permissions-model.md#object-types-and-constraint-fields).
-   **System pipelines and workflows are read-only through the API and web interface.** Records shipped from a `vamsSchema` bundle carry `isSystem: true`; an update, archive, restore, template create or delete, or trigger create or delete against one answers `400` with a message naming the rule. The permitted edits are the `enabled` toggle on the pipeline, the workflow, and its triggers, and a template's `configBody`, `tagSchema`, and `webFormJson`. A toggle is a pause switch: the next registration run (any redeploy that changes a bundle hash) re-asserts the shipped `enabled` values, exactly as the re-registration warning under [Step 4](#step-4-deploy-the-update) describes. While `app.vectorSearch.enabled` is `true`, the only durable way to stop the system GenAI trigger is `app.vectorSearch.enabled: false` together with `app.pipelines.useSystemGenAiMetadata.autoRegisterAutoTriggerOnFileUpload: false`.
-   **`execution.status.json` is a reserved results file name for every pipeline, including custom ones.** When the end-state processor finds an object of that name under an execution's results prefix with `{"status": "FAILED", "error": ..., "cause": ...}`, it records the execution and its end-state pipeline row as `FAILED` after writing back the outputs. A custom pipeline that already writes a results object under that name must rename it; see [Custom pipelines](../pipelines/custom-pipelines.md).
-   **The Lambda base layer moves to boto3/botocore 1.43.89.** The layer is rebuilt in Docker during `cdk deploy` (Docker Desktop must be running and signed in), together with three new Lambda container images for the system pipeline. Custom Lambda code that shares the layer receives the newer SDK.
-   **The Amazon Rekognition VPC interface endpoint is removed.** A Bedrock Runtime interface endpoint is created instead when `app.useGlobalVpc.useForAllLambdas` is `true` and either new feature is enabled, or when vector search is enabled and the search Lambdas are placed in the VPC. A deployment that supplies its own endpoints (`app.useGlobalVpc.addVpcEndpoints: false`) must add a `bedrock-runtime` interface endpoint and allow the DynamoDB service hostnames for the search Lambdas. See [Configuration reference](configuration-reference.md).
-   **Two new DynamoDB tables and two new auto-named Amazon SQS queue pairs.** `VectorEmbeddingsStorageTable` (created on every deployment; its vector index only when `app.vectorSearch.enabled`) and `WorkflowExecutionLocksStorageTable` are added with `RETAIN`; the vector indexer and system-workflow launch queues, each with a dead-letter queue, exist only when vector search is enabled. Back up the new tables like every other table before later updates.
-   **The system workflow uses `concurrencyRestriction: perInputFileVersion`.** Executing it twice for the same file version answers `400 A conflicting execution of this workflow is already running for this file version.`; a newer version of the same file runs concurrently. The value is available to your own workflows as well.

**Required migration steps:**

1.  Abort or drain in-flight executions of the two retired workflows **before** deploying. The deploy deletes the retired pipelines' inner state machines and log groups; an execution caught mid-flight stays `RUNNING` until its workflow task times out (18,000 s for GenAI 3D Metadata Labeling, 900 s for CAD/Mesh Metadata Extraction), then turns `FAILED`, and its full-mode sub-process logs are gone. List and abort them:

    ```bash
    vamscli execution list --workflow-database-id GLOBAL --workflow-id genai-metadata-3d-labeling-obj-glb-fbx-ply-stl-usd --status RUNNING
    vamscli execution list --workflow-database-id GLOBAL --workflow-id metadata-extraction-cad-mesh --status RUNNING
    vamscli execution abort <executionId>
    ```

    The `systemPipelineRetirement` step in step 7 repeats this check after the deploy for anything that slipped through.

2.  Edit `infra/config/config.json`: delete the `app.pipelines.useGenAiMetadata3dLabeling` and `app.pipelines.useConversionCadMeshMetadataExtraction` blocks, and add the `app.vectorSearch` and `app.pipelines.useSystemGenAiMetadata` sections. Both are backfilled **disabled** when absent (with a `cdk synth` warning in the commercial partition), so set `enabled: true` on both to turn the features on. Vector search requires the system pipeline with `autoRegisterWithVAMS` and `autoRegisterAutoTriggerOnFileUpload` both `true`. In AWS GovCloud (US), request Amazon Bedrock access to the analysis and embedding models first; in the AWS European Sovereign Cloud vector search is unavailable. See [Configuration reference](configuration-reference.md).

3.  Deploy the v2.7 CDK stack:
    ...
    The deploy rebuilds the Lambda base layer and builds the system pipeline's Lambda container images, creates the vector table and (when enabled) its index, registers the system pipeline and its trigger, and archives the retired built-ins.

4.  Navigate to the migration scripts directory and copy the configuration file:
...
5.  Set `resource_names_ssm_param_prefix` in the config to the value of the CloudFormation output `ResourceNamesSSMParamPrefixOutput` from your stack, and set `aws_region` (and `aws_profile` if needed):
...
7.  Run the migration. `orphanedTriggers` deletes rows, so the script echoes the resolved account, Region, and caller and asks you to type the account id (`--confirm-account <id>` for an automated run):
    ...
    **Windows**
    ...
    | Step                              | `--steps` value            | What it does                                                                                                                                                                                                                                                                    |
    | --------------------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | Orphaned trigger cleanup          | `orphanedTriggers`         | Enumerates every `fileUpload` trigger row through `TriggersByBaseTypeGSI`, joins it to the workflow table, and deletes rows whose workflow is archived or missing — including the retired built-ins' triggers. Idempotent; **deletes rows**.                                    |
    | Vector index backfill             | `vectorBackfill`           | Invokes the deployed `vectorReindexer` with `{"operation": "enqueue"}` (or `"both"` with `--clear-vectors`), which enumerates the latest live version of every file and enqueues one system-workflow execution per file. Skipped under `all` when vector search is not enabled. |
    | System-pipeline retirement report | `systemPipelineRetirement` | Lists retired-workflow executions still in flight and your workflows that reference a retired pipeline id, with guidance. Writes nothing.                                                                                                                                       |

:::note[Every step is safe to repeat]
`orphanedTriggers` deletes only rows whose workflow is archived or missing, so a second run finds nothing. `vectorBackfill` re-embeds files in place — each vector item is keyed on the file path and S3 version, so a repeat overwrites the same items — and the system workflow's `perInputFileVersion` lock drops a duplicate launch for a version that is still running. `systemPipelineRetirement` only reads.
:::
...
Reindex executions are stored with trigger type `System-Reindex`, so a chunk can be aborted with `vamscli execution abort <executionId> --group-id vec-<runId>-<chunk> --yes`. They stay in the executions list like any other run; removing them afterwards is an operator task (`vamscli execution permanent-delete`). Pass `--limit N` on a first run to enqueue only `N` files, and `--async` on very large deployments so the invoke returns immediately.

`--clear-vectors` (reindexer operation `both`) deletes **every** stored vector before re-embedding, so natural-language search returns nothing until the executions complete. It is off by default and needed only for a rebuild from scratch or a model change.
:::

:::note[Changing the embedding model later]
The vector index is created for one embedding model and dimension count (index name `vec-<model slug>-<dimensions>`), and an item whose model differs from the configured index is never written. Changing `embeddingDimensions` on a populated index is not supported. To change the model:
...
An execution of a retired workflow that was `RUNNING` during the deploy cannot finish: its inner state machine was deleted with the retired nested stack, so no task token arrives and the workflow records it `FAILED` when the task times out (5 hours for GenAI 3D Metadata Labeling, 15 minutes for CAD/Mesh Metadata Extraction). Abort them beforehand (step 1) or afterwards from the `systemPipelineRetirement` report; abort and permanent delete tolerate the missing sub-resources. Full-mode log retrieval for those executions lists the deleted log group under `warnings`; the truncated log stored on the execution row is what remains.
:::

For the IAM policy, the rollback table, and per-step details, see the [v2.6 to v2.7 migration README](https://github.com/awslabs/visual-asset-management-system/blob/main/infra/deploymentDataMigration/v2.6_to_v2.7/upgrade/v2.6_to_v2.7_migration_README.md).
...
| Retired metadata pipelines and their config keys                | v2.6 to v2.7                                      | Remove `app.pipelines.useGenAiMetadata3dLabeling` and `app.pipelines.useConversionCadMeshMetadataExtraction`; add `app.pipelines.useSystemGenAiMetadata` and `app.vectorSearch`; run the `orphanedTriggers` migration step; re-point the workflows the `systemPipelineRetirement` report lists.                                          |
| Built-in category rename (`SYSTEM - Preview`, `SYSTEM - GenAI`) | v2.6 to v2.7                                      | Re-author role constraints whose `category` criterion matches `Preview` or `GenAI`; no script rewrites constraints.                                                                                                                                                                                                                      |
| Reserved results file name `execution.status.json`              | v2.6 to v2.7 (custom pipelines)                   | Rename any results object a custom pipeline writes under that name, or adopt the status-file contract.                                                                                                                                                                                                                                   |
| Rekognition endpoint removed, Bedrock Runtime endpoint added    | v2.6 to v2.7 (`addVpcEndpoints: false`)           | Supply a `bedrock-runtime` interface endpoint and allow the DynamoDB service hostnames for the search Lambdas.                                                                                                                                                                                                                           |
| Vector index backfill                                           | v2.6 to v2.7 (`app.vectorSearch.enabled`)         | Run the `vectorBackfill` migration step after the deploy; files uploaded before the upgrade have no vector until then.                                                                                                                                                                                                                   |
| Lambda base layer boto3 bump                                    | v2.6 to v2.7                                      | None beyond a running Docker daemon at deploy; the layer is rebuilt.                                                                                                                                                                                                                                                                     |
