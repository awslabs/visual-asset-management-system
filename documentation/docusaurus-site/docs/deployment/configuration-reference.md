...
| `app.pipelines.useGenAiCadStepAgent.allowInternetResearch`                      | boolean | `true`                                             | Deployment master switch for the agent's web search and page fetch tools. When `false` the tools are not registered, whatever a run's `ALLOW_INTERNET_RESEARCH` tag says.                                                                                                                                                                                                                             |
| `app.pipelines.useGenAiCadStepAgent.agentCore.warmSessionSlots`                 | number  | `0`                                                | AgentCore runtime only. The number (0-20) of fixed runtime session ids reused across runs so a session stays warm between invocations; `0` starts a fresh session per run.                                                                                                                                                                                                                            |
...
| `app.pipelines.useGenAiCadStepAgent.maxRunSeconds`                              | number  | `3600`                                             | Wall-clock bound (300-7200 s) for one agent run, enforced by the container; a run that exceeds it is reported as failed. On the `agentcore` runtime it cannot exceed `agentCore.maxLifetimeSeconds`.                                                                                                                                                                                                  |
