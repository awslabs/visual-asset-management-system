...
| `app.useFips`                | boolean | `false` | Selects FIPS variants where the CDK creates a FIPS-specific resource: the AWS KMS interface endpoint (`KMSEndpoint_FIPS`, alongside the standard one) and the Amazon Cognito hosted-UI hostname. The flag can also be set as the `useFips` CDK context value or through the `AWS_USE_FIPS_ENDPOINT=true` environment variable at synthesis time. It is not passed to Lambda functions or containers, whose AWS SDK calls resolve the Regional endpoints. The GovCloud template sets it to `true`; configuration validation does not require it there. |
...
| Bedrock Runtime | `useGenAiMetadata3dLabeling.enabled=true` + `useForAllLambdas=true`, or `useGenAiVideoSopBom.enabled=true` | Isolated                        |
| Rekognition     | `useGenAiMetadata3dLabeling.enabled=true` + `useForAllLambdas=true`                                        | Isolated                        |
| Transcribe      | `useGenAiVideoSopBom.enabled=true`                                                                         | Isolated                        |
...
### Video SOP/BOM extraction (`app.pipelines.useGenAiVideoSopBom`)

Transcribes one to four narrated teardown videos (MP4, MOV, M4V, WebM, MKV) selected from one asset with Amazon Transcribe and derives a step-by-step SOP, an LCA bill of materials, and a lab summary with Amazon Bedrock, extracting key frames for visual verification. Runs on AWS Batch with AWS Fargate compute in the isolated subnets. **Requires VPC.** Uses a GPL-licensed library. See the [Video SOP/BOM Extraction pipeline](../pipelines/video-sop-bom.md) page for input rules, templates, limits, and outputs.

:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/pipelines/genAi/videoSopBom/videoSopBomBuilder-nestedStack.ts` (`VideoSopBomBuilderNestedStack`) — AWS Batch on Fargate with Amazon Transcribe and Amazon Bedrock inference.
:::

:::note[Deployment prerequisites]

-   **Partition.** The pipeline is accepted in the commercial (`aws`) and AWS GovCloud (US) (`aws-us-gov`) partitions. Amazon Transcribe and Amazon Bedrock are documented by AWS as available in both AWS GovCloud (US) Regions; the pipeline is not live-tested there, and the GovCloud template ships `bedrockModelId` empty for the operator to set. Enabling it in any other partition fails configuration validation with a message ending `not validated outside the commercial and GovCloud partitions: Amazon Transcribe endpoint availability is unverified and the service-helper has no row for this partition; a missing endpoint hangs a run until its timeout.`
-   **AWS Fargate On-Demand vCPU quota.** Each run reserves 4 vCPU and 16 GiB of memory on a 100 GiB ephemeral volume (`VIDEO_SOP_BOM_EPHEMERAL_STORAGE_GIB`). The default quota is 6 vCPU per account and Region, so two concurrent runs serialize (the second waits in `RUNNABLE`); raise the quota through Service Quotas for deployments that expect concurrent runs.
-   **Interface endpoints.** With `app.useGlobalVpc.addVpcEndpoints`, enabling this pipeline creates Amazon Transcribe and Amazon Bedrock Runtime interface endpoints in the isolated subnets (see the endpoint table under [VPC](#vpc-appuseglobalvpc)). An imported VPC must already provide them.
-   **Amazon Bedrock model access** for the model named in `bedrockModelId`, requested in the Amazon Bedrock console of the deployment Region.

:::

| Field                                                              | Type    | Default                                                                                                          | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------ | ------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `app.pipelines.useGenAiVideoSopBom.enabled`                        | boolean | `false`                                                                                                          | Enables the Video SOP/BOM extraction pipeline.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `app.pipelines.useGenAiVideoSopBom.useCodeBuild`                   | boolean | `false`                                                                                                          | Builds the container image with AWS CodeBuild and Amazon ECR during deployment instead of a local Docker build. The CodeBuild project runs outside the VPC to pull the public base image. CodeBuild builds run asynchronously and outlive the deployment: the stack reports success as soon as the build is STARTED, so a green deploy does not mean the image exists. The pipeline is registered regardless, so a failed build leaves a selectable pipeline whose jobs cannot start — check the CodeBuild project name in the CDK stack outputs before running it.                                                                                                |
| `app.pipelines.useGenAiVideoSopBom.autoRegisterWithVAMS`           | boolean | `true`                                                                                                           | Automatically registers the pipeline, its two templates, and the workflow in the VAMS database during deployment. This pipeline has no `autoRegisterAutoTriggerOnFileUpload` key: an upload trigger would start one single-video run per uploaded file.                                                                                                                                                                                                                                                                                                                                                                                                            |
| `app.pipelines.useGenAiVideoSopBom.bedrockModelId`                 | string  | `global.anthropic.claude-sonnet-5` (commercial template); empty in the GovCloud and EU Sovereign Cloud templates | Amazon Bedrock model ID used for both the text and the vision calls (Converse API with forced tool use). A cross-Region inference-profile prefix is partition-specific: `global.` and `us.` exist in the commercial partition, and AWS GovCloud (US) uses `us-gov.`. Configuration validation rejects an empty value when the pipeline is enabled and rejects a commercial prefix outside the commercial partition. The GovCloud template leaves the value empty for the operator to set from a `us-gov.` inference profile; the EU Sovereign Cloud template also ships it empty, but the pipeline cannot be enabled there (see the partition prerequisite above). |
| `app.pipelines.useGenAiVideoSopBom.limits.maxVideoFiles`           | integer | `4`                                                                                                              | Maximum number of video files per run, 1 to 4. A run selecting more is refused before any job starts, naming the count and the cap.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `app.pipelines.useGenAiVideoSopBom.limits.maxTotalDurationMinutes` | integer | `240`                                                                                                            | Maximum summed audio duration per run, 1 to 480 (Amazon Transcribe accepts at most 28,800 seconds of audio per job). Enforced inside the container from the extracted audio tracks.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

The per-video (4 GB) and per-run (16 GB) byte caps are `config.ts` constants (`VIDEO_SOP_BOM_MAX_VIDEO_FILE_SIZE_MB`, `VIDEO_SOP_BOM_MAX_TOTAL_INPUT_SIZE_MB`) sized to the container's 100 GiB ephemeral volume rather than configuration options; raising them requires raising the volume, which a test pins.
