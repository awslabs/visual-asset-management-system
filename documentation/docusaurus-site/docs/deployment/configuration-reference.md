...
| GenAI CAD STEP agent, Fargate runtime (`useGenAiCadStepAgent`, `runtime: "fargate"`)     | Yes                        | Yes            | 2                                | Batch compute, NAT egress for research                             |
| GenAI CAD STEP agent, AgentCore runtime (`useGenAiCadStepAgent`, `runtime: "agentcore"`) | Yes                        | No             | 0                                | Pipeline Lambdas only; the agent runs on AgentCore outside the VPC |
...
### GenAI CAD STEP agent (`app.pipelines.useGenAiCadStepAgent`)

Creates or modifies a CAD STEP file (`.stp`, `.step`) from a natural-language instruction. A Strands agent plans the change, optionally researches the design on the internet, writes CadQuery scripts, runs them in a sandbox with a bounded self-repair loop, and writes the resulting STEP file plus an outcome report back to the asset. The model is Amazon Bedrock by default and switchable per run to an OpenAI model when the deployment configures an API-key secret. **Requires VPC.** See the [GenAI CAD STEP agent pipeline](../pipelines/genai-cad-step-agent.md) page for the templates, tags and outputs.

:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/pipelines/genAi/cadStepAgent/cadStepAgentBuilder-nestedStack.ts` (`CadStepAgentBuilderNestedStack`) — Amazon Bedrock AgentCore Runtime (`runtime: "agentcore"`) or AWS Batch on Fargate (`runtime: "fargate"`), selected per deployment.
:::

| Field                                                                           | Type    | Default                                            | Description                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------------------------- | ------- | -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app.pipelines.useGenAiCadStepAgent.enabled`                                    | boolean | `false`                                            | Enables the GenAI CAD STEP agent pipeline.                                                                                                                                                                                                                                                                                                                                                               |
| `app.pipelines.useGenAiCadStepAgent.runtime`                                    | string  | `"agentcore"`                                      | Where the agent container runs. `agentcore` hosts it on Amazon Bedrock AgentCore Runtime (commercial partition only; requires `useCodeBuild`; the runtime reaches the internet directly and no container runs in the VPC). `fargate` runs it as an AWS Batch job on Fargate in the private pipeline subnets, which adds public subnets and one NAT gateway per Availability Zone for research egress.    |
| `app.pipelines.useGenAiCadStepAgent.useCodeBuild`                               | boolean | `true`                                             | Builds the container image with AWS CodeBuild and Amazon ECR during deployment (`linux/arm64` for the AgentCore runtime, `linux/amd64` for Fargate) instead of a local Docker build. Required for the `agentcore` runtime. CodeBuild builds run asynchronously and outlive the deployment; check the CodeBuild project name in the CDK stack outputs before running the pipeline.                        |
| `app.pipelines.useGenAiCadStepAgent.autoRegisterWithVAMS`                       | boolean | `true`                                             | Registers the pipeline, its two templates and its two workflows (modify and generate) into VAMS during deployment.                                                                                                                                                                                                                                                                                       |
| `app.pipelines.useGenAiCadStepAgent.autoRegisterAutoTriggerOnFileUpload`        | boolean | `false`                                            | Arms the modify workflow's file-upload trigger for `*.stp` and `*.step` files.                                                                                                                                                                                                                                                                                                                           |
| `app.pipelines.useGenAiCadStepAgent.bedrockModelId`                             | string  | `global.anthropic.claude-sonnet-4-5-20250929-v1:0` | The Amazon Bedrock model the agent uses by default. Cross-Region inference-profile prefixes (`global.`, `us.`) exist only in the commercial partition; the restricted-partition templates ship it empty for the operator to set. Overridable per run through the `MODEL_ID` template tag.                                                                                                                |
| `app.pipelines.useGenAiCadStepAgent.openAi.modelId`                             | string  | `""`                                               | The OpenAI model used when a run selects the `openai` provider. Set together with `openAi.apiKeySecretArn`; both empty disables the provider.                                                                                                                                                                                                                                                            |
| `app.pipelines.useGenAiCadStepAgent.openAi.apiKeySecretArn`                     | string  | `""`                                               | ARN of the AWS Secrets Manager secret holding the OpenAI API key (a plain string, or JSON with an `apiKey` key). The container reads it at run start; the value never appears in `config.json`, environment variables or logs.                                                                                                                                                                           |
| `app.pipelines.useGenAiCadStepAgent.allowInternetResearch`                      | boolean | `true`                                             | Deployment master switch for the agent's web search and page fetch tools. When `false` the tools are not registered, whatever a run's `ALLOW_INTERNET_RESEARCH` tag says. Gates the tools only: the container keeps the network access its own AWS calls need.                                                                                                                                           |
| `app.pipelines.useGenAiCadStepAgent.bedrockGuardrail.guardrailId`               | string  | `""`                                               | Id of an existing Amazon Bedrock guardrail applied to every run: the caller's instruction and every fetched page are screened through it, and on the Bedrock provider it rides on every model invocation. Empty together with `guardrailVersion`: the deployment creates a guardrail with a PROMPT_ATTACK input filter (strength LOW). Set together with `guardrailVersion`; a guardrail id, not an ARN. |
| `app.pipelines.useGenAiCadStepAgent.bedrockGuardrail.guardrailVersion`          | string  | `""`                                               | Numbered version (or `DRAFT`) of the guardrail named by `guardrailId`. The two are set together or left empty together.                                                                                                                                                                                                                                                                                  |
| `app.pipelines.useGenAiCadStepAgent.agentCore.warmSessionSlots`                 | number  | `0`                                                | AgentCore runtime only. The number (0-20) of fixed runtime session ids reused across runs so a session stays warm between invocations; `0` starts a fresh session per run. One session hosts one run at a time; a run that lands on a busy slot is refused and retried by the workflow.                                                                                                                  |
| `app.pipelines.useGenAiCadStepAgent.agentCore.idleRuntimeSessionTimeoutSeconds` | number  | `900`                                              | AgentCore runtime only. How long (60-28800 s) an idle session stays alive before the runtime ends it; cannot exceed `maxLifetimeSeconds`.                                                                                                                                                                                                                                                                |
| `app.pipelines.useGenAiCadStepAgent.agentCore.maxLifetimeSeconds`               | number  | `28800`                                            | AgentCore runtime only. The longest (60-28800 s) a runtime session lives.                                                                                                                                                                                                                                                                                                                                |
| `app.pipelines.useGenAiCadStepAgent.maxRunSeconds`                              | number  | `3600`                                             | Wall-clock bound (300-6000 s) for one agent run, enforced by the container; a run that exceeds it is reported as failed. The bound sits below the 6600 s the pipeline waits on the run and the 7200 s the workflow task waits on the pipeline. On the `agentcore` runtime it cannot exceed `agentCore.maxLifetimeSeconds`.                                                                               |
