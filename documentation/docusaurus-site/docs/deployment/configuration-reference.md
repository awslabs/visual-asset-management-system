...
The `env.partition` field is automatically derived from the Region and should not be set manually. VAMS supports the `aws`, `aws-us-gov`, `aws-eusc` (AWS European Sovereign Cloud), `aws-cn`, and `aws-iso*` partitions. The `aws-us-gov`, `aws-eusc`, and ISO partitions additionally require `app.govCloud.enabled` to be `true` — see [GovCloud (`app.govCloud`)](#govcloud-appgovcloud) and [AWS European Sovereign Cloud deployment](#aws-european-sovereign-cloud-deployment).
:::
...
| `app.adminUserId`       | string | `administrator` | Username for the initial super administrator account. Can be an email address. Can be overridden with the `ADMIN_USER_ID` environment variable. **Immutable after the first deployment** — changing it replaces the Cognito user, which either fails the deployment or leaves the previous administrator behind as an unmanaged account. Synthesis rejects a username Amazon Cognito would refuse. See [Update the solution](./update-the-solution.md). |
| `app.adminEmailAddress` | string | _(required)_    | Email address for the initial admin account. A temporary password is sent to this address during first deployment. Can be overridden with the `ADMIN_EMAIL_ADDRESS` environment variable. **Immutable after the first deployment** for the same reason as `adminUserId`.                                                                                                                                                                                |
...
| `rateBasedRules`    | Rate-based rules for L7 DDoS and brute-force throttling. Each entry sets `name`, `priority`, and `limit` (requests per 5-minute window per source IP address), and may set `blockResponseCode` (default `429`). Every rule is applied per connection IP address.                                                                                                                                                                                                                                                                                                                        |
...
The rate-based rule aggregates on the IP address AWS WAF observes on the connection, and the same policy applies to both the CloudFront-scoped and regional web ACLs. An `X-Forwarded-For` aggregation key is accepted in the policy file for compatibility but is not applied, because the value is supplied by the caller — a client can rotate it to evade the limit — and AWS WAF omits any request that does not carry the header from the rule's evaluation. Where VAMS is fronted by Amazon CloudFront or an Application Load Balancer, or reached through a shared NAT gateway or VPN egress address, requests from behind that address share a counter, so set the limit with that in mind. VAMS is chatty per active user (the executions board polls for live status, uploads issue multi-part requests, and viewers stream large files), so the limit is set well above a single user's normal request rate while still stopping request floods. When the rule blocks, it returns HTTP `429 Too Many Requests` with a small JSON body — the correct throttle status, distinct from the `403` returned for an authorization denial — so clients can recognize throttling and retry. The VAMS web application and the VAMS CLI both treat a `429` as a transient, retryable condition: they honor the `Retry-After` header and retry with backoff rather than surfacing it as an authentication or permission failure.
...
#### Restricted-partition constraints

Configuration validation enforces the following when `app.govCloud.enabled` is `true`, or when the deployment Region resolves to a partition other than `aws`. This is the complete set; a configuration that violates any of them is rejected before synthesis with an error naming the field.

| Constraint                                                       | Setting required                                                                    | Reason                                                                                                                                                                                                                                                |
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| VPC required                                                     | `app.useGlobalVpc.enabled: true`                                                    | The partitions place VAMS resources inside a VPC.                                                                                                                                                                                                     |
| No Amazon CloudFront                                             | `app.useCloudFront.enabled: false`                                                  | Amazon CloudFront is not offered. Use the ALB web deployment (`app.useAlb`).                                                                                                                                                                          |
| No Amazon Location Service                                       | `app.useLocationService.enabled: false`                                             | Amazon Location Service is not offered, so map features are unavailable.                                                                                                                                                                              |
| No AWS Deadline Cloud                                            | `app.pipelines.deadlineCloudExecutionTypeEnabled: false`                            | AWS Deadline Cloud is offered only in the commercial `aws` partition.                                                                                                                                                                                 |
| No Amazon Cognito SAML or OIDC federation                        | `app.authProvider.useCognito.useSaml: false` and `useOidc: false`                   | Both use the Amazon Cognito hosted UI, which is unavailable. Use `app.authProvider.useExternalOAuthIdp` for federated sign-in.                                                                                                                        |
| No next-generation OpenSearch Serverless                         | `app.openSearch.useServerless.nextGen: false`                                       | Next-generation Serverless collection groups are unavailable.                                                                                                                                                                                         |
| No OpenSearch Serverless in the European Sovereign Cloud         | `app.openSearch.useServerless.enabled: false` (use `app.openSearch.useProvisioned`) | Amazon OpenSearch Serverless has no endpoint in the `aws-eusc` partition. Applies to `aws-eusc` only; AWS GovCloud supports Serverless.                                                                                                               |
| No Amazon DynamoDB vector search in the European Sovereign Cloud | `app.vectorSearch.enabled: false`                                                   | Amazon DynamoDB vector search is not offered in the `aws-eusc` partition. Applies to `aws-eusc` only; AWS GovCloud (US) supports it, with Amazon Bedrock model access enabled by hand in both the GovCloud account and its linked commercial account. |

Amazon Cognito advanced security (`AdvancedSecurityMode`) is also unavailable in these partitions. VAMS does not configure it and suppresses the corresponding CDK Nag check automatically, so it needs no configuration change.
...
| Field                                                | Type    | Default       | Description                                                                                                                                                                                                                                                                       |
| ---------------------------------------------------- | ------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app.useGlobalVpc.enabled`                           | boolean | `false`       | Creates or imports a VPC for VAMS resources. Must be `true` when ALB, OpenSearch Provisioned, a private OpenSearch Serverless collection, or any container-based pipeline is enabled; configuration validation otherwise rejects the deployment and lists the offending features. |
...
Each subnet must reside in its own Availability Zone. VAMS spreads every subnet type across a baseline of 2 Availability Zones, which also covers ALB, the EKS pipelines, and a private OpenSearch Serverless collection. Enabling OpenSearch Provisioned sets the count from `app.openSearch.useProvisioned.availabilityZoneCount` instead — `2` by default, optionally `3`.
:::
...
| Bedrock Runtime | `useForAllLambdas=true` + (`useSystemGenAiMetadata.enabled=true` or `vectorSearch.enabled=true`); or `vectorSearch.enabled=true` while the search Lambda functions run in the VPC (`openSearch.useProvisioned.enabled=true`, a private Serverless collection, or `useForAllLambdas=true`) | Isolated                        |
...
:::note[Bring-your-own VPC and vector search]
When `app.useGlobalVpc.addVpcEndpoints` is `false`, the imported VPC provides what the tables above would have created: a `com.amazonaws.<region>.bedrock-runtime` interface endpoint whenever the Bedrock Runtime row applies, and the Amazon DynamoDB gateway endpoint. Vector search calls Amazon DynamoDB on the search hostnames `search-dynamodb.<region>.<dns-suffix>` and `<account-id>.search-ddb.<region>.<dns-suffix>` (and their `-fips` variants). The gateway endpoint's prefix list covers those hostnames, so no additional endpoint is needed. An egress proxy or an interface-type DynamoDB endpoint with private DNS covers `dynamodb.<region>.<dns-suffix>` only, so allow the search hostnames as well.
:::
...
A provisioned domain in a VPC requires the `AWSServiceRoleForAmazonOpenSearchService` service-linked role to exist in the account. AWS normally creates it automatically, but in some accounts it is missing, which fails the deploy with _"Before you can proceed, you must enable a service-linked role"_. VAMS creates this role idempotently during deployment — created if missing, left unchanged if it already exists — so the deployment handles that case without operator action. The role is account-wide and is not removed on stack teardown. See [Common deployment errors](deploy-the-solution.md#common-deployment-errors) for additional troubleshooting.
:::
...
-   **VPC requirement** -- A VPC spanning `availabilityZoneCount` Availability Zones (`2` by default, optionally `3`) must already exist or be created by the same deploy. When importing an existing VPC, it must already provide that many private subnets in separate Availability Zones.
-   **Fragile AWS CloudFormation updates** -- Domain configuration changes (instance type, EBS size, engine version) trigger blue/green updates that can take 30+ minutes and occasionally exceed the AWS CloudFormation custom-resource timeout. A major engine-version upgrade can fail in place and require redeploying with OpenSearch disabled, then re-enabling, before it succeeds.
-   **Service-linked role** -- A provisioned domain in a VPC requires the OpenSearch Service service-linked role. VAMS creates it idempotently during deployment (created if missing, left unchanged if present), so the _"you must enable a service-linked role"_ error needs no manual retry.
...
## Vector search (`app.vectorSearch`)

:::note[Implemented by]
Storage nested stack: `infra/lib/nestedStacks/storage/storageBuilder-nestedStack.ts` — the `VectorEmbeddingsStorageTable` and its vector index. Search nested stack: `infra/lib/nestedStacks/searchAndIndexing/searchBuilder-nestedStack.ts` (`SearchBuilderNestedStack`) — the vector indexer, the reindexer, the system-workflow launcher, and the `POST /search/nlp` Lambda function, all created only while this option is enabled.
:::

Natural-language (semantic) search over files. The SYSTEM GenAI metadata pipeline (`app.pipelines.useSystemGenAiMetadata`) embeds every file version it processes with an Amazon Bedrock embedding model and publishes the vector on the orchestration bus; one indexer in the search stack writes it to an Amazon DynamoDB vector index named `vec-<model-id-slug>-<dimensions>` (for example `vec-amazon-titan-embed-text-v2-0-1024`). Searching works with or without Amazon OpenSearch Service; when OpenSearch is enabled, results are additionally constrained and enriched by its metadata filters. The `VECTORSEARCH` feature switch is published to the web interface and the CLI while this option is enabled.

| Field                                  | Type    | Default                                                                             | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------------------------------- | ------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app.vectorSearch.enabled`             | boolean | `true` (commercial); `false` (AWS GovCloud, AWS European Sovereign Cloud)           | Enables natural-language file search. Requires `app.pipelines.useSystemGenAiMetadata.enabled`, `autoRegisterWithVAMS`, and `autoRegisterAutoTriggerOnFileUpload` to be `true`; configuration validation rejects any other combination and does not change those flags itself. Not available in the AWS European Sovereign Cloud (partition `aws-eusc`), where configuration validation rejects it. Supported in AWS GovCloud (US), where enabling it prints a warning that Amazon Bedrock model access must be enabled by hand in both the GovCloud account and its linked commercial account. When the block is absent from `config.json`, the value follows `app.pipelines.useSystemGenAiMetadata.enabled` outside the restricted partitions and is `false` inside them; a commercial deployment that resolves to `false` this way prints a warning that explains how to enable it. Enabling it on an existing deployment, or re-enabling it after a period with it off, needs a reindex: the vectors written earlier are searchable again as soon as the feature is on, but every file the pipeline analyzed while it was off has no vector until the vector reindexer is invoked, and the reindexer re-analyzes every file it enumerates (see [Reindexing](../concepts/vector-search.md#reindexing)). |
| `app.vectorSearch.embeddingModelId`    | string  | `amazon.titan-embed-text-v2:0` (empty in the AWS European Sovereign Cloud template) | In-Region Amazon Bedrock embedding model id. Embedding models have no cross-Region inference profiles, so the id carries no `global.` or `us.` prefix. Must be non-empty while vector search is enabled. A different model is a differently named vector index: disable vector search, deploy, set the new id, enable it, deploy, and run the `vectorBackfill` migration step.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `app.vectorSearch.embeddingDimensions` | number  | `1024`                                                                              | Width of the vectors the model emits. Must be an integer from `1` to `4096` while vector search is enabled. The width is an immutable property of the vector index, so a different width is a new index and a full reindex.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `app.vectorSearch.indexingConcurrency` | number  | `5`                                                                                 | Maximum number of system-workflow launches that run concurrently while a reindex is in progress (the `MaximumConcurrency` of the launcher's Amazon SQS event source). Must be an integer from `2` to `1000` while vector search is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `app.vectorSearch.reindexOnCdkDeploy`  | boolean | `false`                                                                             | Rebuilds the vector table during the deployment through a CloudFormation custom resource: the vector reindexer clears every vector item and re-launches the SYSTEM GenAI metadata workflow for the latest live version of every file the workflow's input filters admit. The custom resource reports success as soon as the run is started (a run that outlives one Lambda invocation continues on its own, with the `ReindexRunId` in the resource's outputs), so the deployment does not wait for the re-analysis to finish. The run is not incremental and re-analyzes every file it enumerates, so enable it for one deployment after enabling vector search on an existing deployment, after the model-change procedure, or when the vector table must be rebuilt, then set it back to `false`. Requires `app.vectorSearch.enabled`; configuration validation rejects it otherwise. Can be overridden with CDK context `vectorReindexOnCdkDeploy=true`. The OpenSearch `app.openSearch.reindexOnCdkDeploy` flag is independent and rebuilds the OpenSearch indexes only (see [Reindexing](../concepts/vector-search.md#reindexing)).                                                                                                                                                                 |
...
:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/auth/authBuilder-nestedStack.ts` (`AuthBuilderNestedStack`) — Amazon Cognito user and identity pools, SAML federation, and external OAuth IdP wiring. IP-range restrictions (`authorizerOptions.allowedIpRanges`) are enforced by the custom Lambda authorizer built by `infra/lib/lambdaBuilder/authFunctions.ts` (`buildApiGatewayAuthorizerRestFunction`) and attached to the REST API in `apiLambda/constructs/rest-api-gateway-construct.ts`.
:::
...
| `app.webUi.allowLocalhostAuthCallbacks` | boolean | `false` | Registers `http://localhost:3001` as an Amazon Cognito callback and logout URL, so a web front run locally with `npm run start` can complete a federated sign-in against the deployed user pool. Read only when Cognito SAML or OIDC federation is enabled — without federation VAMS performs no OAuth redirect. The URLs are registered on the same user pool that serves application users, and the redirect target is on the developer's own machine, so leave this off for a deployment that serves real users and enable it only on a development deployment. |
...
:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/pipelines/conversion/coordinateTransform/coordinateTransformBuilder-nestedStack.ts` (`CoordinateTransformBuilderNestedStack`) — AWS Batch on Fargate.
:::
...
| `app.pipelines.useConversionCoordinateTransform.useCodeBuild`                        | boolean | `false` | Builds the container image with AWS CodeBuild and Amazon ECR during deployment instead of a local Docker build. The CodeBuild project runs outside the VPC to pull public base images. CodeBuild builds run asynchronously and outlive the deployment: the stack reports success as soon as the build is STARTED, so a green deploy does not mean the image exists. The pipeline is registered regardless, so a failed build leaves a selectable pipeline whose jobs cannot start — check the CodeBuild project name in the CDK stack outputs before running it. |
...
:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/pipelines/preview/pcPotreeViewer/pcPotreeViewerBuilder-nestedStack.ts` (`PcPotreeViewerBuilderNestedStack`) — AWS Batch on Fargate.
:::
...
:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/pipelines/preview/3dThumbnail/preview3dThumbnailBuilder-nestedStack.ts` (`Preview3dThumbnailBuilderNestedStack`) — AWS Batch on Fargate.
:::
...
Generates file attributes and Amazon Bedrock metadata for every file type VAMS has a viewer for and, while `app.vectorSearch.enabled` is `true`, the per-file-version embeddings that natural-language search indexes. Runs on AWS Lambda (container images for Blender, 3D rendering, and media extraction) and does not require a VPC. Registered as a SYSTEM pipeline and workflow (`system-genai-metadata`), which are read-only through the API and the web interface except for their `enabled` toggles.

:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/pipelines/system/genAiMetadata/systemGenAiMetadataBuilder-nestedStack.ts` (`SystemGenAiMetadataNestedStack`) — AWS Lambda container images per render branch (Blender, 3D, media) beside the zip functions; the optional `useFargateRenderer` branch runs the 3D renderer image on AWS Batch on Fargate for files above the Lambda limits.
:::

| Field                                                                                    | Type    | Default                                                                                                                                                                 | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app.pipelines.useSystemGenAiMetadata.enabled`                                           | boolean | `true` (commercial); `false` (AWS GovCloud, AWS European Sovereign Cloud)                                                                                               | Enables the pipeline. Required while `app.vectorSearch.enabled` is `true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `app.pipelines.useSystemGenAiMetadata.bedrockAnalysisModelId`                            | string  | `global.anthropic.claude-haiku-4-5-20251001-v1:0` (commercial); `us-gov.anthropic.claude-sonnet-4-5-20250929-v1:0` (AWS GovCloud); empty (AWS European Sovereign Cloud) | Vision-capable Amazon Bedrock model id or inference profile used for analysis. Must be non-empty while the pipeline is enabled. Cross-Region inference-profile prefixes are partition-specific: configuration validation rejects `global.` outside the commercial partition, `us-gov.` outside AWS GovCloud (US), and `us.` outside the commercial partition except in AWS GovCloud (US), where a `us.` id is accepted with a warning to verify that the inference profile exists in the GovCloud account; an id without a prefix is accepted in every partition. An Anthropic model prints a warning at synthesis: Anthropic requires a one-time use-case form per AWS organization before the first invocation, and until it is submitted executions end `FAILED` with `BedrockAccessDenied` while file attributes are still written.                                                      |
| `app.pipelines.useSystemGenAiMetadata.autoRegisterWithVAMS`                              | boolean | `true`                                                                                                                                                                  | Registers the pipeline, its workflow, and its template during deployment. Required while vector search is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `app.pipelines.useSystemGenAiMetadata.autoRegisterAutoTriggerOnFileUpload`               | boolean | `true`                                                                                                                                                                  | Arms the workflow's upload trigger. Required while vector search is enabled, because the trigger is what produces embeddings for new file versions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `app.pipelines.useSystemGenAiMetadata.useFargateRenderer`                                | boolean | `false`                                                                                                                                                                 | Adds an AWS Batch (Fargate) render branch for files above the Lambda limits. The only option of this pipeline that requires a VPC: `app.useGlobalVpc.enabled` must be `true` whenever this flag is `true`, whether or not the pipeline is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `app.pipelines.useSystemGenAiMetadata.lambdaLimits.maxInputFileSizeMb`                   | number  | `2048`                                                                                                                                                                  | Files above this size are analysed from their attributes only (no render) unless `useFargateRenderer` is enabled. Must be a positive integer while the pipeline is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `app.pipelines.useSystemGenAiMetadata.lambdaLimits.maxPointCloudPoints`                  | number  | `20000000`                                                                                                                                                              | Point clouds above this point count (read from the file header) are analysed from their attributes only unless `useFargateRenderer` is enabled. Must be a positive integer while the pipeline is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `app.pipelines.useSystemGenAiMetadata.bedrockGuardrail.create.enabled`                   | boolean | `true` (commercial); `false` (AWS GovCloud, AWS European Sovereign Cloud)                                                                                               | Creates an Amazon Bedrock guardrail as part of the deployment and sends every analysis prompt with it: a prompt-attack input filter, the PII filter below, encryption with the deployment's KMS key when one is enabled, and a published version the analysis functions reference. The guardrail has a fixed name (`VAMS-SystemGenAiMetadata-<deployment hash>`), so an orphaned copy collides with a redeploy of the same configuration. Off in the restricted-partition templates because Amazon Bedrock Guardrails availability there is unverified. Configuration validation rejects `true` together with a non-empty `guardrailIdentifier`; an absent value defaults to `true` unless a `guardrailIdentifier` is set. Turning it off without naming a guardrail prints a warning at synthesis. See the [pipeline page](../pipelines/system-genai-metadata.md#amazon-bedrock-guardrail). |
| `app.pipelines.useSystemGenAiMetadata.bedrockGuardrail.create.promptAttackInputStrength` | string  | `"LOW"`                                                                                                                                                                 | Strength of the created guardrail's prompt-attack filter on the prompt: `LOW`, `MEDIUM`, or `HIGH`; the response side is always `NONE`. Any other value is rejected. A change publishes a new guardrail version on the next deployment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `app.pipelines.useSystemGenAiMetadata.bedrockGuardrail.create.piiFilter`                 | string  | `"anonymize"`                                                                                                                                                           | How the created guardrail treats an email address, phone number, person name, postal address, US Social Security number, payment card number, AWS access key, AWS secret key, or password, in the prompt and in the model response alike: `anonymize` masks it with its type token (`{NAME}`, …) and the analysis succeeds with the masked text, `block` fails the analysis with `BedrockGuardrailIntervened`, `off` adds no PII filter. Any other value is rejected.                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `app.pipelines.useSystemGenAiMetadata.bedrockGuardrail.guardrailIdentifier`              | string  | `""`                                                                                                                                                                    | Identifier of an operator-owned Amazon Bedrock guardrail used instead of the created one. When set, every analysis prompt is sent with this guardrail and the untrusted prompt sections (file text, existing metadata, descriptions) are tagged for its input filters. Set together with `guardrailVersion` and `create.enabled` `false`, or leave both empty. Configuration validation rejects one without the other and rejects the pair together with `create.enabled` `true`.                                                                                                                                                                                                                                                                                                                                                                                                            |
| `app.pipelines.useSystemGenAiMetadata.bedrockGuardrail.guardrailVersion`                 | string  | `""`                                                                                                                                                                    | Version of the operator-owned guardrail (`1`, `2`, … or `DRAFT`). Set together with `guardrailIdentifier`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `app.pipelines.useSplatToolbox.useCodeBuild`         | boolean | `false` | Build the container image via AWS CodeBuild + Amazon ECR instead of a local Docker build. Recommended for this pipeline — the image is a large CUDA/PyTorch build. CodeBuild runs in the same private VPC subnets as the pipeline Batch compute environments, with NAT Gateway egress. When `false`, the image is built locally with a CDK `DockerImageAsset` (requires local Docker). CodeBuild builds run asynchronously and outlive the deployment: the stack reports success as soon as the build is STARTED, so a green deploy does not mean the image exists. The pipeline is registered regardless, so a failed build leaves a selectable pipeline whose jobs cannot start — check the CodeBuild project name in the CDK stack outputs before running it. |
...
:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/pipelines/multi/rapidPipeline/rapidPipeline-nestedStack.ts` (`RapidPipelineNestedStack`) — Amazon ECS on Fargate.
:::
...
| `app.pipelines.useRapidPipeline.useEks.eksClusterVersion`                     | string  | `1.36`                    | Kubernetes version for the Amazon EKS cluster, as an EKS minor version (`1.NN`).       |
...
| `app.pipelines.useIsaacLabTraining.useCodeBuild`         | boolean | `false` | When true, the Isaac Lab container is built using AWS CodeBuild in the cloud and pushed to ECR. When false (default), the container is built locally during CDK deployment using DockerImageAsset. CodeBuild runs in the same private VPC subnets as the pipeline Batch compute environments, with NAT Gateway egress for internet access, and forwards the `acceptNvidiaEula` flag as the `ACCEPT_EULA` Docker build argument. CodeBuild builds run asynchronously and outlive the deployment: the stack reports success as soon as the build is STARTED, so a green deploy does not mean the image exists. The pipeline is registered regardless, so a failed build leaves a selectable pipeline whose jobs cannot start — check the CodeBuild project name in the CDK stack outputs before running it. |
...
:::note[Implemented by]
Nested stack: `infra/lib/nestedStacks/pipelines/genAi/nvidia/cosmos/cosmosBuilder-nestedStack.ts` (`CosmosBuilderNestedStack`) — AWS Batch on GPU instances. This single stack implements all NVIDIA Cosmos models: Predict, Reason, Transfer, and Cosmos 3.
:::
...
| `app.pipelines.useNvidiaCosmos.useCodeBuild`                                                      | boolean | `false`                                          | When true, Cosmos pipeline containers are built using AWS CodeBuild in the cloud. When false (default), containers are built locally during CDK deployment using DockerImageAsset. CodeBuild runs in the same private VPC subnets as the pipeline Batch compute environments, with NAT Gateway egress for internet access. Consider configuring Docker Hub authentication credentials to avoid rate limiting (429 errors). CodeBuild builds run asynchronously and outlive the deployment: the stack reports success as soon as the build is STARTED, so a green deploy does not mean the image exists. The pipeline is registered regardless, so a failed build leaves a selectable pipeline whose jobs cannot start — check the CodeBuild project name in the CDK stack outputs before running it. |
...
NVIDIA Cosmos Reason Vision Language Models (VLMs) for analyzing video content to generate text-based analysis, captions, descriptions, and reasoning. Still images are not accepted: the upstream model utilities fail when writing an image input back out, so the pipeline restricts its inputs to `.mp4` and `.mov` rather than failing a run after the model has loaded. **Requires VPC** and internet access for HuggingFace model downloads. Shares the same EFS model cache and HuggingFace token as Cosmos Predict pipelines.

| Field                                                                                   | Type    | Default                            | Description                                                                                                                                                          |
| --------------------------------------------------------------------------------------- | ------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app.pipelines.useNvidiaCosmos.modelsReason.reason2B.enabled`                           | boolean | `false`                            | Enables Cosmos-Reason2-2B Vision Language Model for video analysis generating text-based output. Model size: ~5GB.                                                   |
...
| `app.pipelines.useNvidiaCosmos.modelsReason.reason2B.autoTriggerOnFileExtensionsUpload` | string  | `""`                               | Comma-separated file extensions to auto-trigger on upload (e.g., `".mp4,.mov"`). Leave empty to disable.                                                             |
...
:::note[Implemented by]
Construct: `infra/lib/nestedStacks/pipelines/genAi/nvidia/cosmos/constructs/cosmos3-construct.ts` (`Cosmos3Construct`), instantiated by the Cosmos nested stack `infra/lib/nestedStacks/pipelines/genAi/nvidia/cosmos/cosmosBuilder-nestedStack.ts` (`CosmosBuilderNestedStack`) — AWS Batch on GPU instances.
:::
...
| `app.pipelines.useNvidiaCosmos3.useCodeBuild`                                                     | boolean | `false`                                            | Build container image via AWS CodeBuild + ECR instead of local Docker. Recommended for large GPU images. When `false`, uses inline CDK DockerImageAsset (requires local Docker). CodeBuild builds run asynchronously and outlive the deployment: the stack reports success as soon as the build is STARTED, so a green deploy does not mean the image exists. The pipeline is registered regardless, so a failed build leaves a selectable pipeline whose jobs cannot start — check the CodeBuild project name in the CDK stack outputs before running it. |
| `app.pipelines.useNvidiaCosmos3.useWarmInstances`                                                 | boolean | `false`                                            | Keeps GPU instances running when idle for faster pipeline starts. Applies to the **Nano tier only**, and only when a Nano model is enabled — the Super tier's compute environment always scales to zero. Warm instances incur continuous GPU compute cost.                                                                                                                                                                                                                                                                                                 |
| `app.pipelines.useNvidiaCosmos3.warmInstanceCount`                                                | number  | `1`                                                | Number of warm Nano-tier GPU instances to keep running when `useWarmInstances` is `true`. Counted as 48 vCPUs each, which is one `g6e.12xlarge`; a larger configured instance type yields proportionally fewer instances.                                                                                                                                                                                                                                                                                                                                  |
...
| `app.pipelines.useNvidiaCosmos3.modelsOmni.nano16B.instanceTypes`                                 | array   | `["g6e.12xlarge", "g6e.24xlarge", "g6e.48xlarge"]` | EC2 GPU instance types for AWS Batch compute (BEST_FIT_PROGRESSIVE). Each must carry at least 4 GPUs with 48GB+ VRAM; a type with fewer is rejected at synthesis.                                                                                                                                                                                                                                                                                                                                                                                          |
...
:::note[The three Super variants share one AWS Batch compute environment]
Enabling more than one Super (64B) variant does not create a compute environment per variant. They share one environment and one job queue, and AWS Batch places a job on any instance type in that environment's pool, so the pool is the **intersection** of the enabled variants' `instanceTypes` — the types every enabled variant permits. With the defaults for `super64B` and `superText2Image64B` both enabled, the pool is `["p5.48xlarge", "p5e.48xlarge"]`: `p4de.24xlarge` is excluded because `superText2Image64B` requires H100/H200 and does not list it.

Narrowing one variant therefore narrows the shared pool. If the enabled variants' lists have no type in common, deployment is rejected at synthesis — give them at least one type in common, or enable one Super variant per deployment.

`maxVCpus` behaves differently and is not shared in the same way: the environment takes the **largest** value among the enabled variants.
:::
...
| `app.pipelines.useNvidiaGr00t.useCodeBuild`                                     | boolean | `false`                                          | Build container image via AWS CodeBuild + ECR instead of local Docker. Recommended for large GPU images. When `false`, uses inline CDK DockerImageAsset (requires local Docker). CodeBuild builds run asynchronously and outlive the deployment: the stack reports success as soon as the build is STARTED, so a green deploy does not mean the image exists. The pipeline is registered regardless, so a failed build leaves a selectable pipeline whose jobs cannot start — check the CodeBuild project name in the CDK stack outputs before running it. |
...
| `app.pipelines.useNvidiaGr00t.warmInstanceCount`                                | number  | `1`                                              | Number of warm GPU instances to keep running when `useWarmInstances` is `true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
-   The **queue** service role has read access to the execution input locations and write access to the execution output prefixes in the asset bucket.
-   The **fleet** role grants `logs:CreateLogStream` and `logs:PutLogEvents` on `arn:aws:logs:*:<account-id>:log-group:/aws/deadline/*`, in addition to the worker permissions it already holds.

:::warning[The AWS managed fleet-worker policy is not sufficient on its own]
`AWSDeadlineCloud-FleetWorker` grants the worker agent's Deadline Cloud API calls but no Amazon CloudWatch Logs access. A fleet role without the two log actions above produces a failure that reports nothing useful: the worker agent passes `CreateWorker` and `AssumeFleetRoleForWorker`, then fails the `CREATED` → `STARTED` transition with `AccessDeniedException ... does not have sufficient access to perform CreateLogStream`, because the service cannot create the worker's log stream.

The visible symptoms are indirect. The farm, fleet, and queue all report `ACTIVE`, `createJob` succeeds, and the VAMS execution simply never progresses until it times out. Behind that, the agent restarts every few seconds and the fleet replaces the instance every few minutes, so the account accrues charges and raises the fleet's worker-not-responding alarm — an alarm that describes worker health rather than the IAM permission causing it.

If Deadline Cloud pipeline executions hang and the fleet keeps replacing workers, check this permission before investigating capacity.
:::
...
-   **Inline `clientId` + `clientSecret` (default).** Put the credentials directly in the configuration. VAMS creates the Secrets Manager secret during deployment and populates it via a custom resource whose Lambda carries the values in its code asset. Because CDK references code assets by content hash (uploaded to the CDK assets bucket), the credential value does not appear in the synthesized CloudFormation template or its resource properties — and no secret has to be created ahead of deployment.

    What this path does **not** do is confine the credential to Secrets Manager. It is present in the populate Lambda's deployment package, so a principal holding `lambda:GetFunction` on that function — part of the AWS managed `ReadOnlyAccess` policy — can download the package and read it, without the `secretsmanager:GetSecretValue` and `kms:Decrypt` the secret itself requires. The same plaintext is uploaded to the CDK assets bucket, and because CDK assets are content-addressed and not garbage collected, a credential stays readable there after it is rotated in configuration. Choose `credentialsSecretArn` when the credential must be reachable only through Secrets Manager.

-   **`credentialsSecretArn` (operator-managed).** Create the secret yourself with a JSON value of `{ "clientId": "...", "clientSecret": "..." }` and reference it by ARN. VAMS imports the secret by ARN and ignores the inline `clientId`/`clientSecret`. No credential passes through synthesis on this path, so the populate custom resource is not created at all and the exposures described above do not arise. Use this when secret provisioning is centralized, when it must be managed outside the VAMS deployment, or when `lambda:GetFunction` is granted more widely than secret read.
...
The GovCloud template uses a provisioned Amazon OpenSearch Service domain rather than Serverless: next-generation Serverless collections are unavailable in the partition, and configuration validation rejects `openSearch.useServerless.nextGen` when `app.govCloud.enabled` is `true`.
