...
| `app.pipelines.useSystemGenAiMetadata.bedrockGuardrail.create.piiFilter`                 | string  | `"anonymize"`                                                                                                                                                           | How the created guardrail treats an email address, phone number, person name, postal address, US Social Security number, payment card number, AWS access key, AWS secret key, or password detected in the prompt or the model response: `anonymize` masks it with its type token (`{NAME}`, …) and the analysis succeeds with the masked text, `block` fails the analysis with `BedrockGuardrailIntervened`, `off` adds no PII filter. Any other value is rejected.                                                                                                                                                                                                                                                                                                                                                                                                                          |
