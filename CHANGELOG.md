...
-   **Search** Natural-language (vector) search — `POST /search/nlp` embeds the query with the configured Amazon Bedrock embeddings model (Amazon Titan Text Embeddings V2 by default), ranks per-file-version embeddings from the Amazon DynamoDB vector index across the caller's accessible databases, filters hits through both authorization tiers, and returns the latest live file versions in the existing search envelope with an `nlp` block (`truncated`, `candidatesEvaluated`, `itemsCollapsed`, `classIntent`) and a `_vector` block per hit. Segment vectors — a video's time windows and a document's content chunks — are searched beside the whole-file vectors (breadth and depth calls per target) and collapse to one hit per file, which reports `_vector.segmentHits` and the winning `bestSegment`; `includeSegments: false` searches whole-file vectors only. A file type named in the query (`video of …`, `pdf about …`) lists files of that type first and is echoed as `nlp.classIntent` — a boost, never a filter. Asset mode groups hits by asset; OpenSearch-only filters constrain and enrich results when an OpenSearch mode is on and are reported as `warnings` otherwise. Returns `503` while the vector index is building.
    ...
    -   **Web** The search page hosts search providers as tabs — Asset List (always) and Search (when OpenSearch or vector search is enabled) — with a Keyword / Natural language toggle, a reduced filter sidebar when OpenSearch is off, relevance shown as a percentage, and a "top N semantic matches" note when results are truncated.
