...
-   New Per-File-Version Workflow Lock - A `perInputFileVersion` workflow concurrency restriction backed by a DynamoDB lock table rejects a second execution over the same file version while one is running
...
-   **Workflows & Pipelines** `perInputFileVersion` concurrency restriction — a conditional-write lock per workflow, file, and S3 version in the new `WorkflowExecutionLocksStorageTable` (TTL safety net); a conflicting execute returns `400` with a generic message, a newer file version runs concurrently. Surfaced in the pipeline and workflow forms of the web interface and the CLI. Used by the system GenAI workflow.
