...
-   New Physical AI NVIDIA Cosmos 3 (omni) Inference Pipeline - GPU-accelerated generation on NVIDIA's omnimodal Cosmos 3 world foundation models: Cosmos3-Nano (16B) single-GPU and three Cosmos3-Super (64B) variants on multi-GPU FSDP-sharded inference, with metadata-driven prompts
...
-   **CDK** API Gateway migration to REST API (v1) — routes are registered through a cross-stack route registry and materialized into a single `SpecRestApi` with an inline OpenAPI spec by an API nested stack that selects an implementation construct, so alternative entry points can be added later.
    ...
    -   The `app.api` block is restructured around `apiType` and an `apiGatewayRest` sub-block holding the previously flat settings:
        ...
        -   `externalPrivateAPIGatewayVPCEId`: for a `"PRIVATE"` endpoint when `useGlobalVpc.addVpcEndpoints` is `false`, the id of an existing execute-api VPC endpoint to use.
...
-   **Security** Two-tier authorization now fails closed on missing identity — the asset-create, database-create, asset-ingest, download, and upload handlers deny when no authenticated identity is present instead of falling through to the operation.
...
-   Fixed cross-origin (CORS) failures introduced by the REST API migration — the REST integration returns each Lambda response verbatim, so the shared response helpers and the handlers that hand-build headers now set the allow-origin header themselves.
...
-   Fixed the login profile API returning a 500 for users without a stored profile record (for example, a user not yet assigned any roles).
...
-   Raised the GenAI Metadata Labeling pipeline's primary Lambda timeout from 5 to 15 minutes for larger 3D models, and fixed broken cross-reference links in the documentation.
...
-   **CDK** Added a second backend API nested stack (`ApiBuilder2NestedStack`) because the primary stack was approaching the CloudFormation per-stack resource limit; new API endpoints should be added there. Only self-contained domains are moved — the workflow functions stay in the primary stack because the workflow IAM role's path-derived name is baked into existing state machines.
...
-   **CDK** DynamoDB tables are now `RETAIN` on teardown instead of deleted to prevent accidental data loss; tables are uniquely named per deployment so retained tables do not collide on redeploy.
...
-   **Web** Plugin viewers with a custom install that dynamically installs npm packages now run `npm audit fix` before building, applying security patches at build time.
