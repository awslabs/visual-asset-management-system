...
-   **Backend** Workflow completion event: the workflow end-state, error handler and abort path put a `workflow.execution.completed` event on the orchestration bus (`Source` `<eventSourcePrefix>.execution.<executionId>`, detail `executionId`, `workflowDatabaseId`, `workflowId`, `status`, `startedAt`, `completedAt`, optional `executionGroupId`), consumed by the compliance workflow callback.
-   **CDK** Compliance deploys with every stack: five Amazon DynamoDB tables (`ComplianceSchema`, `ComplianceAssetState`, `ComplianceEvaluation` with an `ExecutionIdIndex`, `ComplianceCascade`, `ComplianceAudit`), eight Lambda functions, the `/compliance/*` routes in `apiBuilder2`, an EventBridge rule for the completion event, and admin-role constraints for the three compliance object types. New `app.compliance` settings: `autoLoadDefaultSchema` (default `true`; seeds the `GLOBAL` `default-compliance-schema`) and `quarantineBlocksDownload` (default `false`; the asset download and stream endpoints refuse a quarantined asset when `true`).
-   **Web** Compliance pages — Compliance Schemas (create, edit, sweep), Quarantine (release, exception), Cascade Approvals (approve, reject), Audit Log (event-type filter) — plus a per-database compliance overview, a Compliance Schema field in the database editor and a Compliance tab on the asset view with Evaluate Now and the evaluation history; all permission-filtered through web route constraints.
-   **CLI** `vamscli compliance` command group (`schema list|get|create|update|delete`, `bind`, `unbind`, `bindings`, `evaluate`, `sweep`, `state`, `evaluations`, `quarantine …`, `cascade …`, `audit`) and matching VAMS MCP tools (read tools plus write/destructive-gated `create_compliance_schema`, `update_compliance_schema`, `bind_compliance_schema`, `unbind_compliance_schema`, `evaluate_asset_compliance`, `sweep_compliance_schema`, `release_quarantine`, `grant_quarantine_exception`, `create_compliance_cascade`, `approve_compliance_cascade`, `reject_compliance_cascade`).
