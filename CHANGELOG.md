...
-   **CDK** When the Video SOP/BOM Extraction pipeline is enabled, the auxiliary bucket expires objects under its `pipelines/genai-video-sop-bom/` prefix after 30 days and the VPC builder creates Amazon Transcribe and Amazon Bedrock Runtime interface endpoints for container callers, and Bedrock model-id validation is shared between the GenAI metadata labeling and Video SOP/BOM pipelines.
