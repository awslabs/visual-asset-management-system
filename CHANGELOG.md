...
-   **Pipeline** GenAI CAD STEP Agent pipeline (`app.pipelines.useGenAiCadStepAgent`) - A Strands agent creates or modifies a CAD STEP file (.stp, .step) from a prompt: it optionally researches the design online, writes CadQuery scripts, runs them in a credential-scrubbed sandbox with a bounded self-repair loop, and writes the STEP file, a Markdown report and `cadAgent*` file metadata (status, summary, unresolved items, sources, geometry) back to the asset. Two templates over one pipeline (modify with one input file, generate with none) and a workflow for each; per-run tags for the prompt, output file name and prefix, internet research, model provider (Amazon Bedrock by default, OpenAI through a Secrets Manager secret), model id and attempt budget. Deployable to Amazon Bedrock AgentCore Runtime (warm session slots, idle/max lifetime) or AWS Batch on Fargate per `runtime`, from one container image built by AWS CodeBuild for the runtime's architecture.
    -   Note: The AgentCore runtime is available in the commercial partition only and requires `useCodeBuild`; the Fargate runtime adds private subnets with NAT egress for research. Aborting an in-flight AgentCore run from VAMS is not supported; the run is bounded by `maxRunSeconds`.
