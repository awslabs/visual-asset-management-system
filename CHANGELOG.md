...
-   **Search** Natural-language (vector) search — `POST /search/nlp` embeds the query with the configured Amazon Bedrock embeddings model (Amazon Titan Text Embeddings V2 by default), ranks per-file-version embeddings from the Amazon DynamoDB vector index across the caller's accessible databases, filters hits through both authorization tiers, and returns the latest live file versions in the existing search envelope with an `nlp` block (`truncated`, `candidatesEvaluated`, `itemsCollapsed`, `classIntent`) and a `_vector` block per hit. Segment vectors — a video's time windows and a document's content chunks — are searched beside the whole-file vectors (breadth and depth calls per target) and collapse to one hit per file, which reports `_vector.segmentHits` and the winning `bestSegment`; `includeSegments: false` searches whole-file vectors only. A file type named in the query (`video of …`, `pdf about …`) lists files of that type first and is echoed as `nlp.classIntent` — a boost, never a filter. Asset mode groups hits by asset; OpenSearch-only filters constrain and enrich results when an OpenSearch mode is on and are reported as `warnings` otherwise. Returns `503` while the vector index is building.
    ...
    -   **Web** The search page hosts search providers as tabs — Asset List (always) and Search (when OpenSearch or vector search is enabled) — with a Keyword / Natural language toggle when both engines are enabled (a single-engine deployment shows no toggle), a reduced filter sidebar when OpenSearch is off, relevance shown as a percentage in the first data column of either layout, and a "top N semantic matches" note when results are truncated.
...
-   **Pipelines** Consolidated `SYSTEM - GenAI Metadata Generation` pipeline (`system-genai-metadata`) — a Lambda-first pipeline with three container images (Blender renderer, 3D renderer, media extractor) and a per-file-class branch: images, video keyframes, audio tags, PDF text and pages, text and data files, Word, PowerPoint and Excel documents, 3D Tiles, meshes, USD, CAD, point clouds, splats, and IFC. Writes two layers per file: `sys_*` file attributes (the complete extraction) and file metadata made of a typed `ext_*` promotion of those attributes (dimensions and bounds as `xyz`; vertex, face, point, page, row, and other counts as `number`; watertight, texture, animation, and colour flags as `boolean`; capture and creation dates as `date`; units, size category, CRS, codecs, camera, language, and IFC schema as `string`), a GeoJSON `location` (points from EXIF GPS and IFC sites; polygons from geographic point-cloud bounds, 3D Tiles regions, and GeoJSON footprints) written only when the file has none, and a `genai_*` block from an Amazon Bedrock Converse analysis (Claude Haiku 4.5 by default; Claude Sonnet 4.5 in GovCloud) — title, description, keywords, category and subcategory, style, materials, colors, objects, complexity, orientation, size estimate, text summary — classified against an admin-editable `classificationVocabulary` object in the template body (`vamscli pipeline template update --config-body-file`; deploy restores the shipped default). Per-file segment vectors: content chunks of document, text, and data files (`CONTENT_CHUNKING`, on by default; `.docx`, `.pptx`, and `.xlsx` join the allow list) and, when `VIDEO_SEGMENT_SECONDS` is set above 0, fixed time windows of a video analyzed by a Step Functions Distributed Map (one Bedrock vision call per window, at most 360 windows, per-window descriptions kept as `segments/<segmentKey>.json` results); the counts land in `genai_content_chunk_count`, `genai_segment_count`, and `genai_segment_interval_seconds`. Also writes optional asset keywords and categories, an `analysis-summary.json` results file, and — when vector search is enabled — a per-file-version embedding published as a `vector.embedding.ready` event (published even when the resolved manifest carries no asset bucket id; the vector indexer then resolves the file's bucket from the asset row). Attributes are written before the model is called, so a Bedrock failure still lands them and records the execution FAILED with an explicit `Bedrock*` error code (`BedrockGuardrailIntervened` when the optional Amazon Bedrock guardrail configured in `bedrockGuardrail` blocks a prompt); a database that restricts metadata to schemas records the execution FAILED naming the undeclared `ext_*` field, and the pipeline page ships importable schema JSON for every field. Governed by the `system-genai-metadata-default` template (`SEED_WITH_EXISTING_METADATA`, `INCLUDE_SIBLING_FILES`, `RENDER_VIEWS`, `MAX_TEXT_CHARS`, `WRITE_ASSET_KEYWORDS`, `EMBEDDING_INCLUDE_TEXT_EXCERPT`, `WRITE_EXTRACTED_METADATA`, `EXTRACT_GEO_LOCATION`, `VIDEO_SEGMENT_SECONDS`, `CONTENT_CHUNKING`). Size gates (`lambdaLimits.maxInputFileSizeMb`, `maxPointCloudPoints`) degrade to attributes-only unless the `useFargateRenderer` sub-flag adds the AWS Batch renderer branch.
...
-   If a GenAI metadata execution ends FAILED with a `Bedrock*` error code, the file's `sys_*` attributes and `ext_*` metadata are already written; check Amazon Bedrock model access in the deployment Region (and the Anthropic use-case form) and re-run the workflow.
