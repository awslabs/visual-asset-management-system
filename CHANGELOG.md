...
-   **Pipeline** GenAI CAD STEP Agent pipeline (`app.pipelines.useGenAiCadStepAgent`) - A Strands agent creates or modifies a CAD STEP file (.stp, .step) from a prompt: it optionally researches the design online, writes CadQuery scripts, runs them in a bounded subprocess (own session, allow-listed environment, resource limits, process-group kill on timeout; the agent process is non-dumpable so a script cannot read its `/proc` entry) with a bounded self-repair loop, and writes the STEP file, a Markdown report and `cadAgent*` file metadata (status, summary, unresolved items, sources, geometry) back to the asset. Two templates over one pipeline (modify with one input file, generate with none) and a workflow for each; per-run tags for the prompt, output file name and prefix, internet research, model provider (Amazon Bedrock by default, OpenAI through a Secrets Manager secret), model id and attempt budget. Every run applies an Amazon Bedrock guardrail with a prompt-attack input filter to the instruction and to every fetched page (`bedrockGuardrail`, deployment-created by default); the page-fetch tool accepts only public http(s) hosts and re-validates each redirect hop. Deployable to Amazon Bedrock AgentCore Runtime (warm session slots, idle/max lifetime; one run per session at a time) or AWS Batch on Fargate per `runtime`, from one container image built by AWS CodeBuild for the runtime's architecture; the deployment waits for that build before creating the runtime. On the Fargate runtime an execution abort stops the agent container (nothing is uploaded or reported); a run that lands on a busy warm session slot is retried onto that slot for up to about 15 minutes.
    -   Note: The AgentCore runtime is available in the commercial partition only and requires `useCodeBuild`; the Fargate runtime adds private subnets with NAT egress for research. Aborting an in-flight AgentCore run from VAMS is not supported; the run is bounded by `maxRunSeconds` (300-6000 s).
