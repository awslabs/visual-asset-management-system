...
-   **Web / CDK** The `web` and `infra` lockfiles now record the rolldown and esbuild native bindings for Linux and macOS alongside Windows. npm captures only the binaries matching the platform that generated the lockfile, and a later `npm install` on another platform does not add the missing one, so a Linux build failed with `Cannot find native binding` once Vite began loading rolldown's binary. The bindings are declared as `optionalDependencies` and carry their own `os`/`cpu` constraints, so each machine still installs only its own.
