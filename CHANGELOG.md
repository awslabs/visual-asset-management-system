...
-   **Backend** Compliance: schema-driven compliance enforcement under `/compliance/*`. Versioned `vams-rules-v1` compliance schemas (metadata, relationship and pipeline rule types with `quarantine`/`warn`/`inform` enforcement, schema inheritance, `GLOBAL` or database scope, system schemas) with register, get, update and delete; database and asset-level schema bindings with an automatic-evaluation flag; on-demand evaluation, per-schema sweeps and an SNS-triggered evaluation on asset create/update; quarantine listing, release and documented exceptions (an exception moves the asset to the `exception` state, is scoped to the bound schema's name and version, holds through re-evaluations until it is revoked with `DELETE /compliance/quarantine/{databaseId}/{assetId}/exception` or superseded by an evaluation against another schema or version, and is audited as `exception_granted`/`exception_revoked`/`exception_superseded`); approval-gated cascades that re-evaluate an asset's downstream assets in dependency order, run asynchronously by a dedicated executor Lambda (create and approve return `202` with the cascade `state`, polled through `GET /compliance/cascades/{cascadeId}`); and a per-asset and deployment-wide compliance audit trail, paged and filterable by event type and time window. The database overview, quarantine and binding listings page on `maxItems`/`startingToken`/`NextToken`; a sweep reports the bound assets the caller may not evaluate as `skipped`. Binding, sweep and cascade operations are authorized on the target database or asset in addition to the compliance object; a pipeline rule's completion is recorded at most once and the evaluation finalized exactly once across concurrent callbacks; the automatic trigger reads uploads from the file indexer's SQS-wrapped events, ignores files and asset-row changes written by a workflow execution (the asset row records `lastChangeSource`), evaluates an upload once however many files it carries and whichever detection path reports it first (a change an evaluation has already read is skipped; a later change starts a new evaluation), opens at most one pending cascade per parent, and an older callback never overwrites a newer evaluation's state. Legacy (non-`vams-rules-v1`) schema rows are listed with `schemaFormat: legacy` and cannot be bound. Pipeline rules launch a standard workflow execution per rule with an explicit `inputFiles` selection (`matching` the asset's files through the workflow's, the pipeline's and the rule's own filters, `wholeAsset`, or `explicit` keys, honouring the workflow's input-file arity), read the pipeline's optional `compliance-output.json` from the execution's recorded output results, and derive `execution_success` / `processing_duration_seconds` for a workflow that writes none; files a workflow execution wrote are never selected as inputs and never re-trigger an evaluation, and an input-selection or launch failure is a non-verdict `error` rule result (`TOOLING_FAILURES_APPLY_ENFORCEMENT`) that applies no enforcement, leaves the asset state unchanged and is audited as `evaluation_error`. Schema bodies must be `vams-rules-v1` documents; an ad-hoc evaluation against a named schema never changes the asset's binding. Three Casbin object types (`complianceSchema`, `complianceEvaluation`, `complianceCascade`), two permission templates (`compliance-admin`, `compliance-readonly`) and four `vams-rules-v1` schema templates under `documentation/complianceSchemaTemplates/`. `GET /database/{databaseId}` exposes `complianceSchemaName` and `complianceAutoEval`.
    -   Note: a `compliance-output.json` is written under the execution's standard results output prefix; `backendPipelines/common/compliance_output.py` provides the `write_compliance_output` helper.
-   **Backend** Workflow completion event: the workflow end-state, error handler, abort path and the lazy Step Functions status reconcilers put a `workflow.execution.completed` event on the orchestration bus, published only by the writer whose terminal-status write landed (`Source` `<eventSourcePrefix>.execution.<executionId>`, detail `executionId`, `workflowDatabaseId`, `workflowId`, `status`, `startedAt`, `completedAt`, optional `executionGroupId`), consumed by the compliance workflow callback.
-   **CDK** Compliance deploys with every stack: five Amazon DynamoDB tables (`ComplianceSchema`, `ComplianceAssetState` with a `ComplianceStateIndex`, `ComplianceEvaluation` with an `ExecutionIdIndex`, `ComplianceCascade`, `ComplianceAudit`), nine Lambda functions, the `/compliance/*` routes in `apiBuilder2`, an EventBridge rule for the completion event with a KMS-encrypted dead-letter queue and retries, and admin-role constraints for the three compliance object types. New `app.compliance` settings: `autoLoadDefaultSchema` (default `true`; seeds the `GLOBAL` `default-compliance-schema` once, requires `app.metadataSchema.autoLoadDefaultAssetSchema`) and `quarantineBlocksDownload` (default `false`; the asset download, stream, export and auxiliary-preview endpoints refuse a quarantined asset when `true`).
-   **Web** Compliance pages — Compliance Schemas (create, edit, sweep), Quarantine (release, exception with a reason dialog, paged), an `exception` state badge, a Release action with a reason dialog and a Revoke exception action on the asset Compliance tab, a pipeline-rule input-file selector in the schema editor, Cascade Approvals (approve, reject with a reason dialog, execution progress), Audit Log (event-type filter, paged) — plus a per-database compliance overview, a Compliance Schema field in the database editor and a Compliance tab on the asset view with Evaluate Now and the evaluation history; all permission-filtered through web route constraints.
-   **CLI** `vamscli compliance` command group (`schema list|get|create|update|delete`, `bind`, `unbind`, `bindings`, `evaluate`, `sweep`, `state`, `evaluations`, `quarantine list|release|exception|revoke-exception`, `cascade …`, `audit`; the paged listings take `--max-items`/`--starting-token`) and matching VAMS MCP tools (read tools plus write/destructive-gated `create_compliance_schema`, `update_compliance_schema`, `delete_compliance_schema`, `bind_compliance_schema`, `unbind_compliance_schema`, `evaluate_asset_compliance`, `sweep_compliance_schema`, `release_quarantine`, `grant_quarantine_exception`, `revoke_quarantine_exception`, `create_compliance_cascade`, `approve_compliance_cascade`, `reject_compliance_cascade`).
-   Compare-Mode Visualizers - The viewer plugin framework now supports a compare mode alongside the existing visualize path. Compare-capable viewers declare a `compareMode` block (file-count window plus same-file-versions vs different-files gating) and are surfaced only in compare mode; a viewer that can only compare (the Text Diff Viewer) additionally declares `compareMode.compareOnly` and is never offered on the visualize path. Reachable from search results multi-select ("Compare Selected"), the version-comparison Compare actions, per-file compare in the version file list, and a "Compare Selected Files" action in the asset file manager, all hosted by a Visualize/Compare toggle in the file viewer modal. Every entry point is gated by the registered viewers: Visualize is offered only when a non-compare viewer admits the selection and Compare only when a compare viewer admits its file count, shape and types — the modal shows a single admitted mode without the toggle and a "no viewer for this selection" state when neither applies, a compare-only viewer is never listed in the Visualize picker for any file count, and the version file list offers per-row Compare (selected version vs latest) only for types a differ handles. A single viewed file can enter Compare too: when a differ handles its type, the file viewer modal and the single-file view page offer the Visualize/Compare toggle, and Compare opens the differ on the viewed version against latest (the differ's per-side version picker selects any other version). Ships a new Text Diff Viewer plugin (`react-diff-viewer-continued`, side-by-side/inline, line/word/character granularity, line-number toggle, collapse-unchanged with configurable context lines, syntax highlighting, dark-mode aware) that dynamically loads its diff library so it stays out of the base bundle. Compare selections may span assets: each entry is resolved to its own database/asset (identity is database + asset + key, so the same path under two assets is classified as different files rather than versions of one), viewers opt in via `compareMode.allowCrossAsset`, each side is fetched and authorized independently with a per-side "not authorized" / "archived" state instead of a single failure, and each side carries its own version picker (default latest) that re-fetches only that side.
...
-   **Workflows & Pipelines** The workflow editor offers the **Triggers (optional)** step when creating a workflow as well as when editing one. Drafted triggers are written after the workflow is created; a trigger the server refuses reopens on the new workflow's Triggers step with the reason and a Retry action, and a role that can create workflows but not set triggers sees the step explain that instead of failing at save.
-   **Workflows & Pipelines** Execute-workflow dialog redesigned as one dialog with a step rail: the workflow picker is its first step, a workflow's requirements appear once (on its row, then in a strip under the title), the Inputs step stacks input files, output target (database, asset and path prefix on one row) and metadata sources as full-width sections, workflow rows carry their pipelines' category chips and the search matches them, each pipeline step says what it reads and folds long template instructions, each template input shows the {{tag}} placeholder it fills and a checkbox input shows its name beside the box, the tag icon explains itself on hover and its catalog lists the template's own tags before the system tags, and Review lists every blocker once with Edit links back to the step
-   **Workflows & Pipelines** The execute dialog's Inputs step takes hundreds of files for a multi-file workflow: the selection is one compact list (N files across M assets, grouped by asset, a compatibility badge per file, filter, Edit/Remove, guarded Clear all) that draws only the rows on screen; **Add files…** opens a bulk picker — database and asset, the asset's files listed page by page with checkboxes, a server-side folder prefix, a name filter, **Select all shown** / **Select all matching** (walks the pages up to the 1,000-file limit), **Add whole asset** / **Add folder** where permitted, and a **Paste keys** mode for one key per line; **Add Input File** keeps the single-row Database → Asset → File picker with version pinning. A selection over the 1,000-file limit is a blocker on the rail, under Review and on Launch; Review summarises the inputs per asset with a windowed list. A preset selection from the file manager opens straight into the list.
-   **Workflows & Pipelines** Execution details and logs expose a pipeline step's internal progress and every log it has.
    -   `GET /workflows/executions/{executionId}/details` reports `availableLogs` on every pipeline entry — the step's invocation log, each location the pipeline registered, and a registered nested state machine's log group, each with a stable `logId` — and, with the new `includeSubExecutions=true` query parameter, `subExecutions`: each registered sub-process with its status and an ordered list of stages whose status, timing, retries, and error are derived at read time from the sub-state-machine definition and execution history (a caught failure is marked). Stage reads are bounded and best-effort; `pipelines.subExecutions` is named in `truncatedCollections` when stages are dropped under the response budget.
    -   `GET /workflows/executions/{executionId}/logs` in full mode with a pipeline scope reports `logSources` (each source with a read status of `read`, `denied`, `notFound`, `error`, `empty`, `skipped`, or `unscoped`), names every `subProcessEvents` item's source by `logId`, and takes two new query parameters: `logId` reads one source alone (with its own `nextToken`, and a nested state machine's history), and `stageName` keeps only the sources and history of one stage. Registered container log streams are read exactly, without the execution-scope filter, when they fall under a prefix registered on the same pipeline execution. Full-mode CloudWatch events and retrieval warnings name their log group (`logGroupName`) rather than its ARN.
    -   The registration event a pipeline publishes gains optional `stageName`, `label`, and `sourceType` on each log entry and `stageName` / `label` on the sub-execution; log entries are deduplicated by location with metadata merge, and an event is accepted only when its `Source` names the pipeline execution it registers for. Three validators back the new fields: `SFN_STATE_NAME`, `DISPLAY_LABEL`, `LOG_SOURCE_TYPE`.
    -   Every built-in pipeline that registers a sub-process labels its state-machine log and, for each AWS Batch state, registers the container log group its job definition writes to under that job definition's stream prefix tied to the state; the execution service may read the registered groups and describe Batch jobs to resolve exact streams.
    -   A registered AWS Deadline Cloud job (`deadlineCloudJob`) is reported in `subExecutions` with its live job status folded onto the execution vocabulary (a cancelled job reads `ABORTED`) and a `deadline` block naming its farm, queue, and job id, and contributes an `availableLogs` entry of kind `deadlineCloudJob` (source type `deadlineCloud`) whose full-mode read lists the job's sessions and reads their exact CloudWatch log streams (the 10 most recent) — `notFound` until the job has a session, never `unscoped`. The execution service's Deadline Cloud grant, present only when that execution type is enabled, adds `deadline:ListSessions` and read access to the `/aws/deadline/*` log groups.
    -   **CLI** `vamscli execution details --include-sub-executions` renders each step's `Sub-processes` with per-stage status and its `Logs available`; `vamscli execution logs --log-id` and `--stage-name` (full mode, with `--pipeline-execution-id`) read one source or one stage, and full-mode output lists `Log sources` with their read status and tags each sub-process log line with its `logId`.
    -   **MCP** `get_execution_details(include_sub_executions=...)` and `get_execution_logs(log_id=..., stage_name=...)` forward the same parameters; the importable `vams_mcp.__version__` now matches the package version, pinned by a test.
    -   **Web** The execution detail page renders a `Sub-processes` timeline per step and the log viewer offers a `Log source` selector with a per-source status row; a Deadline Cloud sub-process names its farm, queue, and job id.
    -   **External Plugin** No connector change: neither the NVIDIA Isaac Sim nor the Esri ArcGIS Pro connector invokes `execution details` or `execution logs`, and no command they invoke changed an option or a `--json-output` key (checked as required by the CLI propagation rules).
-   **Workflows & Pipelines** A file version written by a workflow execution links to that execution from the asset file manager. The details pane's **Change Source** row offers **View execution**, which opens the execution quick view beside the page (its **Open full details** link still reaches the execution page), and each workflow-written row of the file-versions modal offers **View execution**, which closes the modal and opens the execution detail page; both links appear only for a caller allowed to read execution details. `GET /database/{databaseId}/assets/{assetId}/fileInfo` and the enriched (non-`basic`) items of `GET /database/{databaseId}/assets/{assetId}/listFiles` report `changeWorkflowId` and `changeWorkflowExecutionId` for the current version, read from the same object metadata as `changeSource`; `vamscli file info` and `vamscli file list` print them as `Change Workflow` / `Change Execution`. The execution quick view also names its **Execution ID**, and it closes on the first Escape when its first control is an info tooltip.
-   **Workflows & Pipelines** Template create/edit form recomposed as **Basic** (name, description, input instructions, default template, execution-time body edit), **Pipeline overrides**, **Tags and Config Body**, and **Review**. Tags and Config Body puts the tag schema beside the configuration body: each declared tag is a chip that inserts its `{{tagKey}}` placeholder at the cursor with the quoting its type needs in a `json` body (string and enum quoted, integer, number, boolean and string-list bare; other formats always bare), a declared tag the body never references is flagged, a `json` body is checked as it is typed against the same shape rule the save applies, and the system-tag catalog sits under the editor. Review lists the options, the overrides, a table of the tags and a read-only body preview; a completed step reopens from the progress strip, and the edit route loads a template into the same steps.

### Bug Fixes

-   **Workflows & Pipelines** Execute-workflow dialog: search dropdowns no longer clip inside the scrolling body or narrow to their column; the Back/Next/Launch buttons stay fixed while a long step scrolls
-   **Workflows & Pipelines** A workflow execute request the workflow itself rejects (no input file for an arity-one workflow, files spanning several assets on a single-asset workflow, a filtered-out file) is now refused with that rule named, instead of a message about the output asset it consequently could not resolve
-   **Pipelines** The Isaac Lab evaluation pipeline labels its registered sub-processes and log sources as evaluation rather than training
-   **Workflows & Pipelines** A step's invocation log (the invoked Lambda's own log group) is read scoped to the execution id alone; scoping it to the pipeline execution id as well, which a plain Lambda step never logs, read every such step's log as empty
-   **Workflows & Pipelines** Executions board scoped to one workflow now preselects that workflow when executing instead of asking which workflow to run
-   **Workflows & Pipelines** A full-mode execution-log continuation no longer passes the CloudWatch `nextToken` to the Step Functions history read, which rejected it; the history section is served on the tokenless first page as documented.
-   **Workflows & Pipelines** `subProcessEvents` are now sorted by timestamp, as the API reference already stated.
-   **Workflows & Pipelines** Redelivery of a registration event naming an already-registered log location no longer duplicates the entry, including a legacy four-key entry redelivered onto a row that stores the seven-key form.
-   **Workflows & Pipelines** Container logs of AWS Batch steps were unreachable through the execution logs API because container output does not carry the execution id the scoped search requires; a registered log-stream prefix (or the exact stream resolved from the sub-state-machine history or `batch:DescribeJobs`) is now read without those terms.
-   **Documentation** The execution details reference stated that internal details are omitted without saying which; it now names ARNs (omitted) and notes that `availableLogs` carries log-group names and stream prefixes.
-   **Web** Asset file manager: on an asset with hundreds of files the listing becomes interactive before the asset name has loaded, and when the name arrived the tree rename cleared the file selection, unmounting the details pane and closing an open **Automation** menu or execute dialog. The rename now leaves the selection alone.
-   **Web** Execute-dialog asset and file pickers request their results in name / key order, so the first page (100 hits) is deterministic and shallow paths list first. Wildcard search hits came back in index order, so a file that shared its name with more than a page of derived outputs (`a1.txt` beside hundreds of `a1.txt.mockout.json`) could not be reached however the term was refined.
-   **Backend** Execution logs redaction: a Step Functions task token that reached the workflow log group wrapped in an AWS Deadline Cloud job-parameter envelope (`"VamsTaskToken": {"String": "…"}`) survived redaction; the envelope is now stepped over and the value is redacted like a bare one.
-   **Backend** Execution logs and sub-execution redaction: a Step Functions task token carried as an AWS environment variable (`{"Name": "TASK_TOKEN", "Value": "…"}` — the shape a failed `batch:submitJob.sync` task reports in its Cause through the AWS Batch DescribeJobs object) survived redaction, and a sub-execution or stage cause was capped before it was redacted, so a cut mid-token kept whatever fit. The environment shape is now redacted by its variable name (`TASK_TOKEN`, `VAMS_TASK_TOKEN`, `EXTERNAL_SFN_TASK_TOKEN`, and the other credential names), and every sub-execution and stage cause is redacted before it is capped.
-   **Web** Asset file manager: a `?filePath=` deep link opened while the asset page was already loaded did not move the selection (the navigation effect consulted a one-shot flag that a later effect reset), and a deep link arriving while the detailed listing phase was still streaming — many seconds on a large asset — was deferred until that phase completed, so the details pane and the Automation dialog kept the previous file. The navigation now keys on the path last navigated to and runs as soon as the basic listing holds the target.
-   **Web** Asset edit form: when the asset record in hand carried no `databaseId`, the tag-type and tag reads fell back to the unscoped lists, so required tag types from other databases (test fixtures among them) were announced as constraints on the form and tags from every database were offered. Both reads now scope to the asset's database (from the route when the record lacks it) plus GLOBAL, and never to the unscoped list.
-   **Web** The database picker for a new pipeline or workflow, and the workflow Review step, mark the GLOBAL scope with the same 🌐 glyph the tag-management forms use.
...
-   **CDK** The Fargate pipeline job definitions (coordinate transform, Blender renderer, 3D thumbnail, PDAL, Potree) write container output to `/aws/vendedlogs/Pipelines/<Name><hash>` log groups encrypted with the deployment's KMS key, under the prefix the workflow execution-service role is granted to read, instead of AWS Batch's default `/aws/batch/job` group.
...
-   **Security / Pipelines** Every pipeline Lambda's log output redacts the Step Functions task token: by key under all of its spellings (`TaskToken`, `taskToken`, `sfnExternalTaskToken`, `externalSfnTaskToken`, `TASK_TOKEN`), inside lists and Batch environment name/value pairs, and inside message text where the event or the token itself was rendered into the line. The handlers log their event as a structured field and no longer log the token on its own, and the 14 vendored logger copies are identical.
-   **Security / Pipelines** The 3D thumbnail, GenAI metadata labeling, and Potree point cloud viewer (PDAL and Potree) containers log the pipeline job name at entry instead of the whole pipeline definition, which carried the raw Step Functions task token into the container's CloudWatch log group.
-   **Workflows & Pipelines / CDK** The five Fargate pipelines (coordinate transform, Blender renderer, 3D thumbnail, PDAL, Potree) register the `/aws/vendedlogs/Pipelines/<Name><hash>` group their job definitions write to as each Batch stage's container log source, and the job definitions' `awslogs` stream prefix is the physical job definition name, so the resolved container stream falls under the registered `<jobDefinitionName>/default/` prefix and the execution log view reads it; the Potree viewer registers one group per converter job. GPU Batch pipelines (NVIDIA Cosmos, GR00T, Isaac Lab, Splat Toolbox) keep registering AWS Batch's default `/aws/batch/job` group.
-   **Pipeline / CDK** The Potree point cloud viewer PDAL container pins the conda-forge `compilers` toolchain to the 1.x generation (`compilers<2`) so the image builds; the unpinned upstream moved to GCC 15, which dropped the CC/CXX/AR activation scripts and rejects PDAL 2.7.0's vendored kazhdan sources.
...
-   New Coordinate Transform Pipeline - Reprojects point clouds between coordinate reference systems (E57, LAS, LAZ in; LAZ, LAS, E57, PLY out) with EPSG/PROJ/WKT/custom-grid support and per-asset metadata overrides
...
-   **An external-bucket-only deployment now requires an explicit default asset bucket.** `app.assetBuckets.externalAssetBuckets` entries take a new `isDefault` field: at most one entry may set it, and exactly one **must** when `app.assetBuckets.createNewBucket` is `false`, or configuration validation fails at `cdk synth`. The default bucket is where a database created without an explicit bucket stores its assets, and where workflow execution inputs and pipeline outputs are written. Setting `isDefault` on an external entry while `createNewBucket` is `true` overrides the created bucket as the deployment default. A v2.5 `config.json` with `createNewBucket` set to `false` therefore needs one entry marked before it deploys.
...
-   **Web** New SuperSplat Editor viewer plugin embedding the open-source PlayCanvas SuperSplat Gaussian-splat editor, supporting `.lcc` (XGRIDS multi-LOD, not previously viewable in VAMS), `.ply`, `.sog`, and `.splat`, and now the default splat viewer. This is the first iframe-embedded viewer; its editing and export tools operate in the browser only and are not saved back to VAMS. The editor renders through WebGPU and requires a WebGPU-capable browser (current Chrome and Edge, Safari 26 or later, Firefox with WebGPU enabled).
...
-   **Web** The create-asset upload wizard no longer advances past a step whose requirements have not finished loading. A database's required tag types and a metadata schema's required fields are both fetched when their step is reached, and each step reported itself valid while its fetch was still in flight — so clicking Next quickly enough submitted an asset with a required tag unselected or a schema-required field empty, which the API then rejected. Each step now reports itself invalid until its requirements have settled and shows a loading indicator while they are pending, so a blocked Next is explained rather than unexplained.
-   **Web** A rejected create-asset call during an upload now reports the server's message and offers a route back to the wizard.
...
-   **Backend** The Users in Roles listing no longer fails when one assignment row was stored without a `createdOn` value.
-   **Backend** Updating a Cognito user no longer erases their phone number. `PUT /user/cognito/{userId}` takes email and phone independently. The update is now partial: an attribute the request does not mention keeps the value it has, and removing the number takes an explicit `clearPhone: true`, accepted on its own or alongside an email change. A request carrying both `phone` and `clearPhone` is refused.
-   **Security / Backend** Deleting a role now removes every user-role assignment it holds, and refuses the deletion when it cannot. Assignments orphaned by an earlier release are not removed by this change and need deleting by hand.
...
-   **Pipelines** A configuration failure in the ModelOps or RapidPipeline conversion pipelines now fails the parent workflow instead of leaving it running for its full timeout. The step that builds the container command holds the workflow's task token and carries no error handler, so raising ended the pipeline's own state machine before the step that reports the token could run; the run showed as RUNNING for hours after it had already failed. The step now reports the token itself, and no longer hands a value back on a failure route — the container command is resolved from that value, so a populated one was started as a command line instead of ending the run. Successful runs are unaffected.
-   **Pipelines** The Gaussian Splat Toolbox templates expose their reconstruction and crop settings as template tags. `RUN_RECON`, `RUN_TRAIN` and `CROP_OUTPUT_BOUNDS` are optional booleans and `CROP_MODE` is an optional enum offering `rigid_body` and `environment` — the values the crop script actually accepts — defaulting to `rigid_body` for the Objects template and `environment` for Environments-360. The keys resolved to real runtime configuration but were not declared, so a caller had no way to set them: an operator-supplied value was dropped before reaching the container, because a tag is only substituted when the template declares it AND its `configBody` references it.
...
-   **Security** Two-tier authorization now fails closed on missing identity — the asset-create, database-create, database-update, asset-ingest, asset-export, download, and upload handlers deny when no authenticated identity is present instead of falling through to the operation.
-   **Security** Asset version and file operations no longer fail open on an empty token list.
-   **Security** The `USE_LOCAL_MOCKS` local-development switch is ignored on a deployed AWS Lambda function. It made `POST /auth/routes` return every submitted web route as allowed without evaluating permissions, and skipped the denial of a request that presented no identity; the switch now applies only where resource names are not resolved from AWS Systems Manager Parameter Store, and the combination is recorded as an error. Web routes gate frontend navigation only and no deployment configuration sets the variable, so no deployment was affected.
-   **Security** Deleting a tag type is authorized before anything is read. `DELETE /tag-types/{tagTypeId}` ran the existence check and the in-use scan of the tag table first, so a caller holding the route but no `tagType` DELETE permission for that scope could tell an absent type (`404`) from a referenced one (`400`) from an unreferenced one (`403`) — an existence and in-use oracle over scopes it cannot read, and a whole-table scan for every probe that named a type that exists. Such a caller is now answered `403` in every case and nothing is read. The tag-type listing is fail-closed on a missing identity as well, so neither path rests on the API-level check alone. An authorized caller sees no change: an unreferenced type still deletes, a referenced one is still refused with `400`, and a name that does not exist is still `404`.
...
-   Building a user's authorization policy reads only that user's rows. Role assignments were read with a full table scan filtered on `userId` even though `userId` is that table's partition key, and the roles table was scanned end to end on every session that had not presented MFA, so the cost of the 60-second policy refresh in every handler Lambda grew with the deployment's total user and role count rather than with the caller's own assignments. A caller holding a valid token whose effective role list is empty — no assignment at all, or, in a session that has not presented MFA, only roles that require it — is also denied immediately, rather than treated as a failed read and retried three times with a one-second pause after each attempt.
...
-   The constraint listing pages past its first page, and each page holds whole constraints. `GET /auth/constraints` returned `NextToken` as the raw DynamoDB pagination key — an object, rather than the opaque string every other VAMS listing returns and the only thing the `startingToken` parameter accepts — so a client that followed the token sent back a rendered object and received a generic listing failure. A malformed token now reports an invalid pagination token instead of a generic failure.
-   **Security / Backend** The database and bucket listings bound the page size a caller may ask for.
-   **Security** Creating or updating a permission constraint is refused when a role named in its group permissions cannot be confirmed to exist. The write is now rejected and can be retried once the role exists. This applies to `POST`/`PUT /auth/constraints`; `POST /auth/constraintsTemplateImport` deliberately still creates its constraints and records a warning when the role is absent, since a template is routinely imported before the role it targets is created. Constraints already stored against a missing role are unaffected and keep needing the role created by hand.
...
-   **Security / Backend** Tag-type create, update, list, and delete no longer return the underlying exception text to the caller. An unexpected failure reports a generic message and the specifics are logged instead; the actionable messages — tag type not found, already exists, or still in use by a tag — are unchanged.
...
-   **Pipelines** An NVIDIA Isaac Lab custom environment package can now be installed.
...
-   **Search** A reindex that could not complete fails the deployment instead of reporting success.
-   **Security / Search** The reindexer no longer writes the CloudFormation `ResponseURL` to its log group.
-   **Search** A file whose name carries no extension is now indexed and searchable. Folder detection treated any path whose last segment had no dot as a folder, so `LICENSE`, `Dockerfile`, `Makefile`, `README` and every extension-less data export were skipped by both the S3-event and metadata-stream indexing paths — reported as a successful skip, so nothing retried and no error surfaced, and the file stayed permanently invisible to file search. Folder-ness is decided from the key shape (a trailing `/`), which is what S3 itself uses for a folder marker. Upload validation refused these files as well and now accepts them (see **Assets**). Re-index to make existing extension-less files searchable.
...
-   **Security** A pipeline container image built by AWS CodeBuild is pushed and consumed under a content-addressed tag derived from its container source, replacing the mutable `:latest`. Under a mutable tag what a pipeline runs can change with no change to this repository and no redeploy, because the next build overwrites the tag the AWS Batch job definition already names. The tag is supplied to the build by the pipeline's own CodeBuild project and named at the pull site from that same value, a build for which it is not supplied fails rather than defaulting, and `:latest` is kept only as a Docker layer-cache alias pushed alongside. Covers the NVIDIA Cosmos and GR00T image builds and the Coordinate Transform, Gaussian Splat Toolbox, and Isaac Lab Training builds added since. One behaviour changes deliberately: a failed build no longer leaves a stale image for jobs to run, so an execution fails with a container-pull error instead of silently running an older image.
-   **Security** The NVIDIA GPU container images clone their upstream repository at a pinned commit, verified after checkout, and record it in the image so a run's log names the inference code it ran.
-   **Pipelines** The Gaussian Splat Toolbox container image ships the segmentation model weights its background-removal and object-removal options load, rather than leaving each job to download them. Attributions for the shipped weights are recorded in `NOTICE.md` and the notices page.
...
-   **Assets** Two overlapping asset-version writes on one asset no longer land on the same version number. Versions already stored are unaffected.
-   **Assets** The internal read of an asset's versions returns the complete set.
-   **Assets** Listing an asset's versions costs one file-count read for the page rather than one per version, and `maxItems`/`pageSize` above 1,000 — the maximum the API specification already published for them — are refused with `400` instead of accepted.
-   **Assets** A file whose name carries no extension can be uploaded. A file with no relative key at all is still refused.
-   **Assets** An upload completion no longer reverts an asset edit made while the upload was in flight.
-   **Assets** An asset preview whose file is not an allowed image type is now rejected when the upload is initialized rather than when it is completed. The extension check ran only for an asset that already had a preview, so the first preview uploaded to an asset was accepted and handed presigned part URLs, and was rejected only at completion — leaving whatever parts the client had already pushed as an incomplete multipart upload. The same check now also covers a `.previewFile.` companion inside an ordinary file upload, which was never validated at initialization. The upload pages and the "Overall Preview File" picker validate the same rule before the upload starts, so the offending file is named in the browser instead of the whole request failing with a batch-level error, and the CLI reads a companion's extension after the `.previewFile.` marker as the API does rather than after the last dot.
-   A permission denial on asset update, archive, unarchive, or permanent delete is reported as `403` rather than `500`.
...
-   **Security** The Gaussian Splat Toolbox container build records which third-party sources its Dockerfile resolves while the image builds rather than at a fixed revision, and fails the deployment's synthesis when that set changes in either direction.
...
-   **Pipelines** The GenAI 3D Metadata Labeling pipeline no longer fails on an unexpected Amazon Bedrock response shape after the run's model and label calls have already been paid for. A summarization reply of `null`, a bare number, or a JSON object under the keywords key raised past the pipeline's only handler, which caught JSON parse errors alone; such a reply is now stored as the keyword value verbatim, as an unparseable reply already was. A per-image reply that carries no usable text is skipped so the remaining images and Amazon Rekognition still contribute labels, while a summarization reply carrying no usable text fails the execution with the response shape named — there is nothing to store, and reporting success with an empty keyword list would hide it.
...
-   **Pipelines / Security** A RapidPipeline EKS conversion reports its outcome without copying the mesh optimiser's own output into the execution record.
-   **Pipelines** A ModelOps run whose input configuration cannot be read fails its workflow immediately instead of leaving it running for four hours.
...
-   **Pipelines** The 3D Basic Conversion pipeline distinguishes the formats it reads from the formats it writes. One list served both checks, so 3MF, XYZ, XAML, 3DXML and COLLADA were offered as conversion targets the Trimesh library cannot produce, and every run naming one failed after downloading its input. XYZ remains a working input format -- Trimesh reads it as a point cloud -- and is now refused only as a target, while XAML, 3DXML and COLLADA are refused as both, since the container has no loader for them either.
-   **Pipelines** The NVIDIA Cosmos Predict pipeline no longer publishes an arbitrary artifact when a run leaves more than one video in its output directory.
-   **Security** Role-constraint criterion values are checked for catastrophic backtracking when saved: a quantifier applied to a group that itself repeats or alternates, a backreference, and a pattern carrying more than three unbounded quantifiers are rejected. A value of this shape re-runs for every policy line on every authorization decision, so one hangs authorization for the affected role until the function times out. Existing stored constraints are unchanged and are never re-validated on read — **re-check stored constraints** for these shapes.
...
-   The API key listings report and follow their pagination. `vamscli api-key list`, `vamscli api-key user list`, and the web API Key Management page each read one response from `GET /auth/api-keys` / `GET /auth/user/api-keys` and neither sent nor followed a pagination parameter, so a deployment holding more keys than one response carries reported a subset as the whole set with nothing to indicate the listing was short. Both commands now take `--page-size`, `--max-items`, `--starting-token`, and `--auto-paginate`, and print the `NextToken` whenever a page is truncated — including a bounded page that legitimately holds no items, which is where the token matters most. The web page pages server-side with a pagination control, requesting the backend's own 1,000-key page size so an ordinary deployment still answers in one page; past that its text filter and column sorting apply to the page shown, which the table header states.
...
-   **Search** The record-type discriminator is written to every indexed document as `str_rectype` (`"asset"` or `"file"`). It was declared on both OpenSearch document models with a leading underscore, which Pydantic v1 excludes from a model's fields entirely, so no indexed asset or file document carried a record type even though both index mappings and the API reference declared one. A `query_string` filter on the documented field was also discarded by the query builder rather than applied, returning the full unfiltered result set; a filter on `str_rectype` is now honoured (use `str_rectype.keyword` for an exact match). The field appears in `POST /search` and `POST /search/simple` hit sources, as a column in `vamscli search` table and CSV output, and backs the asset/file classification in the search map view. Existing indexes acquire the field on reindex.
-   **Search** `POST /search/simple` returns file-index results. The returned page was always cut from the start of the collected hits, and those hits carried every asset hit ahead of every file hit, so a database with at least as many matching assets as the requested page size returned asset hits only — on every page — which is the opposite of what a file or key lookup asks for. **Result ORDERING changes for `POST /search` as well:** the merge sort was added to the shared dual-index read both endpoints call, so a query matching both indexes now interleaves asset and file hits by relevance (or by the requested sort) instead of returning all asset hits first. A caller that relied on the previous grouping sees a different order; no hit is added or removed.
-   **Search** `POST /search` and `POST /search/simple` responses carry the `_shards` object the API reference declares as required. It is now declared under an alias and serialized under the documented `_shards` key; no other response key changes.
-   **Search** The API reference no longer documents search fields no index writes. `num_size` was listed as a hit `_source` field, but a file's size is indexed and returned as `num_filesize`, which the reference lists alongside it. The metadata field-naming section listed `str_meta_{key}` / `num_meta_{key}` / `date_meta_{key}` / `bool_meta_{key}`, a spelling that appears nowhere in VAMS: all of a record's metadata is stored in one field named `MD_` with the keys carried verbatim (file attributes likewise in `AB_`), so metadata `{"product": "Training"}` reads back as `"MD_": {"product": "Training"}`. No indexed field, request parameter, or response key changes.
...
-   **Security / Pipelines** The Fargate pipeline containers run as a non-root account. The GenAI 3D metadata labeling image declares no user and continues to run as root; its job definition is otherwise unchanged. Each container now logs the uid it is running under once at start.
-   **Pipelines** The 3D Basic Conversion and CAD/Mesh Metadata Extraction Lambda functions allocate 4 GiB of ephemeral (`/tmp`) storage instead of the 512 MiB Lambda default, and each invocation now stages its files in a working directory of its own that is removed before the function returns. The pipeline documentation pages state both bounds.
...
-   **Security** A role constraint criterion value is checked for catastrophic backtracking when it is saved. **Constraints already in the table are not re-validated, on read or otherwise, so operators should re-check their stored role constraints** — criterion values written by earlier releases were never checked for complexity and a stored one keeps working exactly as before. Review them through `GET /auth/constraints`, `vamscli role-constraint list`, or the web permissions page, and re-save any whose criterion value repeats a repeating group or uses a backreference.
-   **Security** A user id is normalized before it is validated, stored, or looked up, and a new Amazon Cognito user whose id reads the same as an existing one is refused. User ids stay Unicode, so an external identity provider's non-Latin username is accepted as before, but two compatibility spellings of one name (a fullwidth character for its ASCII counterpart, a decomposed accent for a composed one) no longer become two identities that render identically. The lookalike check covers the Cyrillic and Greek letters that are visually identical to a Latin letter; it runs at creation only, so existing user ids are unaffected.
-   **Security** An asset id supplied by the caller is refused when it carries a non-ASCII character, for the same reason: `pуmp` and `pump` render identically in an asset listing and an audit record. That rule applies at asset creation only — through `POST /assets` and both `POST /ingest-asset` stages — so an asset id already stored stays addressable on every read, download, archive, and workflow execution, and an asset auto-created from an external Amazon S3 bucket still takes its id from the folder name found there.
-   **Security** Resetting another user's Amazon Cognito password requires an explicit confirmation. The request body must set `confirmReset` to `true`; a request that omits the body, sends an empty one, or sends the field as `false` or `null` is refused without reaching Amazon Cognito. The confirmation only took effect when the field was sent explicitly as `false`, so a request that simply left it out reset the account's password unchallenged — a mis-wired client or an accidental retry forced the user through a credential change. `vamscli user cognito reset-password` already requires `--confirm` and the web application always confirms, so neither is affected.
-   **Backend** Unarchiving an asset requires an explicit confirmation. `PUT /database/{databaseId}/assets/{assetId}/unarchiveAsset` must set `confirmUnarchive` to `true`; a request that omits the field, sends an empty body, or sends it as `false` is refused.
-   **Backend** Deleting a metadata schema requires an explicit confirmation. `DELETE /database/{databaseId}/metadataSchema/{metadataSchemaId}` must set `confirmDelete` to `true`; a request that omits the field, sends an empty body, or sends it as `false` is refused.
-   **Backend** Metadata writes refuse the reserved reindex marker key. `metadataKey` was bounded only by length, so a metadata create or update on an asset, file, asset link, or database could store `REINDEX_METADATA_RECORD`. The name is now rejected at request validation; reading and deleting a record an earlier release stored under it are unaffected. A metadata key carrying the VAMS-internal `VAMS_` prefix or a leading underscore remains accepted: the record is stored and every metadata read returns it, and the prefix costs only search indexing and, for a leading underscore, asset export output.
-   **Backend** Surrounding whitespace is removed from a submitted `description` or version `comment` before it is length-checked and stored, so a padded value and its clean spelling are one record rather than two that render identically. This covers asset create/update and asset ingest, asset version create/revert/update, database create/update, tag and tag type create/update, role create/update, permission constraint create, and API key create/update (both the administrative and the self-service routes). **A padded value whose trimmed length falls below the field's minimum is now refused with `400` where it was previously accepted and stored** — for example a four-character minimum against `"  ab  "`. Interior whitespace is preserved. Values already stored padded are unchanged, and the trim is deliberately not applied to file paths, S3 keys, opaque echoed tokens, or search terms, where a surrounding space is either meaningful or changes which rows match. Pipeline, template and workflow descriptions are unaffected and remain stored as submitted.
-   **Backend** `PUT /roles` no longer requires `description`. Updating only a role's other fields meant re-sending the existing description or being refused with `400`, because the update model declared it required while the record already carried one. It is now optional on update and omitting it leaves the stored value untouched; supplying it still enforces the same 1–256 character bound, and `POST /roles` still requires it.
-   **Security** The API Gateway authorizer picks up an identity provider signing-key rotation without waiting out its key cache, and both of its request caches are bounded. The API key and user role caches hold a bounded number of entries and drop already-expired ones first, so a stream of distinct invalid API keys no longer grows a warm container's memory for the life of the container.
-   **Backend** An asynchronous large-file upload records the initiating user on its audit entry. A file over 1 GB is finalized in a separate queue-triggered function, which attributed the entry to a one-off identity of its own (`SYSTEM_ASYNC_PROCESSOR`) instead of the user the queuing request recorded — an identity present in neither the user table nor the user-roles table, so the entry could not be correlated with the synchronous upload-completion entry or with the change provenance stamped on the same file version. The entry now names that user, falling back to the reserved `SYSTEM_USER` identity when the queued message carries none.
-   **Backend** Creating an asset link reads every page of the asset-link table's `fromAssetGSI`, so the duplicate-link and cycle checks see the whole set.
-   **Search / Garnet Framework** The asset and file indexers read every page of the DynamoDB queries behind a document's relationship flags, its current version, and its `MD_`/`AB_` metadata. The Garnet Framework add-on's asset, file, and database indexers do the same for the relationship flags, metadata, attributes, asset links, and current version they publish as NGSI-LD properties. Existing documents and published entities keep the wrong flags and the truncated metadata until they are reindexed.
-   **Search** Removing a permanently deleted file from the search index resolves its database only from an asset record registered in the bucket and prefix the event came from. An asset id is unique within a database but not across databases, so once the deleted asset's own record was gone and exactly one other database happened to hold an asset with the same id, that record was accepted unchecked and the delete removed the other database's **live** file document by exact document id — which then stayed invisible in search until a reindex, with no error anywhere to attribute it to. A record that belongs to another bucket or prefix, or that cannot be verified against one, now hands off to the orphan cleanup that matches on asset id, path, bucket, and prefix together. The remaining failure direction is a document left in the index rather than one removed from another database, and a reindex clears it.
-   **Search** A failed index operation is retried instead of being dropped. Updates lost by an earlier release are not recovered by this change; reindex to restore them.
-   **Backend** The asset-link child tree walk expands each asset once and is bounded in depth and node count. Cycle handling is unchanged, and an ordinary hierarchy returns exactly as before.
-   **Backend** A failed asset-link tree traversal is answered with an error instead of an empty hierarchy. The largest hierarchies are both the ones most likely to fail and the ones whose emptiness is least plausible, so the answer a caller could least verify was the one most likely to be wrong. Such a request is now refused with a general error. An asset that really has no children still answers `children: []` with HTTP 200.
-   **Backend** A linked asset the read could not retrieve is no longer reported as one the caller is not permitted to see. `batch_get_item` answers a partially throttled request — or one that hits the 16 MB response cap — with HTTP 200 and the remaining keys in `UnprocessedKeys`, raising nothing, so those assets were simply absent from the batch result and every relationship arm counted them into `unauthorizedCounts`. A genuine authorization denial is still counted as before.
-   **Backend** Exporting an asset costs per asset instead of per file, and an asset holding more files than one page can carry is exported across pages. **A caller that follows `NextToken` can now receive the same asset on more than one page carrying a different subset of its files, so a consumer that accumulates pages must merge an asset's `files` by `databaseid`/`assetid` instead of appending a second entry for it.** `vamscli assets export --auto-paginate` (the default) does that merge, so its combined output still holds one record per asset with the complete file list, and it gains a `--max-files` option. A page never separates a file from its `.previewFile.` companions, so the preview association is the same as in an unpaged export. An export that already fits inside the budget behaves exactly as before, with no `NextToken`.
-   **Security / Backend** The asset export no longer writes the caller's token claims, or the asset-links response body, to its log group.
-   **Backend** A metadata write or delete that DynamoDB did not fully accept is reported as a failure instead of a clean success.
-   **Backend** Metadata schema validation no longer fails open. Schema conformance, the controlled-list check, the value-type-change guard, and the `restrictMetadataOutsideSchemas` off-schema key prohibition ran inside a block that logged a warning and continued on any unexpected error, so a transient DynamoDB error on the existing-metadata read or the schema read turned a configured governance control off and answered `200` with off-schema metadata stored. A metadata create or update whose validation cannot complete is now refused with a `400` and nothing is written; the four metadata delete paths refuse a deletion whose validation cannot complete in the same way. Injecting schema-supplied default values stays best-effort, since it only adds fields the checks have already accepted.
-   **Backend** Every AWS client the backend builds now carries the adaptive retry configuration the backend development rules require (`retries={'max_attempts': 5, 'mode': 'adaptive'}`).
-   **Backend** Subscriber change notifications are delivered for assets whose name is long or carries a line break. The Amazon SNS email subject was built from the asset name unbounded, and SNS refuses a subject of 100 characters or more or one containing a line break or control character — so an asset name of roughly 70 characters or more, or one carrying a newline (both of which the asset-name validator accepts), made every version-change notification for that asset fail permanently while only logging. The subject now folds control characters to spaces and is trimmed to the documented bound; the message body is unchanged and still carries the full name. An asset whose notification topic was removed when its subscription was deleted is a recorded no-op instead of a logged stack trace, and every path of the notification function returns its response.
-   **CLI / Web** An upload no longer reports success for a file the server refused.
-   **CLI** A `.previewFile.` companion is no longer lost when several files are uploaded together. A companion is always sequenced separately from the file it previews, and the API accepts it only if that base file arrives in the same request or already exists in S3 — so the companion's sequence had to complete after the base file's.
-   **Backend** A file uploaded over an existing key is no longer treated as new because of the error code its absence check reads.
