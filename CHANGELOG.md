...
-   New Natural-Language (Vector) Search - Semantic search over files backed by Amazon Bedrock embeddings stored in an Amazon DynamoDB vector index, with optional Amazon OpenSearch constraint and enrichment, per-file segment vectors (video windows, content chunks) that collapse to one hit per file, and a soft file-type boost from the query's wording, exposed through a new `POST /search/nlp` endpoint, the web search page, the CLI, and the MCP server. Enabled by default on commercial AWS, off by default in AWS GovCloud (US), and unavailable in the AWS European Sovereign Cloud
-   New System Pipelines - Pipelines and workflows shipped from `vamsSchema` bundles can be marked `isSystem`; system records are read-only through the API, web, CLI, and MCP except for their `enabled` switches and template configuration bodies and tag schemas. The 3D Preview Thumbnail pipeline ships as `SYSTEM - Preview`
-   New Consolidated SYSTEM GenAI Metadata Pipeline - One Amazon Bedrock-only Lambda pipeline (`system-genai-metadata`) replaces the GenAI 3D Metadata Labeling and CAD/Mesh Metadata Extraction pipelines: it analyzes every viewer-supported file type, always writes file attributes and GenAI file metadata, and produces per-file-version embeddings when vector search is enabled. Runs without a VPC by default; an optional AWS Fargate renderer sub-flag adds an AWS Batch branch
-   Search Page Provider Model - The Assets and Files page hosts search providers as tabs (asset list always; unified keyword/natural-language search when OpenSearch or vector search is enabled) with a natural-language toggle
-   Workflow Execution Locks - The `perAsset`, `perInputFile`, and new `perInputFileVersion` workflow concurrency restrictions are all locks on a DynamoDB lock table, so a conflicting second execution is rejected while the first is running and an asset's execution history never affects a launch

### ⚠ BREAKING CHANGES

-   **The GenAI 3D Metadata Labeling (`genai-metadata-3d-labeling-obj-glb-fbx-ply-stl-usd`) and CAD/Mesh Metadata Extraction (`metadata-extraction-cad-mesh`) GLOBAL pipelines are retired and replaced by the `SYSTEM - GenAI` pipeline (`system-genai-metadata`).** Their nested stacks are removed, their registrations are archived on deploy, and user workflows and role constraints that name the old pipeline ids must be repointed. The configuration keys `app.pipelines.useGenAiMetadata3dLabeling` and `app.pipelines.useConversionCadMeshMetadataExtraction` are rejected by configuration validation; use `app.pipelines.useSystemGenAiMetadata` (analysis model id in `bedrockAnalysisModelId`). Legacy `autoGeneratedKeywords` asset metadata and `AB_*` attribute rows are left in place.
    -   **Migration note:** run `infra/deploymentDataMigration/v2.6_to_v2.7/upgrade` after deploying: `orphanedTriggers` removes file-upload trigger rows whose workflow is archived or missing (including the retired built-ins'), `vectorBackfill` invokes the vector reindexer to embed the latest live file versions, and `systemPipelineRetirement` reports still-running executions and user workflows that reference the retired ids. Abort in-flight executions of the retired workflows before deploying; otherwise they turn FAILED after their task timeout.
-   **The 3D Preview Thumbnail pipeline's category changes from `Preview` to `SYSTEM - Preview`, and the new pipeline's category is `SYSTEM - GenAI`.** `category` is a `pipeline`/`workflow` constraint field, so role constraints scoped to the `Preview` or `GenAI` categories must be updated.
-   **Amazon Rekognition is no longer called by any built-in pipeline.** The Rekognition VPC interface endpoint is no longer created, and the pipeline Lambda roles carry no `rekognition:*` grant. The Amazon Bedrock Runtime VPC interface endpoint is created whenever an in-VPC Lambda function needs it.
-   **The backend Lambda layer's boto3/botocore pin moves to 1.43.89** (the DynamoDB `SearchVectors` API floor). Redeploying rebuilds the base layer with Docker.
...
-   **Every concurrency restriction is a lock on the `WorkflowExecutionLocksStorageTable`.** `perAsset` locks each selected input asset, `perInputFile` each selected input file, and the new `perInputFileVersion` each workflow, file, and S3 version; a second execute over a locked scope while one is running returns `400` with a message naming the granularity, and a stale lock expires through the table's TTL. The launch no longer inspects the asset's execution history for `perAsset` and `perInputFile`, so the `limit could not be fully confirmed` warning is no longer emitted and an asset's depth of history cannot delay or refuse a launch.
...
-   **Search** Natural-language (vector) search — `POST /search/nlp` embeds the query with the configured Amazon Bedrock embeddings model (Amazon Titan Text Embeddings V2 by default), ranks per-file-version embeddings from the Amazon DynamoDB vector index across the caller's accessible databases, filters hits through both authorization tiers, and returns the latest live file versions in the existing search envelope with an `nlp` block (`truncated`, `candidatesEvaluated`, `itemsCollapsed`, `classIntent`) and a `_vector` block per hit. Segment vectors — a video's time windows and a document's content chunks — are searched beside the whole-file vectors (breadth and depth calls per target) and collapse to one hit per file, which reports `_vector.segmentHits` and the winning `bestSegment`; `includeSegments: false` searches whole-file vectors only. A file type named in the query (`video of …`, `pdf about …`) lists files of that type first and is echoed as `nlp.classIntent` — a boost, never a filter. Asset mode groups hits by asset; OpenSearch-only filters constrain and enrich results when an OpenSearch mode is on and are reported as `warnings` otherwise. Returns `503` while the vector index is building.
    -   **CDK** New `app.vectorSearch` configuration section (`enabled`, `embeddingModelId`, `embeddingDimensions`, `indexingConcurrency`, `reindexOnCdkDeploy` — a CloudFormation custom resource that clears and rebuilds the vector table during a deployment, the vector counterpart of `app.openSearch.reindexOnCdkDeploy`; also the CDK context `vectorReindexOnCdkDeploy`), `VECTORSEARCH` feature switch, the `VectorEmbeddingsStorageTable` with a conditional DynamoDB vector index (`vec-<model>-<dims>`), and a vector indexing construct in the search stack: an EventBridge rule for `vector.embedding.ready`, the vector indexer queue and dead-letter queue, the vector indexer (the single writer, handling new versions, archive/unarchive, and delete), the vector reindexer (`clear`/`enqueue`/`both`, invoked directly or as the reindex-on-deploy custom resource), and the paced system-workflow launcher. The search stack builds after the pipeline stack while vector search is enabled, so the reindexer finds the SYSTEM workflow registered.
    -   **Backend / CDK** Search and indexing handlers are grouped by family: `handlers/osSemanticSearch` (the keyword `/search` route and the OpenSearch `osFileIndexer` / `osAssetIndexer` stream consumers), `handlers/osVectorSearch` (the vector indexer, reindexer, launcher, and `/search/nlp`), and the core `handlers/indexing` (bucket sync, SNS queuing, and the row-rewriting `crReindexer` every family's consumers re-process). The Lambda builders follow the same split: `indexingFunctions.ts`, `osSemanticSearchFunctions.ts`, and `osVectorSearchFunctions.ts`.
    -   **Web** The search page hosts search providers as tabs — Basic Asset List (always) and Primary Search (when OpenSearch or vector search is enabled). Primary Search places the query in a full-width band under the page title: a Keyword / Natural language switch when both engines are enabled (a single-engine deployment shows no switch), the search box with its Search button, and a line stating what the active mode matches — keyword lists everything in scope on an empty query, natural language searches file contents including image and video scenes and shows a "Describe what you are looking for" state (no count, no Clear-filter action) until a description is entered. The sidebar's Assets/Files selector is "Search for"; the "Search inside files" option also matches file contents, including image and video scenes. A reduced filter sidebar when OpenSearch is off, relevance shown as a percentage in the first data column of either layout, and a "top N semantic matches" note when results are truncated.
    -   **Web** The Basic Asset List tab offers Delete Selected (archive or permanent delete) and Unarchive Selected through the same modals as Primary Search, plus a Show archived toggle, so bulk asset actions are reachable on a deployment with no search engine.
    -   **CLI** New `vamscli search nlp` command (gated on the `VECTORSEARCH` feature switch) with database, entity-type, file-class, file-extension, archived, segment (`--no-segments`), size (`--size`), and OpenSearch filter options; the table output adds a `segment` column when a hit was found through a window or chunk. **MCP** New read-tier `search_nlp` tool (`include_segments`).
    -   **Backend** The existing `/search` routes read their OpenSearch environment lazily and return the documented `404` when OpenSearch is disabled.
-   **Pipelines** Consolidated `SYSTEM - GenAI Metadata Generation` pipeline (`system-genai-metadata`) — a Lambda-first pipeline with three container images (Blender renderer, 3D renderer, media extractor) and a per-file-class branch: images, video keyframes, audio tags, PDF text and pages, text and data files, Word, PowerPoint and Excel documents, 3D Tiles, meshes, USD, CAD, point clouds, splats, and IFC. Writes two layers per file: file attributes — `sys_*` groups (the complete extraction), the `ext_*` facts promoted from them (dimensions and bounds, vertex, face, point, page and row counts, watertight/texture/animation/colour flags, capture and creation dates, units, size category, CRS, codecs, camera, language, IFC schema; string-typed, as attributes are), and the run's `genai_model`, `genai_generated_at`, `genai_source_modalities`, `genai_complexity` and `genai_content_chunk_count` provenance — and file metadata made of a GeoJSON `location` (points from EXIF GPS and IFC sites; polygons from geographic point-cloud bounds, 3D Tiles regions, and GeoJSON footprints) written only when the file has none, and the descriptive `genai_*` block from an Amazon Bedrock Converse analysis (Claude Haiku 4.5 by default; Claude Sonnet 4.5 in GovCloud) — title, description, keywords, category and subcategory, style, materials, colors, objects, orientation, size estimate, text summary — classified against an admin-editable `classificationVocabulary` object in the template body (`vamscli pipeline template update --config-body-file`; deploy restores the shipped default). Per-file segment vectors: content chunks of document, text, and data files (`CONTENT_CHUNKING`, on by default; `.docx`, `.pptx`, and `.xlsx` join the allow list) and, when `VIDEO_SEGMENT_SECONDS` is set above 0, fixed time windows of a video analyzed by a Step Functions Distributed Map (one Bedrock vision call per window, at most 360 windows, per-window descriptions kept as `segments/<segmentKey>.json` results); the counts land in the `genai_content_chunk_count` attribute and the `genai_segment_count` and `genai_segment_interval_seconds` metadata rows. Also writes optional asset keywords and categories, an `analysis-summary.json` results file, and — when vector search is enabled — a per-file-version embedding published as a `vector.embedding.ready` event (published even when the resolved manifest carries no asset bucket id; the vector indexer then resolves the file's bucket from the asset row). Attributes are written before the model is called, so a Bedrock failure still lands them and records the execution FAILED with an explicit `Bedrock*` error code (`BedrockGuardrailIntervened` when the optional Amazon Bedrock guardrail configured in `bedrockGuardrail` blocks a prompt); a database that restricts metadata to schemas records the execution FAILED naming the undeclared `genai_*` field, and the pipeline page ships importable schema JSON for every metadata field. Governed by the `system-genai-metadata-default` template (`SEED_WITH_EXISTING_METADATA`, `INCLUDE_SIBLING_FILES`, `RENDER_VIEWS`, `MAX_TEXT_CHARS`, `WRITE_ASSET_KEYWORDS`, `EMBEDDING_INCLUDE_TEXT_EXCERPT`, `WRITE_EXTRACTED_METADATA`, `EXTRACT_GEO_LOCATION`, `VIDEO_SEGMENT_SECONDS`, `CONTENT_CHUNKING`). Size gates (`lambdaLimits.maxInputFileSizeMb`, `maxPointCloudPoints`) degrade to attributes-only unless the `useFargateRenderer` sub-flag adds the AWS Batch renderer branch.
-   **Workflows & Pipelines** System pipelines and workflows — `isSystem: true` is set only by the `vamsSchema` importer and surfaced in pipeline and workflow responses; system records reject update, archive, restore, template add/delete, and trigger add/delete with `400`, allowing only the `enabled` switches, template `configBody`/`tagSchema`/`webFormJson`, and trigger `enabled`. Web list pages show a **System** badge and open system records read-only; CLI and MCP help text names the toggle paths (`workflow trigger set --disable`, `set_workflow_trigger(..., {"enabled": false})`).
-   **Workflows & Pipelines** Workflow execution locks — the new `WorkflowExecutionLocksStorageTable` (TTL safety net) holds one conditional-write lock row per asset (`perAsset`), input file (`perInputFile`), or workflow + file + S3 version (the new `perInputFileVersion` restriction) a running execution holds; a conflicting execute returns `400` with a message naming the granularity, a newer file version runs concurrently under `perInputFileVersion`, and the terminal handlers release the rows. Surfaced in the pipeline and workflow forms of the web interface and the CLI. Used by the system GenAI workflow.
-   **Workflows & Pipelines** `System-Reindex` trigger type on executions launched by the vector reindexer (`executionGroupId` `vec-<runId>-<chunk>`), filterable in the executions list, the web board, and `execution list --trigger-type`. Executions and workflow-execution input records carry the manifest's asset bucket ids.
-   **Pipelines** Reserved `execution.status.json` results file and `outputS3AssetResultsPath` in the resolved manifest — any pipeline can record a FAILED outcome after writing partial outputs, and any pipeline with `events:PutEvents` on the orchestration bus can contribute embeddings by publishing `vector.embedding.ready`.
-   **CDK** The trigger dispatcher skips triggers whose workflow is archived or disabled; archiving a workflow deletes its trigger rows (a restore does not re-create them); the registration custom resource's `Delete` path deletes triggers before archiving.
-   **CDK** The SYSTEM GenAI metadata pipeline creates its Amazon Bedrock guardrail (`bedrockGuardrail.create`, on by default in the commercial template): a prompt-attack input filter at `promptAttackInputStrength` (`LOW`/`MEDIUM`/`HIGH`), PII and credential filters per `piiFilter` (`off`/`anonymize`/`block`), encryption with the VAMS KMS key, and a published version the two analysis functions reference and hold `bedrock:ApplyGuardrail` on; `guardrailIdentifier`/`guardrailVersion` still reference an operator-owned guardrail with `create.enabled` `false`.
    -   **Note** The guardrail is named `VAMS-SystemGenAiMetadata-<deployment hash>`; an orphan left by a failed teardown must be deleted before redeploying the same configuration. The GovCloud and EU Sovereign templates ship with `create.enabled` `false`.
...
-   **Pipelines / Vector search** The SYSTEM GenAI metadata pipeline screens the text it embeds — the composed whole-file text, every content chunk (several per call), and each video window's text — through the configured Bedrock guardrail (`ApplyGuardrail`) before the embeddings model receives it, and embeds and stores the guardrail's masked output as `sourceText`, so PII masked in the analysis prompt is no longer embedded raw or persisted in the vector table; a blocked text is a `BedrockGuardrailIntervened` failure (a blocked chunk is skipped while the others are embedded), the embedding summary records `guardrailMasked` with the masked types, and the embedding function holds `bedrock:ApplyGuardrail` on that guardrail.
-   **Backend** The concurrency lock rows of a workflow execution stopped outside VAMS (a direct AWS Step Functions stop, a deleted state machine) are released when the executions list or the execution details view first observes the run as terminal, instead of blocking every launch on that asset, file, or file version until the lock table's time-to-live expired them.
-   **Web** Search page: a search submitted with Enter or the Search button within the 500 ms auto-refresh window after a mode or filter change ran the same query twice (embedding it twice in natural-language mode), and a mode switch made while typing searched the half-typed text when the window closed; an explicit search now supersedes the pending refresh and the refresh runs only the query it was armed with.
-   **Web** Search page accessibility: the result table's row-selection checkboxes, the pagination arrows, the toast dismiss button and status icon, the filter dropdowns' filtering inputs, and the Table/Map view switch carry accessible names. The keyword search box no longer advertises wildcards, which the general query escapes.
-   **CLI** `vamscli search ... --output-format csv` writes only the CSV document to stdout (the status lines led the file when redirected). With OpenSearch off and vector search on, the keyword search refusal points at `vamscli search nlp`.

### Chores

-   **Documentation** New Vector search concept page, System pipelines and SYSTEM GenAI metadata pipeline pages; the GenAI labeling and CAD/Mesh extraction pages are removed; search, architecture, deployment, overview, troubleshooting, user-guide, CLI, API, and MCP pages updated.
-   **Documentation** Root, backend, backend test, pipeline, infra, web, CLI, MCP, and documentation `CLAUDE.md` files and their Kiro mirrors updated for the new domains and rules; `/add-pipeline` corrected (`vamsSchemaDir`, placement-dependent VPC blocks, NOTICE step), `/add-api-endpoint`, `/update-changelog`, `/update-docs`, and `/deploy-check` refreshed.
...
-   Natural-language search evaluates at most 100 candidates per vector query (the DynamoDB `SearchVectors` `TopK` ceiling) and has no cursor; the response reports `truncated` when a whole-file window filled and the `segments:window_full` warning when only a segment window did.
-   Amazon Bedrock calls from Lambda functions use standard (non-FIPS) endpoints even when `app.useFips` is true.
-   DynamoDB vector search is not available in the AWS European Sovereign Cloud; `app.vectorSearch.enabled` must be `false` there.
-   Changing `app.vectorSearch.embeddingDimensions` on a populated vector index is unsupported; follow the model-change procedure (clear, disable, change, enable, backfill).
-   Only the latest live version of a file is searchable through the natural-language API, although embeddings are stored per file version.
-   The CLI `search nlp` command does not expose the `tags` and `geoSearch` constraints that `POST /search/nlp` accepts and the MCP `search_nlp` tool forwards; both are OpenSearch-only constraints that the API ignores with a warning when OpenSearch is disabled.
-   Per-window video analysis (`VIDEO_SEGMENT_SECONDS`) costs about one Bedrock vision call per window (three frames, about 5,000 input tokens; about $0.005 with Claude Haiku 4.5, about $2 per hour of video at a 10-second interval) and is off by default; matching windows are reported through `_vector.bestSegment`, and returning every matching timestamp is not in this release.
-   Anthropic models on Amazon Bedrock require a one-time use-case form per AWS organization; until it is submitted, GenAI metadata executions end FAILED with `BedrockAccessDenied` while file attributes are still written.
...
-   If a GenAI metadata execution ends FAILED with a `Bedrock*` error code, the file's `sys_*` and `ext_*` attributes are already written; check Amazon Bedrock model access in the deployment Region (and the Anthropic use-case form) and re-run the workflow.
-   If natural-language search returns `503 Vector index is being built`, the vector reindexer is rebuilding the index; wait for the `System-Reindex` executions to finish and retry.
-   If a file-upload trigger of a retired pipeline still fires after upgrading, run the `orphanedTriggers` step of the v2.6 to v2.7 migration.
