...
-   **CDK** Added Amazon Cognito VPC interface endpoints for VPC/isolated deployments. When Amazon Cognito is the authentication provider, the VPC builder now creates `cognito-idp` and `cognito-identity` interface VPC endpoints (with FIPS variants when `useFips` is set), gated on `useGlobalVpc.addVpcEndpoints`. This lets a VAMS deployment authenticate entirely within an isolated VPC — the browser completes Amazon Cognito SRP sign-in and the backend performs token/credential exchange without internet egress. It also **enables the Amazon Cognito MFA check for VPC-attached Lambda functions**. The MFA check is now disabled only for the AWS GovCloud (US) or AWS European Sovereign Cloud partition, where Amazon Cognito PrivateLink is not yet available (the endpoints are skipped in those partitions to avoid a deployment failure).
...
-   **Web** Fixed the API Key Management page guessing admin mode before the user's allowed API routes were known. Mode resolution now waits for (or directly fetches) the allowed-routes list, so self-service-only users get the user mode form (own user, required expiration) and API instead of the admin version.
...
-   **Web** Fixed the broken web Jest test infrastructure: installed the missing `jest-environment-jsdom` package (split out of Jest core since Jest 28), consolidated conflicting `transformIgnorePatterns` entries that were excluding ESM packages from transformation, and registered `@testing-library/jest-dom` matchers via `setupFilesAfterEnv`. Updated stale tests that no longer matched current component behavior (Navigation web-route permission filtering, FileMetadata MetadataContainer rendering, RoleGroupPermissionsTable role loading via APIService, WorkflowEditor element generation, removed Jest 30 deprecated matcher aliases).
...
-   Standardize the system user across the backend to the single ID `SYSTEM_USER`, previously a combination of `SYSTEM` and other lowercase/uppercase variations. `SYSTEM_USER` is the identity seeded into the user and user-roles tables during CDK deployment (assigned to the `admin` role) and is now used consistently for all system-process actions: lambda cross-calls, pipeline workflow executions, bucket-sync asset creation, seeded record `createdBy`/`modifiedBy` values, and `changeUserId` provenance fallbacks.
...
-   **CDK** Added `Access-Control-Allow-Origin` to API Gateway gateway-level responses (default 4XX and 5XX). Responses produced by API Gateway itself — the custom authorizer returning Unauthorized/Access Denied, a missing authentication token, and other 4XX/5XX errors — never reach a Lambda, so the per-handler CORS header could not cover them. A cross-origin caller (for example, the web application on a token-expiry `401`) now receives the real status instead of an opaque CORS/network error.
-   **CDK** The REST API now always writes an explicit resource policy matching the configured `endpointType`: an `aws:SourceVpce`-restricted policy for `PRIVATE` and a public allow-all policy for `REGIONAL`. Amazon API Gateway does not remove a previously-set resource policy when an update stops supplying one, so switching an existing deployment from `PRIVATE` to `REGIONAL` previously left the VPC-endpoint restriction in place — denying every public request (including the CORS preflight) with `403 AccessDeniedException` at the resource-policy layer, which a browser reports as a failed CORS preflight. Emitting the policy for both endpoint types guarantees a switch in either direction overwrites the prior policy.
