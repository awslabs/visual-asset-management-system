...
-   **web/infra/documentation**: Set `min-release-age = 7d` in all `.npmrc` files and require npm >= 11.0.0 via `engines.npm` constraints in `package.json`. Node 22 ships with npm 10.9.x, which does not support `min-release-age`; CI workflows and build sites (GitHub Actions, GitLab CI pages job, Amplify) now pin npm 11.0.0 after Node setup to enforce the 7-day minimum release age for all npm dependencies (#329).
