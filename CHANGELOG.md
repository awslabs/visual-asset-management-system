...
-   **Web** Compliance pages — Compliance Schemas (create, edit, sweep), Quarantine (release, exception with a reason dialog, paged), Cascade Approvals (approve, reject with a reason dialog, execution progress), Audit Log (event-type filter, paged) — plus a per-database compliance overview, a Compliance Schema field in the database editor and a Compliance tab on the asset view with Evaluate Now and the evaluation history; all permission-filtered through web route constraints.
-   **CLI** `vamscli compliance` command group (`schema list|get|create|update|delete`, `bind`, `unbind`, `bindings`, `evaluate`, `sweep`, `state`, `evaluations`, `quarantine …`, `cascade …`, `audit`; the paged listings take `--max-items`/`--starting-token`) and matching VAMS MCP tools (read tools plus write/destructive-gated `create_compliance_schema`, `update_compliance_schema`, `bind_compliance_schema`, `unbind_compliance_schema`, `evaluate_asset_compliance`, `sweep_compliance_schema`, `release_quarantine`, `grant_quarantine_exception`, `create_compliance_cascade`, `approve_compliance_cascade`, `reject_compliance_cascade`).
