...
-   **CDK** The 3D preview thumbnail, Potree point cloud viewer, and system GenAI metadata (Fargate renderer) state machines can describe and terminate the AWS Batch jobs they submit, so stopping an execution also stops its Fargate container.
-   **CDK** The Fargate pipeline job definitions (coordinate transform, 3D thumbnail, PDAL, Potree, and the system GenAI metadata renderer) write container output to `/aws/vendedlogs/Pipelines/<Name><hash>` log groups encrypted with the deployment's KMS key, under the prefix the workflow execution-service role is granted to read, instead of AWS Batch's default `/aws/batch/job` group.
...
-   **Security / Pipelines** Every pipeline Lambda's log output redacts the Step Functions task token: by key under all of its spellings (`TaskToken`, `taskToken`, `sfnExternalTaskToken`, `externalSfnTaskToken`, `TASK_TOKEN`), inside lists and Batch environment name/value pairs, and inside message text where the event or the token itself was rendered into the line. The handlers log their event as a structured field and no longer log the token on its own, the system GenAI metadata pipeline's handlers and containers log identifiers and keys rather than the event or state, and every vendored logger copy — the pipeline Lambdas' and the system GenAI metadata pipeline's three container and Lambda copies — is identical.
-   **Security / Pipelines** The 3D thumbnail and Potree point cloud viewer (PDAL and Potree) containers log the pipeline job name at entry instead of the whole pipeline definition, which carried the raw Step Functions task token into the container's CloudWatch log group.
-   **Workflows & Pipelines / CDK** The Fargate pipelines (coordinate transform, 3D thumbnail, PDAL, Potree, and the system GenAI metadata renderer) register the `/aws/vendedlogs/Pipelines/<Name><hash>` group their job definitions write to as each Batch stage's container log source, and the job definitions' `awslogs` stream prefix is the physical job definition name, so the resolved container stream falls under the registered `<jobDefinitionName>/default/` prefix and the execution log view reads it; the Potree viewer registers one group per converter job. GPU Batch pipelines (NVIDIA Cosmos, GR00T, Isaac Lab, Splat Toolbox) keep registering AWS Batch's default `/aws/batch/job` group.
