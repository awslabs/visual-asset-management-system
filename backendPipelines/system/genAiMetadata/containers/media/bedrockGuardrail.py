...
block that carries the untrusted parts of a prompt so the guardrail's input filters evaluate them, and the
account of an intervention that the caught failure records.
...
def guardrail_cause(response: dict) -> str:
    ...
    trace = (((response or {}).get("trace") or {}).get("guardrail") or {})
