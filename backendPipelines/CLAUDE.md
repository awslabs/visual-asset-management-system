...
-   **Use `outputS3AssetResultsPath`** for structured documents that describe the run rather than the asset (a summary, a report, the reserved `execution.status.json`). Every object under it is recorded as a `PipelineExecutionOutputResults` row (text, up to the row budget) and nothing under it is written to the asset. See "Reporting a Post-Write-Back Failure" below.
...
## Reporting a Post-Write-Back Failure (`results/execution.status.json`)

A pipeline whose outputs have already landed can still fail afterwards — the system GenAI metadata
pipeline writes a file's `.attribute.json` first and only then calls Amazon Bedrock, and a denied model
call must not discard the attributes. Raising at that point routes the sub-state-machine to
`SendTaskFailure`, the workflow to `HandleExecutionError`, and the staged outputs are never ingested. The
results channel carries the alternative:

1. Write `execution.status.json` directly under `outputS3AssetResultsPath` with the body
   `{"status": "FAILED", "error": "<code>", "cause": "<text, up to 1,024 characters>"}`, then **return
   normally** — no raise, no `SendTaskFailure`, no `$.error`. `PipelineEndTask` reports `SendTaskSuccess`
   as on any successful run and the workflow reaches its process-output step.
2. Process-output ingests `files/`, `previews/`, `metadata/` and `results/` exactly as for a succeeded
   run, then reads the status object on **both** terminal paths (results-only and asset write-back).
   `status: FAILED` records the execution and its end-state pipeline row `FAILED` with
   `executionError = "<error>: <cause>"`; any other status, or no object, leaves the outcome to the
   write-back result. The reported error heads `executionError`; write-back failure summaries, if any,
   follow it.
3. The object is recorded as an ordinary `PipelineExecutionOutputResults` row, so the report is also
   readable from the execution record.

`execution.status.json` is therefore a **reserved results object name**: a document of that name under
the results prefix is read as a status report whatever the pipeline meant by it. A status object that is
not a JSON object is itself recorded as a failure (`The pipeline execution status file could not be
read.`), because the pipeline signalled an outcome the step cannot read. The name is
`common.s3PathPatterns.EXECUTION_STATUS_RESULTS_FILENAME`; both terminal paths are pinned by
`backend/tests/handlers/workflows/test_processOutput_execution_status_file.py`.

Use the status object for a failure the pipeline **chooses to record** after its outputs exist. A crash
(function timeout, out-of-memory, import error) keeps the existing contract — it routes through `$.error`
to `PipelineEndTask` and `SendTaskFailure`, and staged outputs may be lost.

## Publishing an Embedding for Vector Search (`vector.embedding.ready`)

Any pipeline may contribute a per-file-version embedding to the vector index. The pipeline never writes
the vector table: it writes one embedding document and publishes one event on the orchestration bus, and
the vector indexer in the search stack (present only when `app.vectorSearch.enabled`) owns every table
write, including the latest/archived lifecycle of the item.
...
-   **Event** — `EventBusName` the deployment's orchestration bus, `Source` the manifest's
    `systemConfig.orchestrationEventPrefix` (`resolve_inputs()["orchestrationEventPrefix"]`),
    `DetailType` `vector.embedding.ready`, `Detail` the document's fields **without** `embedding` and
...
-   **Reference producer** — `backendPipelines/system/genAiMetadata/lambda/generateEmbedding.py`; its
...
-   the `manifestHelper.py` byte-identity check across every vendored copy
    (`backendPipelines/tests/test_manifest_helper_byte_identity.py`)
...
2. Add Lambda handler in `lambda/` subdirectory. **Every pipeline `lambda/` directory MUST include:**
    ...
    **All `customLogging/logger.py` copies must stay byte-identical** — verify with
    ...
    the canonical `preview/3dThumbnail/lambda/manifestHelper.py`, and
    `find backendPipelines -name manifestHelper.py -exec md5sum {} \; | awk '{print $1}' | sort -u`
    must print exactly one hash. Edit the canonical copy, then `cp` it over the rest in the same change.
    A pipeline that embeds also vendors `vectorsearch/embeddings.py`; its canonical copy is
    `backend/backend/common/vectorsearch/embeddings.py` and the same edit-then-`cp` rule applies, pinned by
    `backendPipelines/system/genAiMetadata/lambda/tests/test_sysgenai_vendored_copies.py`.
...
10. **Add pipeline flag to VPC builder** (`infra/lib/nestedStacks/vpc/vpcBuilder-nestedStack.ts`). A pipeline using AWS Batch, ECS, or Fargate goes into some of the VPC builder's three condition blocks — **which ones depends on the subnets its compute runs in.** Decide that first from what `pipelineBuilder-nestedStack.ts` passes as its `pipelineSubnets`: `pipelineNetwork.isolatedSubnets.pipeline` or `pipelineNetwork.privateSubnets.pipeline`. Search for `useSplatToolbox` (private) and `usePreview3dThumbnail` (isolated) to see both treatments.
    ...
    Isolated-subnet flags (block 2 only): `useConversionCoordinateTransform`, `usePreview3dThumbnail`, `usePreviewPcPotreeViewer`, `useSystemGenAiMetadata.useFargateRenderer`. Private-subnet flags (all three blocks): `useModelOps`, `useNvidiaCosmos`, `useNvidiaCosmos3`, `useNvidiaGr00t`, `useRapidPipeline.useEcs`, `useRapidPipeline.useEks`, `useSplatToolbox`. `useIsaacLabTraining` sits in blocks 1 and 2 and takes the ECS endpoint through `needsEcsIsolated`, so it is in neither list. Lambda-only pipelines (3D basic conversion, the SYSTEM GenAI metadata pipeline without its Fargate renderer) appear in no block; a container branch behind a sub-flag keys the block condition and the `vpcRequiringFeatures` entry on the sub-flag, never on the pipeline's `enabled`. The authoritative lists live in `infra/lib/nestedStacks/pipelines/CLAUDE.md` and are asserted against the source by `infra/test/platform/steeringVectorSearchRelease.test.ts`. Regression coverage asserting both directions: `infra/test/pipelines/coordinateTransformVpcPlacement.test.ts`.
