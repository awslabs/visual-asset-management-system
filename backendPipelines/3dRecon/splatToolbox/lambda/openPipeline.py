...
BATCH_STATE_NAME = "SplatToolboxBatchJob"
...
def register_sub_execution(orchestration_bus_name, orchestration_event_prefix,
    ...
    detail = {
        ...
        "subExecution": {
            ...
            "label": "Splat Toolbox processing",
        },
    }
    logs = []
    if STATE_MACHINE_LOG_GROUP_NAME or STATE_MACHINE_LOG_GROUP_ARN:
        logs.append({
            ...
            "label": "Splat Toolbox state machine",
        })
...
def lambda_handler(event, context):
    ...
    # Validate the extension against exact members of the comma-separated allow list. A
    # containment test against the joined string accepts any prefix of a listed extension
    # ('.mp' against '.mp4,.mov'), which admits a file the container cannot read.
