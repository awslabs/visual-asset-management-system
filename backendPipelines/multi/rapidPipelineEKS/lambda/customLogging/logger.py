...
class CustomFormatter(LambdaPowertoolsFormatter):
    def serialize(self, log) -> str:
        """Serialize final structured log dict to JSON str"""
        masked_log = mask_sensitive_data(event=log)  # process the log dict
        return self.json_serializer(masked_log)  # use configured json serializer
