...
class TestOutputExtensionResolution:
    ...
    def test_uploaded_object_carries_the_extension(self):
        ...
        assert command.rstrip().endswith(f"/files/{subdir}/pump.glb")
    ...
    def test_a_format_changing_conversion_uploads_directly_to_the_files_prefix(self):
        ...
        command, _ = self._run({"outputType": ".gltf"}, self._event())
        assert command.rstrip().endswith("/files/pump-gltf.gltf")
