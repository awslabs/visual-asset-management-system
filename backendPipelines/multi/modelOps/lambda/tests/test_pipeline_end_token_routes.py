...
"""modelOps ``pipelineEnd`` finds the workflow's callback token on both routes into it.
...
*   **Completed ECS task.** ``ModelOpsRunFargate`` declares no ``resultPath``, so the task
...
*   **Caught ECS error.** ``addCatch(handleModelOpsError, { resultPath: "$.error" })`` keeps the
...
token left unreported there holds the workflow task RUNNING for the pipeline's full 14400-second
taskTimeout while the container has already stopped.

The two event shapes here are built from the state machine in
``infra/lib/nestedStacks/pipelines/multi/modelOps/constructs/modelOps-construct.ts`` (the task's
result path, the catch's result path, and the single ``externalSfnTaskToken`` environment override).
"""
...
_MODULE_NAME = "_pipeline_end_for_modelops_token_routes"
...
def _load():
    ...
    reload re-resolves the same way. The modelOps and rapidPipeline copies are byte-identical, so
...
def _construct_pipeline_output():
    ...
    return {
        ...
        "commands": ["/bin/bash", "-c", "printf '%s' '{}' | index.js"],
...
def _completed_ecs_task_event():
    ...
    return {
        "TaskArn": "arn:aws:ecs:us-east-1:1:task/modelops-cluster/abc",
        "LastStatus": "STOPPED",
        "Overrides": {
            "ContainerOverrides": [{
                "Name": "ModelOpsContainerDefinition",
                "Command": ["/bin/bash", "-c", "printf '%s' '{}' | index.js"],
...
class TestTheCompletedTaskRouteReportsTheToken:
    ...
    must keep doing so, since the fields constructPipeline emitted are not in that shape."""
    ...
    def test_a_task_description_without_the_environment_name_still_resolves(self):
        ...
        name is carried in the description."""
...
@pytest.mark.unit
class TestADirectInvokeReportsNothing:

    def test_no_token_anywhere_skips_the_callback(self):
        event = _completed_ecs_task_event()
