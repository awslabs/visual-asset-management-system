...
def register_sub_execution(orchestration_bus_name, orchestration_event_prefix,
    ...
    detail = {
        ...
        "subExecution": {
            ...
            "label": "Cosmos Predict processing",
        },
    }
    logs = []
    if STATE_MACHINE_LOG_GROUP_NAME or STATE_MACHINE_LOG_GROUP_ARN:
        logs.append({
            ...
            "label": "Cosmos Predict state machine",
        })
...
def lambda_handler(event, context):
    """
    OpenPipeline
    Starts StepFunctions State Machine for processing Cosmos Predict pipeline.
    Validates input based on model type (text2world skips file validation, video2world requires valid file).
    """
    ...
    if model_type == "video2world":
        # Video2World requires a valid input file
        if not input_s3_asset_files_uri:
            abort_external_workflow("Input S3 URI is required for video2world", external_sfn_task_token)
            return {
                'statusCode': 400,
                'body': {
                    "message": "Input S3 URI is required for video2world"
                }
            }
        ...
        # Text2World does not require an input file
        logger.info("Model type is text2world, skipping file validation")

    # Validate prompt (required for text2world, optional for video2world)
    if model_type == "text2world" and not cosmos_prompt:
        abort_external_workflow("Cosmos prompt is required for text2world", external_sfn_task_token)
        return {
            'statusCode': 400,
            'body': {
                "message": "Cosmos prompt is required for text2world"
            }
        }
    ...
    try:
        ...
        responses.append({
            'statusCode': 200,
            'body': {
                "message": "Starting Cosmos Predict Pipeline State Machine",
