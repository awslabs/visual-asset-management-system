...
"""The name openPipeline runs the cosmos predict state machine under.

A workflow may carry several triggers of one type, so one upload fans out to simultaneous runs of the
SAME model type; Step Functions rejects a repeated name with ExecutionAlreadyExists, which
...
for _k, _v in {
    ...
    "STATE_MACHINE_LOG_GROUP_ARN":
        "arn:aws:logs:us-east-1:1:log-group:/aws/vendedlogs/CosmosPredict:*",
}.items():
...
class TestSubStateMachineExecutionName:
    def test_two_runs_of_the_same_model_type_get_different_names(self):
        # The fan-out case: same model type, same wall-clock second, different pipeline executions.
        mod = _open_pipeline()
        assert (mod.build_job_name("text2world", _prefix("P1"))
                != mod.build_job_name("text2world", _prefix("P2")))
    ...
    def test_the_same_run_always_derives_the_same_name(self):
        ...
        mod = _open_pipeline()
        prefix = _prefix("P1")
        assert mod.build_job_name("text2world", prefix) == mod.build_job_name("text2world", prefix)

    def test_a_direct_invocation_without_a_prefix_is_still_unique(self):
        mod = _open_pipeline()
        names = {mod.build_job_name("video2world", "") for _ in range(20)}
        assert len(names) == 20
    ...
    def test_the_name_obeys_the_step_functions_constraints(self):
        ...
        for prefix in (_prefix("a" * 32), ""):
            for model_type in ("text2world", "video2world"):
                name = mod.build_job_name(model_type, prefix)
                assert len(name) <= 80
                assert ":" not in name and "/" not in name

    def test_the_model_type_still_leads_the_name(self):
        mod = _open_pipeline()
        assert mod.build_job_name("video2world", _prefix("P1")).startswith("cosmos-video2world-")
        assert mod.build_job_name("text2world", _prefix("P1")).startswith("cosmos-text2world-")
...
class TestConcurrentFanOutThroughTheHandler:
    ...
    BASE = {"modelType": "video2world",
            "inputS3AssetFilePath": "s3://b/xid/in.mp4",
