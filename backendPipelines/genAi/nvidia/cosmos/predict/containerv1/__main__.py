"""
VAMS Cosmos Predict Container Wrapper

Orchestrates the full pipeline:
1. Load pipeline definition
2. Ensure all models are available (diffusion + tokenizer + T5-11B)
...
EFS_BASE_PATH = "/mnt/efs/cosmos-models"
...
def download_from_s3(s3_uri: str, local_path: Path) -> None:
    """
    Download file from S3 using AWS CLI (more reliable than boto3 in this container).
...
def upload_to_s3(local_path: Path, s3_uri: str) -> None:
    """
    Upload file to S3 using AWS CLI (more reliable after torchrun than boto3).
...
def main():
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("VAMS Cosmos Predict Pipeline Starting")
        logger.info("=" * 80)
        ...
        model_size = definition.get("modelSize", "7B")
        ...
        # INVALIDATE_COSMOS_MODELS: "true" → clear EFS/S3 cache, re-download from HuggingFace
        # DISABLE_GUARDRAILS: "true" → skip safety guardrail checks (faster, no Llama-Guard needed)
        ...
        try:
            ...
            if input_params:
                ...
                disable_guardrails = str(params.get("DISABLE_GUARDRAILS", "")).lower() == "true"
                generate_preview_gif_flag = str(params.get("GENERATE_PREVIEW_GIF", "")).lower() == "true"
                if invalidate_models:
                    logger.info("INVALIDATE_COSMOS_MODELS=true: will clear EFS/S3 cache and re-download all models")
                if disable_guardrails:
                    logger.info("DISABLE_GUARDRAILS=true: safety guardrails will be disabled for this run")
                if generate_preview_gif_flag:
                    logger.info("GENERATE_PREVIEW_GIF=true: will generate preview GIF after inference")
        except Exception:
            pass
        ...
        # Step 1: Ensure all models are available
        logger.info("=" * 80)
        logger.info("Step 1: Ensuring models are available")
        logger.info("=" * 80)
        ...
        for hf_model_id in required_models:
            model_dir = ensure_model_available(
                ...
                efs_base_path=EFS_BASE_PATH,
                s3_bucket=s3_model_bucket,
                hf_token=hf_token,
                invalidate=invalidate_models
            )
            model_dirs[hf_model_id] = model_dir
            logger.info(f"✓ {hf_model_id} → {model_dir}")
        ...
        # Determine if we should disable prompt upsampler
        disable_prompt_upsampler = False
        if model_type == "video2world" and prompt and prompt.strip():
        ...
        run_inference(
            ...
            efs_model_dir=EFS_BASE_PATH,
            num_gpus=4,
            disable_prompt_upsampler=disable_prompt_upsampler,
            disable_guardrails=disable_guardrails
        )

        # Free GPU/CPU memory after inference before S3 upload
        import gc
        import torch
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        gc.collect()
        logger.info("Freed GPU/CPU memory after inference")
        sys.stdout.flush()
        ...
        # Step 5: Determine output S3 key
        logger.info("=" * 80)
        ...
        if model_type == "video2world":
            ...
            # Output: {output_base}{relative_subdir}/{input_filename}_CosmosPredictVideo2World_{timestamp}.mp4
            relative_subdir = compute_relative_subdir(input_s3_asset_file_path, asset_id)
            input_filename_stem = Path(parse_s3_uri(input_s3_asset_file_path)[1]).stem  # filename without extension

            output_filename = f"{input_filename_stem}_CosmosPredictVideo2World_{timestamp}.mp4"
            output_key = f"{output_base}{relative_subdir}{output_filename}"
            output_s3_uri = f"s3://{output_bucket}/{output_key}"

        else:  # text2world
            output_filename = f"cosmos-text2world-{timestamp}.mp4"
        ...
        # Step 6: Generate and upload preview GIF (optional, controlled by GENERATE_PREVIEW_GIF param)
        # Default is false because after inference the system uses ~170GB RAM,
        # spawning ffmpeg can trigger OOM. Enable only if memory headroom is available.
        preview_s3_uri = ""
        if generate_preview_gif_flag:
            logger.info("=" * 80)
