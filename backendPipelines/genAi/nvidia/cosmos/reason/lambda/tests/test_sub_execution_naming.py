...
"""The name openPipeline runs the cosmos reason state machine under.

A workflow may carry several triggers of one type, so one upload fans out to simultaneous runs of
this pipeline; Step Functions rejects a repeated name with ExecutionAlreadyExists, which openPipeline
...
for _k, _v in {
    ...
    "STATE_MACHINE_LOG_GROUP_ARN":
        "arn:aws:logs:us-east-1:1:log-group:/aws/vendedlogs/CosmosReason:*",
}.items():
...
class TestSubStateMachineExecutionName:
    def test_two_concurrent_runs_get_different_names(self):
        # The fan-out case: same wall-clock second, different pipeline executions.
        mod = _open_pipeline()
        assert mod.build_job_name(_prefix("P1")) != mod.build_job_name(_prefix("P2"))
    ...
    def test_the_same_run_always_derives_the_same_name(self):
        ...
        mod = _open_pipeline()
        prefix = _prefix("P1")
        assert mod.build_job_name(prefix) == mod.build_job_name(prefix)

    def test_a_direct_invocation_without_a_prefix_is_still_unique(self):
        mod = _open_pipeline()
        names = {mod.build_job_name("") for _ in range(20)}
        assert len(names) == 20
    ...
    def test_the_name_obeys_the_step_functions_constraints(self):
        ...
        for prefix in (_prefix("a" * 32), ""):
            name = mod.build_job_name(prefix)
            assert len(name) <= 80
            assert ":" not in name and "/" not in name

    def test_the_pipeline_still_leads_the_name(self):
        mod = _open_pipeline()
        assert mod.build_job_name(_prefix("P1")).startswith("cosmos-reason-")
        assert mod.build_job_name("").startswith("cosmos-reason-")
