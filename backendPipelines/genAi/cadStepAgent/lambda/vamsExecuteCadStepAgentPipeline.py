...
"""
Lambda to execute the GenAI CAD STEP agent pipeline from VAMS workflows.
Note: Lambda function name must start with "vams" to allow invoke permissioning.

The pipeline takes ZERO or ONE input file: the modify template supplies one STEP file, the generate
template supplies none and the agent creates a new STEP file on the execution's output asset.
"""
import os
import boto3
import json
...
logger = safeLogger(service="VamsExecuteCadStepAgentPipeline")
...
def execute_pipeline(input_s3_asset_file_path, output_s3_asset_files_path,
    ...
    messagePayload = {
        # Empty for a generate run (no input file); openPipeline skips the extension gate then.
        "inputS3AssetFilePath": input_s3_asset_file_path or "",
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Zero (generate) or one (modify) input file per execution; more is a workflow authoring error.
        ...
        execute_pipeline(
            resolved.get('inputS3AssetFilePath') or "",
