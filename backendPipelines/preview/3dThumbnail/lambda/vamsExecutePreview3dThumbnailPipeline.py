...
logger = safeLogger(service="VamsExecutePreview3dThumbnailPipeline")
...
def abort_external_workflow(error, task_token):
    ...
    try:
        sfn_client.send_task_failure(
            ...
            error="Preview3dThumbnailPipelineError",
            cause=str(error)[:256]
        )
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Resolve input/output/aux locations from the workflow manifest, falling back to payload path fields
        ...
        execute_pipeline(resolved['inputS3AssetFilePath'],
