...
os.environ.setdefault("ALLOWED_INPUT_FILEEXTENSIONS", ".laz,.las,.e57,.ply")
...
def _load_open_pipeline():
    ...
    spec = importlib.util.spec_from_file_location("pcPotreeViewer_openPipeline_under_test", path)
...
def _body():
    ...
    return {
        ...
        "inputS3AssetFilePath": "s3://abkt/xid/scan/cloud.laz",
        ...
        "inputOutputS3AssetAuxiliaryFilesPath": "s3://aux/xid/scan/cloud.laz/preview/p1/",
    ...
    def test_execute_pipeline_raises_with_the_reported_error_type(self):
        ...
        with patch.object(mod.lambda_client, "invoke", invoke):
            with pytest.raises(Exception) as raised:
                mod.execute_pipeline("s3://abkt/xid/cloud.laz", "s3://abkt/files/",
                                     "s3://abkt/previews/", "s3://abkt/metadata/",
                                     "s3://aux/tmp/", "s3://abkt/metadata.json",
                                     "s3://abkt/config.json", TASK_TOKEN, "user@x", "")
...
class TestOpenPipelineAbortPropagates:
    ...
    def test_the_handler_propagates_so_the_invoke_reports_function_error(self):
        ...
        sfn.send_task_failure.side_effect = Exception("AccessDeniedException")
        event = {
            "inputS3AssetFilePath": "s3://abkt/xid/scan/",
