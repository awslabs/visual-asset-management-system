...
class TestNotifyQuarantine:
    @patch("backend.backend.handlers.fmm.fmmNotifications.sns_client")
    @patch("backend.backend.handlers.fmm.fmmNotifications.dynamodb_client")
    def test_publishes_to_asset_topic(self, mock_dynamo, mock_sns):
        ...
        from backend.backend.handlers.fmm.fmmNotifications import (
            notify_quarantine,
        )
