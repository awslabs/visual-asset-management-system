...
the extension is inserted between the output asset's location key and each output file's relative
path (final key = assetLocationKey + extension + relativePath). It defaults to '/' (no extra
segment) and applies to asset FILE outputs (path-structured), not previews (basename-only).

The upload lambda prepends the asset base key, so process_external_upload's job is to produce a
relativeKey of `<extension>/<relativePath>` for asset files. _collect_output_descriptors records
the same extended relativeFilePath so output provenance + the version-history join stay aligned.
"""
...
class TestProcessExternalUploadPathExtension:
    ...
    def test_extension_is_inserted_as_middle_segment(self):
        rels = self._run("/exec-2026/")
        # The extension lands between the asset location key (prepended later by the upload
        # lambda) and the file's relative path.
        assert rels[self.PREFIX + "test/pump.glb"] == "exec-2026/test/pump.glb"
        assert rels[self.PREFIX + "root.glb"] == "exec-2026/root.glb"

    def test_extension_normalized_without_surrounding_slashes(self):
        # A bare value (no slashes) is treated the same as a slash-wrapped one.
        assert self._run("exec-2026")[self.PREFIX + "root.glb"] == "exec-2026/root.glb"
...
class TestCollectDescriptorsPathExtension:
    ...
    def test_recorded_relative_path_includes_extension(self):
        ...
        assert descs[0]["relativeFilePath"] == "exec-2026/test/pump.glb"
