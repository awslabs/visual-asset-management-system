...
class TestDiscoverChildren:
    @patch("backend.backend.handlers.compliance.complianceCascadeExecutor.asset_links_table")
    def test_finds_direct_children(self, mock_table):
        ...
        from backend.backend.handlers.compliance.complianceCascadeExecutor import (
            discover_children,
        )
...
class TestDiscoverAllDescendants:
    @patch("backend.backend.handlers.compliance.complianceCascadeExecutor.discover_children")
...
class TestTopologicalSort:
    @patch("backend.backend.handlers.compliance.complianceCascadeExecutor._get_parent_keys")
...
class TestExecuteCascade:
    @patch("backend.backend.handlers.compliance.complianceCascadeExecutor.cascade_table")
    ...
    @patch("backend.backend.handlers.compliance.complianceCascadeExecutor.audit_table")
    @patch("backend.backend.handlers.compliance.complianceCascadeExecutor.cascade_table")
    @patch(
        "backend.backend.handlers.compliance.complianceCascadeExecutor.discover_all_descendants"
    )
    ...
    @patch(
        "backend.backend.handlers.compliance.complianceCascadeExecutor.topological_sort"
    )
    @patch(
        "backend.backend.handlers.compliance.complianceCascadeExecutor._get_asset_schema"
    )
    def test_skips_nodes_without_schema(
        ...
        mock_eval_engine = types.ModuleType("common.compliance.evaluationEngine")
        mock_eval_engine.evaluate_asset = MagicMock(return_value={})
        sys.modules["common.compliance.evaluationEngine"] = mock_eval_engine
