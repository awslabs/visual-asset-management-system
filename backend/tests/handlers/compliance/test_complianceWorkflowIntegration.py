...
class TestProcessPipelineResults:
    ...
    def test_failed_execution_marks_all_rules_failed(self):
        from backend.backend.handlers.compliance.complianceWorkflowCallback import (
    ...
    def test_succeeded_with_compliance_output(self):
        ...
        with patch(
            "backend.backend.handlers.compliance.complianceWorkflowCallback._read_compliance_output"
        ) as mock_read:
...
class TestReadComplianceOutput:
    ...
    def test_reads_from_metadata_path_key(self):
        ...
        with patch(
            "backend.backend.handlers.compliance.complianceWorkflowCallback.s3_client"
        ) as mock_s3:
...
class TestGetAssetInfo:
    ...
    def test_returns_asset_info_on_success(self):
        from backend.backend.common.compliance.evaluationEngine import _get_asset_info

        with patch(
            "backend.backend.common.compliance.evaluationEngine.asset_table"
        ) as mock_asset_table, patch(
            "backend.backend.common.compliance.evaluationEngine.s3_asset_buckets_table"
...
class TestInvokePipelineRules:
    ...
    def test_starts_execution_with_correct_input(self):
        from backend.backend.common.compliance.evaluationEngine import (
        ...
        with patch(
            "backend.backend.common.compliance.evaluationEngine._get_asset_info"
        ) as mock_info, patch(
            "backend.backend.common.compliance.evaluationEngine._get_workflow_arn"
        ) as mock_arn, patch(
            "backend.backend.common.compliance.evaluationEngine.sfn_client"
        ) as mock_sfn:
        ...
        assert "complianceContext" in input_metadata
        assert input_metadata["complianceContext"]["evaluationId"] == "eval-abc"
        assert input_metadata["complianceContext"]["ruleName"] == "coord-check"
