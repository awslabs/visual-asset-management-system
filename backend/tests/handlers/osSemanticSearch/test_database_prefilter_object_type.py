...
``CasbinEnforcer.enforce()`` on it, in both of its access paths (``get_accessible_database`` and
``get_accessible_databases``). For a role
...
``DatabaseAccessManager`` lives in ``common/databaseAccess.py`` and reaches ``search.py`` as a re-export.
Both modules are loaded from their file paths because the root conftest registers mock ``handlers``/
``common`` packages that shadow the real ones -- ``common.databaseAccess`` resolves to a no-access
stand-in and ``handlers.osSemanticSearch.search`` to ``tests/mocks/handlers/osSemanticSearch/search.py``. The real
``databaseAccess`` module is installed in ``sys.modules`` before ``search.py`` is loaded, so the class
the search module re-exports IS the object whose module globals (``dynamodb_client``,
``database_storage_table``, ``CasbinEnforcer``) these tests patch; the fixture asserts that identity.
"""
...
_SEARCH_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "backend", "handlers", "osSemanticSearch", "search.py"
)
...
@pytest.fixture
def access_module():
    """The real common.databaseAccess module, loaded by file path with boto3 stubbed and installed in
    sys.modules so the search module loaded next binds the SAME class object."""
    saved = {name: sys.modules.get(name) for name in ("handlers.authz", "common.databaseAccess")}
    ...
    try:
        with patch("boto3.client", side_effect=_boto_client), patch(
            ...
            spec = importlib.util.spec_from_file_location(
                "databaseAccess_under_test_prefilter", os.path.abspath(_ACCESS_PATH)
            )
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
    finally:
        if saved["handlers.authz"] is not None:
            sys.modules["handlers.authz"] = saved["handlers.authz"]
    sys.modules["common.databaseAccess"] = module
    yield module
    if saved["common.databaseAccess"] is not None:
        sys.modules["common.databaseAccess"] = saved["common.databaseAccess"]


@pytest.fixture
def search_module(access_module):
    ...
    assert module.DatabaseAccessManager is access_module.DatabaseAccessManager, (
        "search.py must re-export the class object from common.databaseAccess"
    )
    return module
...
@pytest.fixture
def spy(search_module, access_module):
    _EnforcerSpy.documents = []
    access_module.CasbinEnforcer = _EnforcerSpy
    return _EnforcerSpy


@pytest.fixture
def tag_scoped_role(search_module, access_module):
    _TagScopedRoleEnforcer.documents = []
    access_module.CasbinEnforcer = _TagScopedRoleEnforcer
...
def _stub_scan(access_module, pages):
    paginator = MagicMock()
...
class TestGetAccessibleDatabases:
    """The list path: common/databaseAccess.py, get_accessible_databases."""

    def test_the_enforcement_point_is_reached(self, search_module, access_module, spy):
        ...
        _stub_scan(access_module, [{"Items": [_DB_ROW_SERIALIZED]}])
    ...
    def test_no_enforcement_without_tokens(self, search_module, access_module, spy):
    ...
    def test_database_rows_are_not_typed_as_assets(self, search_module, access_module, spy):
        _stub_scan(access_module, [{"Items": [_DB_ROW_SERIALIZED]}])
...
class TestGetAccessibleDatabase:
    """The single-database path: common/databaseAccess.py, get_accessible_database. Covered separately because the fix has to change both
    ...
    def test_the_enforcement_point_is_reached(self, search_module, access_module, spy):
        access_module.database_storage_table = MagicMock()
        access_module.database_storage_table.get_item.return_value = {"Item": dict(_DB_ROW_PLAIN)}
        result = search_module.DatabaseAccessManager.get_accessible_database("smoke-db", _CLAIMS)
        assert result == "smoke-db"
        assert len(spy.documents) == 1

    def test_database_row_is_not_typed_as_asset(self, search_module, access_module, spy):
        access_module.database_storage_table = MagicMock()
        access_module.database_storage_table.get_item.return_value = {"Item": dict(_DB_ROW_PLAIN)}
...
class TestTagScopedRoleReachesItsDatabases:
    ...
    def test_the_granted_database_is_returned(self, search_module, access_module, tag_scoped_role):
        ...
        _stub_scan(access_module, [{"Items": [_db_row("smoke-db")]}])
        assert search_module.DatabaseAccessManager.get_accessible_databases(_CLAIMS) == ["smoke-db"]

    def test_an_ungranted_database_is_still_excluded(self, search_module, access_module, tag_scoped_role):
        ...
        _stub_scan(access_module, [{"Items": [_db_row("other-db")]}])
        assert search_module.DatabaseAccessManager.get_accessible_databases(_CLAIMS) == []

    def test_a_mixed_page_keeps_only_the_granted_database(self, search_module, access_module, tag_scoped_role):
        _stub_scan(
            access_module, [{"Items": [_db_row("other-db"), _db_row("smoke-db")]}]
        )
        assert search_module.DatabaseAccessManager.get_accessible_databases(_CLAIMS) == ["smoke-db"]

    def test_the_single_database_lookup_returns_the_granted_database(
        self, search_module, access_module, tag_scoped_role
    ):
        access_module.database_storage_table = MagicMock()
        access_module.database_storage_table.get_item.return_value = {"Item": dict(_DB_ROW_PLAIN)}
        assert (
    ...
    def test_the_single_database_lookup_denies_an_ungranted_database(
        ...
        access_module.database_storage_table = MagicMock()
        access_module.database_storage_table.get_item.return_value = {
