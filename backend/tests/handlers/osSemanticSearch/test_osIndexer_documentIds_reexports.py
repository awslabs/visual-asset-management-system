...
_INDEXING_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "backend", "handlers", "osSemanticSearch"
)
...
@pytest.fixture
def fileIndexer():
    return _load_indexer("osFileIndexer")


@pytest.fixture
def assetIndexer():
    return _load_indexer("osAssetIndexer")
...
class TestAssetIndexerReExport:
    ...
    def test_no_inline_asset_id_f_string_remains(self):
        with open(os.path.abspath(os.path.join(_INDEXING_DIR, "osAssetIndexer.py")), encoding="utf-8") as handle:
