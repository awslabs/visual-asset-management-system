...
logger = safeLogger(service_name="FetchVisualizerAsset")
...
def lambda_handler(event, context):
    ...
    for user_name in claims_and_roles["tokens"]:
        ...
        if casbin_enforcer.enforce(f"user::{user_name}", asset_object, "GET") and casbin_enforcer.enforce(
    ...
    if operation_allowed_on_asset:
        try:
            ...
            if file_size > 5.9 * 1024 * 1024:
                ...
                message = "Error: Visualizer File Size Chunk is Larger than ~5.9MB, a AWS service limit. Use a smaller header file Range for visualizer asset retrieval."
                error_response = {
                    'statusCode': 400,
                    'body': json.dumps({"message": message}),
                    'headers': {
            ...
            logger.exception(e)
            message = "Error Fetching Visualizer File from Path Provided"
            error_response = {
                'statusCode': 400,
                'body': json.dumps({"message": message}),
                'headers': {
