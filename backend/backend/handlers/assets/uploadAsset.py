...
newObject = {
    ...
    "assetLocation": {"Bucket": "bucket", "Key": "key"},
    "previewLocation": {"Bucket": "bucket", "Key": "key"},
    "authEdit": [],
...
def getS3MetaData(bucket, key, asset):
    ...
    asset["currentVersion"]["FileSize"] = str(resp["ContentLength"] / 1000000) + "MB"
    return asset
...
def updateParent(asset, parent):
    ...
    try:
        ...
        resp = table.query(
            KeyConditionExpression=Key("databaseId").eq(parentdbId) & Key("assetId").eq(parentId),
...
def iter_Asset(body, item=None):
    ...
    if "previewLocation" in body and body["previewLocation"] is not None:
        asset["previewLocation"] = {"Bucket": body["previewLocation"]["Bucket"], "Key": body["previewLocation"]["Key"]}

    asset["assetLocation"] = {"Bucket": body["bucket"], "Key": body["key"]}
    asset["assetType"] = body["assetType"]
...
def upload_Asset(body, queryParameters, returnAsset=False):
    ...
    try:
        resp = table.query(
            KeyConditionExpression=Key("databaseId").eq(body["databaseId"]) & Key("assetId").eq(body["assetId"]),
            ScanIndexForward=False,
        )
        # upload a new asset
...
def lambda_handler(event, context):
    ...
    try:
        ...
        (valid, message) = validate(
            {
                "databaseId": {"value": event["body"]["databaseId"], "validator": "ID"},
                "assetId": {"value": event["body"]["assetId"], "validator": "ID"},
                "assetType": {"value": event["body"]["assetType"], "validator": "FILE_EXTENSION"},
                "description": {"value": event["body"]["description"], "validator": "STRING_256"},
            }
        )
        if not valid:
            print(message)
