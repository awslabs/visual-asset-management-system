...
def create(ctx: click.Context, user_id: str, email: str, phone: Optional[str], json_output: bool):
    ...
    This command creates a new user in the Cognito user pool. Cognito generates a
    temporary password and emails it to the user directly; it is not returned in the
    response, so it cannot be read from the output or scripted against. The user must
...
def reset_password(ctx: click.Context, user_id: str, confirm: bool, json_output: bool):
    ...
    This command starts Cognito's password reset flow. Cognito emails the user a reset
    code — or, for an account that has never completed a first sign-in, a new invitation
    carrying a fresh temporary password. Nothing is returned in the response, so the new
    credential cannot be read from the output or scripted against. The user must change
